const n=`{
 "cells": [
  {
   "cell_type": "markdown",
   "id": "71c6c78533a149628ddf4562cfcfbd99",
   "metadata": {},
   "source": [
    "# 智能体记忆：让上下文超出窗口"
   ]
  ...
  {
   "cell_type": "markdown",
   "id": "2e33dd529cbb4a8cb43a6192e9d7fd72",
   "metadata": {},
   "source": "> 上一讲我们把 Agent 放进长代码库和长 issue 里反复工作，直接撞上上下文窗口的边界：信息太多，窗口装不下。这一讲正面处理这条边界，讨论怎样让一个窗口有限的 Agent 拥有接近无限的记忆。\\n>\\n> 我们从三条路线分别作答。MemGPT 把上下文当作稀缺的内存，让模型自己决定哪些信息换出去、哪些换回来。Cartridges 不搬运文本，把整份语料压成一组可训练的向量，模型直接读这组向量。CacheBlend 在工程层面把已经算过的中间结果存下来重复使用。三条路线合起来，是本讲对记忆问题的完整回答。"
  },
  {
   "cell_type": "markdown",
   "id": "cf318a0c473947e89325b7e30d7035f6",
   "metadata": {},
   "source": "和 ChatGPT 这类模型聊天，聊得久了，它常常忘了开头说过的话。一开始我们告诉它自己的名字、喜欢的口味，聊到后面它全想不起来。更彻底的是，关掉对话新开一个，它什么都不记得，好像之前的一切都没发生过。\\n\\n原因是上下文窗口。第 1 讲我们说过，模型一次只能读一段固定长度的文本，这段文本叫上下文窗口。窗口里放得下的信息，模型看得到；放不下的，对模型来说就不存在。对话一长，前面的消息装不进去，就被挤掉了。这不是模型笨，而是它的固定设计：模型本身不会保存任何东西。\\n\\n真实系统里，Agent 需要记住信息才能把事办成。一个客服助手三个月前接待过一位用户、记住了他的称呼和偏好，三个月后用户再回来，它一开口就能叫对名字；如果什么都不记得，用户得重新填一遍表单。一个要写书、要维护大项目的 Agent 也一样，只靠窗口里的信息，做到一半就会忘记最初的目标。可以这样理解记忆：我们自己的记忆分两部分，正在想着的是短期记忆，存了很久、随时能调出来的是长期记忆。模型的窗口只相当于短期记忆，它还需要一个长期记忆。\\n\\n这一讲解决的就是这个问题：让一个窗口有限的 Agent，拥有超出窗口的记忆。做法是把窗口当成稀缺资源来管理，放不下就把信息挪到外面存起来，需要时再取回来。学完这一讲，我们能亲手实现一个跨轮次、跨对话都能记住信息的 Agent。在第 1 讲的 Agent 框架里，这补上的正是\\"感知\\"这一环：Agent 得先记得自己做过什么，才能决定下一步做什么。我们先从最直观的问题入手：一个 Agent 的窗口到底能撑几轮对话。"
  },
  {
   "cell_type": "markdown",
   "id": "d1a05c39d2b0472f9fb41e7380a6aea8",
   "metadata": {},
   "source": "## 1. 记忆为什么重要\\n\\n这一节先弄清楚一件具体的事：一个 Agent 的窗口到底能撑几轮对话。真实系统里，工程师要提前估计对话能持续多久，才能决定什么时候该上记忆机制。我们先认识衡量文本的最小单位，再估算一段对话的消耗，最后看窗口内部哪里记得牢、哪里容易忘。\\n\\n模型处理的文本不是按\\"字\\"算的，而是按一种更小的单位算，这个单位叫 token。英文里一个 token 大约对应四分之三个单词，中文里一个字常占一到两个 token。上下文窗口给每个 Agent 划了一条硬边界：模型推理时只能看到窗口内的 token，窗口外的一切都不存在。\\n\\n给一段对话的各个组成部分分配一个量级，就能估算它能持续多少轮：\\n\\n| 组成部分 | 每轮量级（token） |\\n|:---|:---|\\n| 系统指令（固定） | 约 1000 |\\n| 用户消息 | 约 200 |\\n| 助手回复 | 约 300 |\\n| 检索结果 | 约 200 |\\n\\n以 8k 窗口为例，扣掉固定开销后还有约 7000 token 留给对话，每轮消耗约 700 token，十轮左右窗口就被占满。窗口扩大到 128k 也只是把这一刻推迟，不能消除它。\\n\\n窗口内部的记忆强度也不均匀。Lost in the Middle 的实证表明，模型对上下文两头的 token 记忆更牢，中间部分容易被忽略。把窗口塞满并不等于记住全部信息。要突破窗口的限制，只能改变信息的组织方式：逐出、压缩、复用。我们从最直观的窗口预算开始，看对话到底在第几轮撑满窗口。"
  },
  {
   "cell_type": "code",
   "execution_count": null,
   "id": "af9a9d3b70404f63b5708c13def47c81",
   "metadata": {},
   "outputs": [],
   "source": [
  ...
  },
  {
   "cell_type": "code",
   "execution_count": null,
   "id": "f85034968e904838976ec8ae5347fbc0",
   "metadata": {},
   "outputs": [],
   "source": [
    "# 可视化：主上下文占用随轮次线性增长\\n",
  ...
  },
  {
   "cell_type": "markdown",
   "id": "5b2fc603",
   "source": "### 窗口为什么会满：逐轮手算\\n\\n这一小节把上一节的估算一步一步算出来，看窗口占用怎么逐轮增长、为什么到第 10 轮就满了。\\n\\n上下文窗口是模型单次推理能看到的 token 总数，窗口之外的内容对它不存在。估算对话规模不必精确，我们约定：一条用户消息估成 200 token，一条助手回复估成 300 token，一次检索结果估成 200 token，合计每轮约 700 token。\\n\\n占用增长的关键在累积。默认情况下，模型每轮看到的提示包含此前所有轮次的消息，用户消息、助手回复、检索结果都被原样保留，第 t 轮的窗口占用是前面所有轮次之和。因此占用随轮次线性增长。以 8k 窗口（8000 token）为例，逐轮列出来：\\n\\n| 轮次 | 窗口占用（token） | 说明 |\\n|:---|:---|:---|\\n| 0 | 1000 | 只有系统指令 |\\n| 1 | 1700 | +700 |\\n| 2 | 2400 | +700 |\\n| 3 | 3100 | +700 |\\n| 4 | 3800 | +700 |\\n| 5 | 4500 | +700 |\\n| 6 | 5200 | +700 |\\n| 7 | 5900 | 越过 70% 警告线（5600） |\\n| 8 | 6600 | +700 |\\n| 9 | 7300 | +700 |\\n| 10 | 8000 | 占满窗口 |\\n\\n算术过程：扣掉固定系统指令后，可用预算为 $8000 - 1000 = 7000$ token；每轮新增 700 token，能撑 $7000 \\\\div 700 = 10$ 轮。70% 警告线在 $0.7 \\\\times 8000 = 5600$ token，减去系统指令后剩 4600 token，$4600 \\\\div 700 \\\\approx 6.6$，所以第 7 轮越过警告线。把窗口撑到 128k 只是把同样的算术推迟，第 t 轮的占用仍是 $1000 + 700t$，占满只是时间问题。\\n\\n窗口占满之后怎么办，本讲从三个层面回答。最直接的是把最老的消息逐出主上下文、需要时再取回，这是 MemGPT 的调度路线。也可以不搬运文本，把整份语料离线压进一组可训练向量，模型查询时直接读取这组向量，这是 Cartridges 的压缩路线。还可以把算好的中间结果缓存下来，同一段内容反复出现时免去重算，这是 CacheBlend 的复用路线。",
   "metadata": {}
  },
  {
   "cell_type": "markdown",
   "id": "3b1dc70f23774390ae0eb579dc69e2df",
   "metadata": {},
   "source": "占用不断增长，主上下文必须保持有界。信息本身不会消失，但放不下的时候，得有人决定哪条消息让出位置。这个决定叫逐出（eviction）。操作系统、数据库缓存、浏览器的标签页，都用逐出规则决定先丢谁，模型的外接记忆也一样。\\n\\n三种逐出策略各按一条标准。FIFO 按到达顺序，最先进来的先走。重要性打分按语义价值，最不重要的先走。LRU 按最近使用时间，最久没被用到的先走。三种策略对同一批消息会选出不同的让位者。我们用一段玩具数据比较它们。"
  },
  {
   "cell_type": "code",
   "execution_count": null,
   "id": "ef6b4f7ccfbb49cc8b5a3343efa7e45e",
   "metadata": {},
   "outputs": [],
   "source": [
  ...
  },
  {
   "cell_type": "markdown",
   "id": "e45cb462",
   "source": "### 三种逐出策略各选谁\\n\\n这一小节把上一步的玩具数据逐项读出来，看每种策略到底让谁先走、为什么是它。\\n\\n上面四条记忆各有三个字段：ts 是到达顺序，importance 是语义重要性打分，last_used 是最近一次被用到的时间。把四条记忆列全：\\n\\n| id | ts（到达顺序） | importance（重要性） | last_used（最近使用） |\\n|:---|:---|:---|:---|\\n| a | 0 | 0.3 | 5 |\\n| b | 1 | 0.9 | 8 |\\n| c | 2 | 0.6 | 2 |\\n| d | 3 | 0.2 | 0 |\\n\\nFIFO 按到达顺序让位：ts 最小的是 a（ts=0），所以逐出 a。它模拟最朴素的直觉，最早进来的消息已经过时。坏处是可能把重要的旧信息一起丢掉，用户三个月前留下的住址如果落在最老的消息里，FIFO 会先把它让出去。\\n\\n重要性策略按语义价值让位：importance 最低的是 d（0.2），所以逐出 d。它需要一个打分机制判断每条记忆值不值得留，MemGPT 里这个分由模型自己给。坏处是打分本身可能出错，而且判断不看使用频率。\\n\\nLRU 按使用时间让位：last_used 最小的是 d（0），所以逐出 d。它利用\\"最近用过的记忆近期还会用\\"这一规律，适合对话这类强时间局部性的场景；代价是它无法区分一条高频且有价值的旧记忆与一条低频的普通记忆。\\n\\n同一批记忆，三种策略选出不同的让位者：FIFO 选 a，重要性与 LRU 都选 d。没有一种策略在所有场景最优，真实系统常组合使用，先按重要性分层，层内再按 LRU。",
   "metadata": {}
  },
  {
   "cell_type": "code",
   "execution_count": null,
   "id": "82fedb046b09404c8c05fd48ad1566d1",
   "metadata": {},
   "outputs": [],
   "source": [
    "# 可视化：三种策略下记忆内容随轮次变化\\n",
  ...
  {
   "cell_type": "markdown",
   "id": "f8eaa752a31c4c3ba7b480c0f77ce7c0",
   "metadata": {},
   "source": "## 2. 把上下文当作有限内存\\n\\n这一节解决一个问题：窗口满了之后，信息搬到哪里去、需要时怎么再搬回来。真实系统里，客服助手把用户偏好存进长期记忆、程序员助手把常用的代码库知识存起来跨会话使用，用的都是\\"放不下的挪出去、需要时取回来\\"的思路。这一节我们亲手实现一套这样的两级存储。\\n\\n前面我们说过，我们自己的记忆分短期和长期：正在想着的是短期记忆，存了很久、随时能调出来的是长期记忆。Agent 也需要这两样。上下文窗口只相当于短期记忆，放不下整个项目的全部信息；窗口之外要有一个长期记忆，才能记住跨会话的知识。MemGPT 这个方案借鉴了操作系统的安排：物理内存之外加一块磁盘，暂时用不到的换到磁盘、需要时再换回来。对应到模型上，上下文窗口相当于内存，外部存储相当于磁盘，模型自己决定什么换出、什么换回。\\n\\nMemGPT 把主上下文分成三段固定顺序的部分。system instructions 只读，写明控制流与函数用法；working context 是一段可读写的定长文本，专门放用户关键事实与偏好；FIFO queue 是滚动消息历史，存最近几轮对话。主上下文之外是外部上下文，包含两个库：recall storage 存全部历史消息（永不删除），archival storage 存需要长期保存的档案。信息从主上下文换出后进入外部上下文，需要时由模型发函数调用取回。下面的数据结构把这两级存储落成代码。"
  },
  {
   "cell_type": "code",
   "execution_count": null,
   "id": "b2df18bd1bf84a6e8d1406992dc5e5e3",
   "metadata": {},
   "outputs": [],
   "source": [
    "# 两级记忆存储：主上下文与外部上下文\\n",
  ...
  },
  {
   "cell_type": "markdown",
   "id": "9e2fd103",
   "source": "### 主上下文与外部上下文的分工\\n\\n这一小节把两级存储的分工逐条说清：哪部分快、哪部分大、信息怎么在两个库之间移动。\\n\\n上面的 MainContext 与 ExternalContext 就是两级存储的代码形态。主上下文对应操作系统里的物理内存：快、能被模型直接看到、空间有限。外部上下文对应磁盘：容量大、但模型无法直接看到内容，需要主动去取。代码里用字符数近似 token 数，tokens 方法返回的正是这个近似值。\\n\\n主上下文内部按固定顺序排三段。system 是只读指令，写清楚模型的职责与可用函数；working context 是一段定长可读写文本，放用户的关键事实与偏好，比如\\"用户：小明，水果：苹果\\"；FIFO queue 是滚动消息历史，只存最近几轮对话。三段内容按顺序拼起来，就是喂给模型的提示。\\n\\n外部上下文有两个库。recall storage 记录全部历史消息，只进不出，任何一条消息都不会真正丢失；archival storage 存需要长期保留的档案，按关键词组织。换出把消息从主上下文移进外部上下文，但不删除，消息只是从\\"模型看得到\\"变成\\"需要时才能取回\\"。\\n\\n召回是换出的逆过程。模型收到问题后先调用检索函数在 recall 库里找相关消息，把命中内容放回主上下文，再生成回答。上面的代码演示了完整一圈：检索\\"苹果\\"从 recall 库命中\\"我叫小明，我最爱的水果是苹果\\"，返回给模型。\\n\\n两级结构的要点是换出解决容量问题、召回解决可用性问题。消息永远不丢，代价是模型要清楚在什么时机、用什么关键词去取。这正是 MemGPT 让模型自己决定读写记忆的原因，调度逻辑放在模型的函数调用里，而不是写死在外部程序里。",
   "metadata": {}
  },
  {
   "cell_type": "markdown",
   "id": "7386c82d8d0a429c880a071abc6452b9",
   "metadata": {},
   "source": "现在剩下一个问题：模型怎么自己决定写哪条记忆、查哪条记忆。做法是让模型通过函数调用来读写记忆。每个函数带一份说明，写明它的名字、要传的参数、以及它是干什么的。函数执行器解析模型输出里的函数调用，校验参数后执行，把结果（成功信息或错误）回喂给模型。下面实现四个最常用的函数：向 working context 追加、替换 working context、往 archival 写入、检索 recall 库。"
  },
  {
   "cell_type": "code",
   "execution_count": null,
   "id": "39a339a861b54efc800dff2a654e149d",
   "metadata": {},
   "outputs": [],
   "source": [
    "import re\\n",
  ...
  {
   "cell_type": "markdown",
   "id": "223ee8853f93490f88d8868496d8453e",
   "metadata": {},
   "source": "把上面的组件拼成一个最小的 MemGPT 式循环，我们给它起名 ToyAgent。ToyAgent 每轮接收一条消息，写入主上下文与 recall 库；maintain 维护预算，超限时逐出最老的消息，并把旧摘要和这条消息合并成一份新摘要，这叫递归摘要；ask 用关键词检索外部存储，把命中放回主上下文再调用模型。下面先初始化 LLM 客户端，再定义这个 agent。"
  },
  {
   "cell_type": "code",
   "execution_count": null,
   "id": "e03787daaf5246bd85c5edddf12c4062",
   "metadata": {},
   "outputs": [],
   "source": [
    "import os\\n",
    ...
    "        \\"\\"\\"用 LLM 生成递归摘要，真实 API 演示退化为确定性拼接。\\"\\"\\"\\n",
    "        if False:\\n",
    "            return old + \\" ~ \\" + evicted\\n",
    "        prompt = f\\"已有摘要：{old}\\\\n新逐出消息：{evicted}\\\\n请合并成新摘要：\\"\\n",
  ...
  },
  {
   "cell_type": "code",
   "execution_count": null,
   "id": "b9656469ee824a998b61e23265656d1d",
   "metadata": {},
   "outputs": [],
   "source": [
  ...
  },
  {
   "cell_type": "code",
   "execution_count": null,
   "id": "bd072549084d46e092b7248854b5ed7f",
   "metadata": {},
   "outputs": [],
   "source": [
    ...
    "print(\\"模型回答:\\", reply, \\"（真实 API 演示为占位输出）\\")\\n",
  ...
  },
  {
   "cell_type": "markdown",
   "id": "499a1cc8",
   "source": "### 维护预算与递归摘要\\n\\n这一小节仔细看 maintain 和 ask 两个方法：逐出为什么用 while 而不是 if，摘要为什么叫递归，被逐出的信息是怎么被找回的。\\n\\nToyAgent 的 maintain 把预算守成一条线：只要主上下文 token 数超过 budget，就逐出队列最老的一条消息，并做两件事。第一件是把被逐消息记进 recall 库、标注\\"已逐出\\"，这样消息没有真正消失。第二件是把旧摘要与被逐消息合成一条新摘要。\\n\\n摘要采用递归结构。已有摘要 summary 压缩了此前所有被逐消息，新逐出一条时，把 old 与 evicted 合并成新摘要。递归的含义是每轮摘要在前一轮基础上增量更新，而不是重新压缩全部历史。代码里 if False 的那条分支用字符串拼接模拟合并，真实系统里这一步由一次 LLM 调用完成。\\n\\nmaintain 用 while 而不是 if，因为逐出一条不一定够。逐出后若还在预算之上，就继续逐出、继续更新摘要，直到占用回落到 budget 以内。这段对话里 system 约 48 字符，四条消息各约 9 到 20 字符，第 2 条消息起每轮占用都超过 70 的预算，所以几乎每轮都会触发逐出。\\n\\nask 演示召回。用户问\\"我最爱的水果是什么？\\"，agent 用关键词\\"水果\\"检索 recall 库，命中两条：最初写入的原始消息，以及逐出时写入的副本，内容都是\\"我叫小明，我最爱的水果是苹果，住在北京。\\"。命中放回主上下文后，模型才回答。提问前\\"苹果\\"已经不在主上下文，检索后它重新出现。\\n\\n被逐出的事实经 recall.search 找回后重新进入主上下文，这是两级存储的核心价值：容量受限不会导致信息丢失，只会提高取回的成本。",
   "metadata": {}
  },
  {
   "cell_type": "code",
   "execution_count": null,
   "id": "5bcd1dbd2b0f44bc931d6493bf844022",
   "metadata": {},
   "outputs": [],
   "source": [
    "# 可视化：主上下文占用保持在预算内\\n",
  ...
  {
   "cell_type": "markdown",
   "id": "d34229164d344a9b87cdadcb51daf2f7",
   "metadata": {},
   "source": "## 3. 把旧信息压缩成可复用表示\\n\\n上一节的 MemGPT 搬运的是文本，每一步换入换出都要让模型重读一遍，很费推理。这一节换一条思路：不搬文本，提前把整份语料压成一组可以调整的向量，模型需要时直接读这组向量。真实系统里，把一本书、一个代码库的知识压缩成能反复查询的表示，用的就是这类做法。\\n\\n一段知识可以用一串数字表示，这串数字叫向量。模型读文本，最终也会把每个词变成一组向量。给模型输入的最前面加上一组可以训练的向量，模型就能通过注意力去读它们，从里面取知识。注意力按相似度挑选向量来读，这种\\"在一堆向量里按相似度找最相关的内容\\"的做法叫向量检索，RAG 和记忆系统里都会用到。\\n\\n加在输入前面的这组向量没有对应文字，这个做法叫前缀调优（prefix-tuning）。把一份语料单独训练出来的那组向量，就叫 cartridge。一份 cartridge 相当于把这份语料的知识提前压好，之后任意次数的问题都能复用同一份。下面先把模型\\"读向量\\"背后的 KV 缓存讲清楚，再实现并比较两个训练目标。"
  },
  {
   "cell_type": "markdown",
   "id": "321c1f8a",
   "source": "### KV 缓存：记忆的载体\\n\\n这一小节补一个 Cartridges 需要的前置概念：模型是怎么把一段信息\\"读\\"进去的。理解了这个，才能明白为什么把知识压成向量就能让模型记住它。\\n\\n模型读文本时，会把每个词变成一组数字向量，按用途分成三类，叫 query、key、value。模型回答一个问题时，拿自己的 query 去和每个词的 key 比对相似度，相似度越高就越注意那个词的 value，最后把所有 value 按注意程度加权求和，得到这一步的输出。整段文本的 key 和 value 只算一遍就能存下来，之后生成每一个新词都直接复用，不必重算。这组存下来的中间结果叫 KV 缓存，预先算好它再开始生成，这一步叫 prefill。\\n\\nKV 缓存对记忆的意义在于：一段信息只要被编码成 key 和 value，模型的 query 就能通过注意力去读它。文本 token 是记忆的一种编码方式，KV 缓存里的数字向量是另一种。Cartridges 走的就是后者，不搬文本，只搬已经压好的向量。\\n\\n前缀调优把这种想法再推一步。正常使用时，输入开头是文本 token，模型为它们算 key 和 value。如果把开头换成 p 个没有对应文字、可以训练的向量，模型照样通过注意力读它们。这 p 个向量就是虚拟 token，数量 p 通常远小于语料的 token 数。\\n\\ncartridge 就是一份语料单独训练出来的那组虚拟向量。代码里每层各维护一组形状 [p, d] 的向量，真实模型权重全部冻结，只训练这些向量。27 token 的玩具语料，每层只配 8 个虚拟 token，训练在离线完成，成本被反复查询摊薄：语料只训一次，之后任意次数的问题都复用同一份 cartridge。\\n\\n训练目标很关键。直接做 next-token prediction，模型学的是\\"下一个词是什么\\"，只能背诵语料，答不了问题。context-distillation 换一个目标，让模型学会\\"语料在上下文时怎么答问题\\"。下面用两个小分布手算这个目标。",
   "metadata": {}
  },
  {
   "cell_type": "code",
   "execution_count": null,
   "id": "cff344b3106949a5bf7b697059e8b995",
   "metadata": {},
   "outputs": [],
   "source": [
  ...
  },
  {
   "cell_type": "markdown",
   "id": "d219b955",
   "source": "### KL 散度：分布之间的距离\\n\\n这一小节解释上一步代码里的损失函数：它算什么、为什么这么算。\\n\\ncontext-distillation 的损失函数是 KL 散度，它度量两个概率分布的接近程度。定义是 $KL(P \\\\| Q) = \\\\sum_i p_i \\\\log \\\\frac{p_i}{q_i}$，对每个类别取 p 与 q 的对数比值，再按 p 加权求和。可以把 log(p/q) 理解成用 q 代替 p 时的惊讶程度，KL 就是这个惊讶程度的加权平均。\\n\\n上面代码里教师分布把 0.90 的权重放在正确答案上，初始学生是均匀分布，每个类别 0.20。逐项手算：\\n\\n| 类别 | p（教师） | q（初始学生） | p·ln(p/q) |\\n|:---|:---|:---|:---|\\n| 答案 | 0.90 | 0.20 | 0.90 × ln 4.5 ≈ 1.354 |\\n| 噪声 1 | 0.02 | 0.20 | 0.02 × ln 0.1 ≈ −0.046 |\\n| 噪声 2 | 0.02 | 0.20 | 0.02 × ln 0.1 ≈ −0.046 |\\n| 噪声 3 | 0.03 | 0.20 | 0.03 × ln 0.15 ≈ −0.057 |\\n| 噪声 4 | 0.03 | 0.20 | 0.03 × ln 0.15 ≈ −0.057 |\\n\\n五项相加，$1.354 - 0.046 - 0.046 - 0.057 - 0.057 \\\\approx 1.148$。答案类别贡献了最大的一项：教师认为它该有 0.90，学生只给 0.20，这种低估在加权求和里被放大。噪声类别虽然也对不上，但 p 本身很小、权重低，负值被压住。KL 恒非负，只有 P 与 Q 完全相同时才为 0，蒸馏后学生与教师相同，KL 降到 0。\\n\\nKL 不对称，$KL(P\\\\|Q) \\\\ne KL(Q\\\\|P)$，严格说它不是距离。这里总是写 KL(教师‖学生)，因为教师是目标分布、学生是被训练的一方，方向不能反。\\n\\n玩具训练里教师给答案 0.85，上面的手算例子取 0.90，两者都只是软标注的不同取法。真正的理由在真实系统里：教师是模型把语料放进上下文后的 softmax 输出，本身就是一个软分布，不会把全部概率押在一个词上。玩具用解析形式模拟这种软标注，答案占 0.85、剩余概率摊到所有类别上。硬标注（1.0）会把学生推向过度自信，也失去了教师分布的细节。",
   "metadata": {}
  },
  {
   "cell_type": "code",
   "execution_count": null,
   "id": "8fd71a24cca042a4b8e93806e41bd0ee",
   "metadata": {},
   "outputs": [],
   "source": [
  ...
  },
  {
   "cell_type": "code",
   "execution_count": null,
   "id": "7e69224969244aaa8d8976074c56928e",
   "metadata": {},
   "outputs": [],
   "source": [
  ...
  },
  {
   "cell_type": "code",
   "execution_count": null,
   "id": "eb15943334354d4e81f38e0a48a5d601",
   "metadata": {},
   "outputs": [],
   "source": [
  ...
  },
  {
   "cell_type": "code",
   "execution_count": null,
   "id": "4ce864ef7d13485a89e4b6b0f01e89b8",
   "metadata": {},
   "outputs": [],
   "source": [
  ...
  },
  {
   "cell_type": "code",
   "execution_count": null,
   "id": "cba1866bfcf542549a9ea1cb26383d55",
   "metadata": {},
   "outputs": [],
   "source": [
  ...
  },
  {
   "cell_type": "code",
   "execution_count": null,
   "id": "f586d9bf6e3741e3971119ef5df0d98c",
   "metadata": {},
   "outputs": [],
   "source": [
  ...
  {
   "cell_type": "markdown",
   "id": "751aaffbfc4148efa04ddb76a5283689",
   "metadata": {},
   "source": "## 4. 复用已经算过的上下文\\n\\n这一节要解决另一类记忆问题：同一段知识被反复使用时，怎么省掉重复的计算。真实系统里，RAG（检索增强生成）把大文档切成多个文本块，用户问不同问题时，把相关文本块拼进输入。整段输入每次都从头算，很慢，直接决定用户看到第一个字要等多久。这一节我们要弄明白：哪些计算可以复用，复用会损失什么，怎么在两者之间权衡。\\n\\n要复用的，是模型\\"读一段文本\\"时产生的中间结果。把整段输入重新算一遍很贵，把算过的中间结果缓存下来、同一段内容反复出现时直接复用，就能省下这部分计算。代价是缓存与真实计算之间有偏差，于是有三种做法：只复用公共前缀的 prefix caching、全部复用的 full KV reuse、以及按需重算一部分的 CacheBlend。\\n\\n三类方案各有短板。prefix caching 只复用公共前缀 chunk 的 KV，前缀之后的部分照常重算，命中率有限；full KV reuse 复用所有 chunk 的 KV，却忽略了 chunk 之间的跨 chunk 注意力，质量受损。CacheBlend 提出选择性重算：按层只重算一小部分 token 的 KV，其余沿用缓存，同时拿到复用速度与全量重算质量。\\n\\n选择性重算的直觉来自注意力稀疏性：只有少数\\"跨界\\"token 的 KV，在单独计算与整段计算之间偏离最大，重算它们就足够恢复跨 chunk 注意力。下面先手算一个 4 token 的注意力例子，再看三种方案的注意力矩阵。"
  },
  {
   "cell_type": "markdown",
   "id": "016a5da4",
   "source": "### prefill、复用与选择性重算\\n\\n这一小节把三种方案各做了什么、各自的短板在哪里，逐个讲清楚，再引出选择性重算的核心直觉。\\n\\nRAG 把文档切成多个 chunk，每个 chunk 先单独 prefill 并把 K/V 缓存下来。用户查询到来时如果整段输入重新 prefill，几万 token 的 K/V 都要重算，首 token 延迟显著增大。复用缓存就是为了省掉这部分重复计算。\\n\\n最朴素的是 prefix caching：只复用公共前缀的 K/V，前缀之后的部分照常 prefill。它要求两次请求共享同一个开头，而 RAG 里不同查询的文本往往不同，前缀复用的命中率有限。\\n\\nfull KV reuse 把每个 chunk 缓存好的 K/V 全部加载，不再 prefill 任何 chunk。问题出在 chunk 是分开 prefill 的：一个 token 在单独 prefill 时，它的中间表示只和自己的 chunk 交互过；整段 prefill 时，它通过注意力与跨 chunk 的 token 交互，中间表示不同。因此缓存 K/V 与真实 K/V 存在偏差，注意力质量受损。\\n\\n偏差的分布不均匀。靠近 chunk 边界的 token 受跨 chunk 注意力影响最大，偏差也最大；远离边界的 token 几乎不受影响。选择性重算正是利用这一点：按 token 逐个判断，只重算偏差最大的那一小部分，其余沿用缓存。重算比例 r 决定质量与速度的取舍，r 越小越快，压得过低则损失质量。\\n\\n\\"token 级\\"说的是判断单位是单个 token 而不是整个 chunk。缓存里每个 token 的 K/V 独立决定用缓存值还是重算值，重算的候选是一个集合而不是一个连续的段。下面用一个 4 token 的小注意力矩阵把这个权衡手算一遍。",
   "metadata": {}
  },
  {
   "cell_type": "code",
   "execution_count": null,
   "id": "100236447c1d428384854115a47bb072",
   "metadata": {},
   "outputs": [],
   "source": [
  ...
  },
  {
   "cell_type": "markdown",
   "id": "6f27716d",
   "source": "### 手算：重算两个跨界 token 恢复注意力分数\\n\\n这一小节把上一步代码里的 4 个 token 例子手算一遍，验证\\"重算边界 token 就能恢复注意力分数\\"。\\n\\n上面代码的玩具数据是 4 个上下文 token，chunk A = [a1, a2]，chunk B = [b1, b2]。四个 key 取成四维基向量，a1=[1,0,0,0]、a2=[0,1,0,0]、b1=[0,0,1,0]、b2=[0,0,0,1]，query 取 q=[0.7,0.3,0.5,0.2]。\\n\\n全量重算时，注意力分数是 query 与每个 key 的点积。因为 key 是基向量，每个分数正好等于 q 对应坐标，s_full = [0.7, 0.3, 0.5, 0.2]。逐个验证：q 与 a1 点积得 0.7，与 a2 点积得 0.3，与 b1 点积得 0.5，与 b2 点积得 0.2。\\n\\n缓存里 a2 与 b1 偏离了：a2 的 key 掺进了 chunk B 方向的坐标，b1 掺进了 chunk A 方向的坐标，a1 与 b2 保持原样。q 与缓存 key 的点积变成 s_cache = [0.7, 0.71, 0.62, 0.2]，偏差 |s_cache − s_full| = [0, 0.41, 0.12, 0]。分数偏差提示哪些 token 的 key 该重算，但它依赖具体 query，CacheBlend 用更稳的 KV deviation，即 K 向量本身到真实值的欧氏距离。\\n\\n逐 token 算 KV deviation。a2 的缓存 key 与真实 key 相差 [0.5, −0.2, 0.2, 0.1]，距离 $\\\\sqrt{0.5^2+0.2^2+0.2^2+0.1^2} \\\\approx 0.58$；b1 相差 [0.1, 0.2, −0.1, 0.2]，距离 $\\\\sqrt{0.1^2+0.2^2+0.1^2+0.2^2} \\\\approx 0.32$；a1 与 b2 都是 0。偏差最大的是 a2 与 b1，恰好是紧贴 chunk 边界的两个 token。a1 在最开头、b2 在最结尾，单独 prefill 与整段 prefill 的结果一致，把重算预算花在它们身上没有收益。\\n\\nr=0.5 表示重算 4 个里偏差最高的 2 个，即 a2 与 b1。把它们的 key 换回真实值后，分数回到 s_cb = [0.7, 0.3, 0.5, 0.2]，与全量重算一致。这个按 KV deviation 选 token 的做法在论文里叫 HKVD（Highest KV Deviation，最高键值偏离）。玩具说明一件事：跨 chunk 注意力只需修复边界附近的少数 token，就能把注意力分数拉回全量水平。",
   "metadata": {}
  },
  {
   "cell_type": "code",
   "execution_count": null,
   "id": "75e234cd77034b0799576b19e5b7c01b",
   "metadata": {},
   "outputs": [],
   "source": [
    "# 三种方案的注意力矩阵对比\\n",
  ...
  },
  {
   "cell_type": "code",
   "execution_count": null,
   "id": "7fbfd68deb0d47a3849620398192e896",
   "metadata": {},
   "outputs": [],
   "source": [
  ...
  },
  {
   "cell_type": "code",
   "execution_count": null,
   "id": "0680735e0c80470288326949914b499f",
   "metadata": {},
   "outputs": [],
   "source": [
  ...
  },
  {
   "cell_type": "markdown",
   "id": "2ca4c3f9",
   "source": "### 为什么重算可以是免费的\\n\\n这一小节解释上一步延迟模型里的关键数字：为什么说重算可以是免费的，背后的流水线是怎么排的。\\n\\n上面的延迟模型展示 CacheBlend 的一个系统工程细节：把重算与加载排成流水线。重算第 i 层的选定 token 时，后台同时从慢速存储加载第 i+1 层的 KV。两条操作并行，只要重算时间不超过加载时间，重算就藏在加载的耗时里，单层总延迟是两者中的较大值而不是两者之和。\\n\\n单层 KV 缓存体积是确定的：Llama-7B 量级、2048 token、K 与 V 各 4096 维、每维 2 字节，即 $2048 \\\\times 2 \\\\times 4096 \\\\times 2 \\\\approx 33.6$ MB。加载耗时取决于存储速度：HBM 约 0.008 ms，NVMe SSD 约 16.8 ms，SATA SSD 约 61 ms，HDD 约 224 ms。重算 15% token 的单层耗时约 3 ms。\\n\\n比较谁被谁隐藏。NVMe 加载 16.8 ms 大于重算 3 ms，总延迟取加载值，重算免费；SATA 与 HDD 更不用说。HBM 加载只有 0.008 ms，远小于重算，此时重算成为瓶颈，但缓存放在 HBM 本身没有加载问题。曲线里 NVMe 在 r < 0.84 时保持平直，正是这个道理：只要重算不超过 16.8 ms（即 r ≤ 0.84），延迟都由加载决定。\\n\\n这个设计的价值是把 KV 缓存放到更便宜、更慢的设备上而不增加首 token 延迟。重算在 GPU 上算，加载走存储总线，两者互相独立，重叠后总耗时等于较慢的一方。质量底线把 r 限定在 15% 以上，在 0.15 到 0.84 的区间里，NVMe 上的重算对延迟完全不可见。",
   "metadata": {}
  },
  {
   "cell_type": "markdown",
   "id": "d4b9ae4411a04f21988554415b682799",
   "metadata": {},
   "source": [
    "## 小结\\n",
    "\\n",
  ...
  {
   "cell_type": "markdown",
   "id": "be60d498532847868d6c8a1d7d7e1b24",
   "metadata": {},
   "source": [
    "## 作业\\n",
    "\\n",
  ...
  {
   "cell_type": "markdown",
   "id": "2911b57397f44370a8f4f45f313323df",
   "metadata": {},
   "source": [
  ...
  },
  {
   "cell_type": "code",
   "execution_count": null,
   "id": "f761c61669794b5a885322aa33bfafcf",
   "metadata": {},
   "outputs": [],
   "source": [
    "# 填空 1：判断是否到达冲水线\\n",
  ...
  {
   "cell_type": "markdown",
   "id": "ec747eca418b4988819675897b92b534",
   "metadata": {},
   "source": [
    "**作业 2：HKVD 选择与层间相关**\\n",
    "\\n",
  ...
  },
  {
   "cell_type": "code",
   "execution_count": null,
   "id": "4bd50e5de40e40859e41c0da2db1a2aa",
   "metadata": {},
   "outputs": [],
   "source": [
    "import numpy as np\\n",
  ...
  {
   "cell_type": "markdown",
   "id": "d458275abefb4a0ab03185e37bea8e9a",
   "metadata": {},
   "source": [
  ...
  },
  {
   "cell_type": "code",
   "execution_count": null,
   "id": "388bf061fd6845a992c028492e49d1df",
   "metadata": {},
   "outputs": [],
   "source": [
    "import torch\\n",
  ...
  {
   "cell_type": "markdown",
   "id": "d98eb1205beb4008b6e767e2782b0a95",
   "metadata": {},
   "source": [
    "## 参考资料\\n",
    "\\n",
