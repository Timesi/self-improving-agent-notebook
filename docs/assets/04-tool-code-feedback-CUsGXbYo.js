const n=`{
 "cells": [
  ...
  },
  {
   "cell_type": "markdown",
   "id": "86671509",
   "source": "先用一个具体例子看 CoT 的边界。假设问模型《现代文学》创刊于哪一年，而训练数据里没有这本刊物的确切信息。CoT 先写一段推理再给出答案，推理不接触任何外部资料，模型只能从内部记忆里找。找不到时，模型通常不回答\\"不知道\\"，而是顺着\\"文学刊物创刊一般较早\\"这类直觉编一个年份。推理链越长，编造的机会越多。\\n\\n同一个问题交给 ReAct，模型不用只靠记忆。模型先写下当前的想法，再发出检索动作，工具返回的观察成为下一步推理的事实来源。思考为行动提供依据，行动的观察又修正下一步的思考。下面把一条 ReAct 轨迹按角色打印出来，我们逐行读一遍。",
   "metadata": {}
  },
  {
   "cell_type": "code",
