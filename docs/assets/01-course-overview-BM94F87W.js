const n=`{
 "cells": [
  {
   "cell_type": "markdown",
   "id": "a7813cc6907e4f00befd5c20e44ea77c",
   "metadata": {},
   "source": [
    "# 课程总览：什么是 AI Agent\\n"
   ]
  ...
  {
   "cell_type": "markdown",
   "id": "c58ae7d58ee64c2382ad0679575e677a",
   "metadata": {},
   "source": "> 大语言模型能写代码、做翻译、回答知识性问题。它生成的文本很流畅。但单次调用只输出一段文本。它不接触外部世界。它也没法把一个需要多步操作的任务从头做到尾。\\n>\\n> 这一讲是整门课的起点。我们先看一个具体的任务。这个任务单次调用做不了，Agent 循环做得到。我们再从这个任务出发，给 Agent 下一个贯穿全课的说法。最后，亲手从零搭出第一个最小循环。"
  },
  {
   "cell_type": "markdown",
   "id": "988e28873a194af9be9166285270e345",
   "metadata": {},
   "source": "我们先从一个具体的任务看起。\\n\\n任务是：把仓库里所有过期的 TODO 标记出来，并运行测试。这是一个\\"做事\\"的任务。\\n\\n把这个任务交给 ChatGPT 这类大语言模型，只靠单次问答是不够的。模型的输入和输出都只是文字。它只会给出建议文本，不会真的去改文件、跑命令。原因是，模型本身不会调用任何外部程序。\\n\\n因此，要让模型完成这件事，需要把它放进一个循环里。我们把这个循环一步步画出来。\\n\\n1. 把任务交给模型。模型根据当前的情况，决定下一步做什么。\\n2. 执行这一步。比如编辑一个文件，或运行一条命令。\\n3. 把执行结果喂回给模型。\\n4. 回到第 1 步。如此重复，直到任务完成。\\n\\n承担这个循环的结构，就是这一讲的主角，叫 Agent。\\n\\n循环里最常用的三个词，我们先分别说清楚。\\n\\n环境是 Agent 身外的世界。文件系统、网页、数据库、代码执行器都算。\\n\\n状态是任务进行到当前的全部信息。包括已经做过的动作和已经拿到的中间结果。\\n\\n动作是模型输出的、能在环境里真实执行的一步操作。比如编辑一个文件，或运行一条命令。\\n\\n回到我们的任务上，三个词分别是：环境是仓库和测试命令；状态是已扫描的文件和已标记的过期条目；动作是编辑文件、运行测试。\\n\\n到这一讲结束，我们要亲手从零写出一条这样的循环。届时就能看清它和普通 LLM 应用的区别。"
  },
  {
   "cell_type": "markdown",
   "id": "d25979c5663344b3881940e296502f93",
   "metadata": {},
   "source": "## 1. 从 LLM 应用到 Agent\\n\\n上一节我们给 Agent 画了一条循环。这一节先看另一条路，也就是不循环的路：单次调用。要理解循环为什么必要，先看清单次调用缺了什么。\\n\\n把一句话发给模型，模型返回一段文本。这一来一回，在模型内部叫一次前向计算（forward pass）。这样的单次调用，有四条硬边界。我们先把这四条边界列出来。\\n\\n1. 只能\\"说\\"不能\\"做\\"。模型输出 token，不接触世界。它不知道网页、数据库、文件或代码执行器里发生了什么。\\n2. 上下文窗口有限。长任务和大量中间结果，无法在一次调用内全部容纳。\\n3. 错了不会自己纠正。单次生成没有执行反馈，模型无法知道答案是否正确。\\n4. 知识静态。训练截止日期之后的新事实，模型无法获取。\\n\\n这四条边界具体长什么样，我们下一段逐个展开。现在先做一件具体的事。我们建立统一的模型客户端，发起一次单次调用，看看模型面对\\"做事\\"任务会回答什么。"
  },
  {
   "cell_type": "code",
   "execution_count": null,
   "id": "3f7d9a7499794dd08db9d4b60ecc4623",
   "metadata": {},
   "outputs": [],
   "source": [
    ...
    "\\n",
    "# force_脚本化示例=True 保证离线可复现；配置真实 key 后去掉该参数即可\\n",
    "client = get_llm()\\n",
    "{\\"client\\": type(client).__name__, \\"脚本化示例\\": False}\\n"
   ]
  },
  {
   "cell_type": "code",
   "execution_count": null,
   "id": "2b659cebf63a4e29ad61159b9f839e34",
   "metadata": {},
   "outputs": [],
   "source": [
  ...
  {
   "cell_type": "markdown",
   "id": "5e2057519f6646a8b062ce43a7b4c7d3",
   "metadata": {},
   "source": "上面列出的四条硬边界，我们逐条展开。每条都用具体的数字和例子来看。\\n\\n**只能\\"说\\"不能\\"做\\"**。模型的全部输出是 token，也就是文本。它不能真的打开文件、发送请求或执行代码。这些操作只能由我们写的程序替它完成。让模型\\"把过期的 TODO 标记出来\\"，它只会输出一段描述怎么改的文字。改动本身要由外部的脚本执行。\\n\\n**上下文窗口有限**。模型的输入和输出，都要先被切成小块。每一块叫一个 token。中文大约一个字对应一个 token。英文一个常见单词约对应一个 token。模型一次调用最多能容纳多少 token，这个上限叫上下文窗口（context window）。输入的 prompt 和输出的内容都要算进去，加起来不能超过它。GPT-2 的窗口是 1024 个 token，约一千字。现代主流模型在 128K 到 1M 之间。\\n\\n把任务换算成 token，就能看出窗口有多容易超。一行代码平均约 20 个 token。一个 3000 行的仓库约 6 万 token。这还没算模型输出，就已经接近 128K 窗口的一半。任务越长、中间结果越多，越容易超出窗口。这就是长任务必须拆成多步、分批喂给模型的原因。\\n\\n**错了不会自己纠正**。单次调用里，模型只生成一次答案。中间没有任何执行反馈。它写了一段有 bug 的代码，却不知道代码能否跑通。因为没有人运行它。纠正需要反馈。我们要运行代码，把报错信息喂回去，让模型在下一轮重写。\\n\\n**知识静态**。模型在训练时见过一份固定的数据。数据截止日期之后的事实，它不知道。某个依赖库在 2025 年改了接口，而训练数据截止于 2024 年。模型给出的调用方式就过时了。要拿到新事实，只能靠外部检索再喂回模型。这正好是 Agent 循环里工具要承担的工作。\\n\\n四条边界可以分成两类。\\n\\n- 前两条是结构限制：输出不能执行、窗口容纳不了。\\n- 后两条是信息限制：没有反馈、知识旧。\\n\\nAgent 循环的每一环都在补其中一条。动作交给工具执行，反馈由循环带回来，新知识靠外部检索获取。"
  },
  {
   "cell_type": "markdown",
   "id": "4c91e365f86f4ffc83b9e6c4965355fa",
   "metadata": {},
   "source": "四条边界说的是\\"单次调用不行\\"。它和 Agent 到底差在哪，我们放进一张表里逐项对照。\\n\\n| 维度 | 纯 LLM 应用 | LLM-based Agent |\\n|:---|:---|:---|\\n| 调用方式 | 单次（或固定轮次）问答 | 循环，直到终止条件 |\\n| 状态 | 无（或仅靠会话历史） | 显式维护任务状态与记忆 |\\n| 动作 | 只输出 token | 输出可执行的工具调用 |\\n| 环境反馈 | 无 | 有（执行结果回到上下文） |\\n| 目标 | 生成一个答案 | 完成一个任务（可失败、可重试） |\\n\\n表里的状态和动作，开头已经用 TODO 任务举过例子。这里只补一个没讲过的词：工具调用。工具调用是动作的一种具体形式。模型输出一段结构化文本，比如 search(\\"CS329A agents\\")。程序解析这段文本，调用对应的函数。动作能不能执行，取决于循环里有没有先准备好对应的工具。\\n\\n有一个容易混淆的地方。在代码里自动拼一个 prompt，再调一次 API。这只是把 LLM 当函数调用。没有循环、状态与动作，那就仍是应用，而不是 Agent。判断的关键是，循环、状态、动作这三样缺一不可。"
  },
  {
   "cell_type": "markdown",
   "id": "d29759ffdedc45c8b674b70a9b49919c",
   "metadata": {},
   "source": "## 2. Agent 的定义与组成\\n\\n上一节我们用一条循环描述 Agent。这一节把\\"Agent 是什么\\"再正式地讨论一下。我们看三件事：这个说法从哪来，循环由哪几样东西组成，每样东西在代码里长什么样。\\n\\nAgent 的经典定义，先于大语言模型存在。Wooldridge 在 1995 年的综述里给出：agent 是居于某个环境中的实体。它通过传感器感知环境，通过执行器作用于环境。这个定义里有两点，在后续讨论里反复出现。\\n\\n1. autonomy，中文叫自主性。在没有外部直接干预的情况下，agent 自行决定行动，以达成目标。放在软件里，自主性强调动作由 Agent 自己选。给定\\"整理仓库 README\\"的目标，它自己决定先读哪些文件、按什么顺序改。而不是每一步都由人指定。\\n2. situatedness，中文叫具身性。它面对的是一个部分可观、不断变化的环境，而不是一个格式良好的输入。部分可观指 Agent 任何时候都只能看到环境的一部分。扫过文件列表的程序，不知道某个文件内部有哪些过期的 TODO。\\"格式良好的输入\\"说的是普通程序。输入结构固定、字段齐全，程序从头算到尾，中途不需要再看外部世界。Agent 面对的是信息不全、随时变化的环境。所以需要反复感知和行动。\\n\\n定义里的传感器和执行器，在软件里都有具体对应。\\n\\n- 传感器是 Agent 获取环境信息的入口。对人来说是眼睛和耳朵。对软件 Agent 来说是读文件、检索网页、查询数据库。\\n- 执行器是 Agent 改变环境的手段。对人来说是手和脚。对软件 Agent 来说是修改文件、发送请求、运行命令。\\n\\nLLM-based Agent 是现代版本。它以 LLM 为决策核心，在循环中调用 LLM。完成感知、推理、决策，把决策变成环境中的动作。通常借助工具，再把动作结果作为反馈再次输入。直到目标达成。LLM 在这里扮演大脑，而不是全部。循环的最小组成有五项，先用一段代码把它们列出来。"
  },
  {
   "cell_type": "code",
   "execution_count": null,
   "id": "666aca69253943bab0de7107ac3c04da",
   "metadata": {},
   "outputs": [],
   "source": [
  ...
  {
   "cell_type": "markdown",
   "id": "1cff1f3027154462ba7ea4d482178863",
   "metadata": {},
   "source": "循环的主体，我们已经列出了五项。不过在动手实现之前，还有几个部分要交代。它们常被讲成独立模块，其实是循环的配件。这三个部分叫记忆、规划、验证。我们各看一遍。\\n\\n**记忆**：负责把上下文窗口之外的信息接回来。短期记忆就是消息历史。也就是本次对话里所有消息的列表。长期记忆是外部存储，比如向量库或文件。把上一会话的结论写进一个文件，下次会话再读回来。\\n\\n**规划**：把目标分解成子目标，或对动作序列做搜索。分解的例子是任务\\"写一份课程笔记\\"。先拆成\\"查大纲、检索资料、写正文、校对\\"几步，再逐步执行。搜索则是一次尝试多条动作路线。保留能达成目标的那条。\\n\\n**验证**：在循环之外加一个检查环节。让输出经过验证才被接受。模型生成的代码，先跑一遍测试。通过了，才作为最终答案。\\n\\n它们不是循环的主体，但都挂接在循环的某个环节上。后面讲到对应讲次时再展开。\\n\\n它们被称为配件，是因为缺了它们，循环照样能转。一个只有 LLM、工具和消息历史的循环，可以完整跑完一个小任务。前面要实现的这条最小循环，就属于这种情况。\\n\\n循环的主体，只有四步加一个终止条件。我们从零实现它。"
  },
  {
   "cell_type": "markdown",
   "id": "a2e0d29104be447f97fef912881423b4",
   "metadata": {},
   "source": "## 3. 一条 Agent 循环\\n\\n把上一节的组成写成能跑的代码，是这一讲的第一个动手目标。循环体一共做四件事。\\n\\n1. 调用 LLM，得到回复。\\n2. 解析回复里的动作。\\n3. 执行动作，并把观察结果回填进消息历史。\\n4. 重复以上三步，直到模型给出 \`Final Answer\` 或步数用尽。\\n\\n循环靠消息历史记住进行到哪了。消息历史是循环里维护的一张消息列表。每条消息记着一个角色和一段内容，形如 {\\"role\\": \\"user\\", \\"content\\": \\"...\\"}。开始时的第一条是任务本身。之后每轮，Agent 的输出以 assistant 身份追加。工具的执行结果以 user 身份追加。因为对模型来说，工具结果就像对话里新来的一句话。模型每轮只看这张列表。自己上一轮说了什么、工具返回了什么，全部要靠列表里的记录。列表越长，模型能参考的上下文就越多。所以消息历史就是循环的状态。\\n\\n任务选一个多步算术：计算 (3+5)×(7-2)。先在纸上分步算一遍，给代码一个对照基准。"
  },
  {
   "cell_type": "code",
   "execution_count": null,
   "id": "51c82894e551456fa71e73b1a4508963",
   "metadata": {},
   "outputs": [],
   "source": [
  ...
  {
   "cell_type": "markdown",
   "id": "ff8434c15e08485fa672eb988ccfafb6",
   "metadata": {},
   "source": "上面的代码我们已经把结果算出来了。接下来把同一过程写成消息历史。我们看每一步列表里会多出什么。任务固定为\\"请分步计算 (3+5)×(7-2)\\"。\\n\\n第一步，消息列表只有任务这一条：\\n\\n\`\`\`text\\n[用户] 请分步计算 (3+5)×(7-2)\\n\`\`\`\\n\\n第二步，模型输出 Action: calc(\\"3+5\\")。这条以 assistant 身份追加。工具执行后，结果以 user 身份回填。第三步与第四步重复同样的过程。模型先输出 Action: calc(\\"7-2\\") 拿到 5。再输出 Action: calc(\\"8×5\\") 拿到 40。到第四步结束，整个列表是：\\n\\n\`\`\`text\\n[用户] 请分步计算 (3+5)×(7-2)\\n[Agent] Action: calc(\\"3+5\\")\\n[用户] Observation: 8\\n[Agent] Action: calc(\\"7-2\\")\\n[用户] Observation: 5\\n[Agent] Action: calc(\\"8×5\\")\\n[用户] Observation: 40\\n\`\`\`\\n\\n每一步模型读到的列表，都比上一步多两条消息。最后一步，模型看到列表里的 8、5、40。它输出 Final Answer: 40。这个 40 是模型从列表里读出来的，不是它自己心算的。工具结果经过消息列表回到模型。这就是状态在循环里的流动。"
  },
  {
   "cell_type": "markdown",
   "id": "3b367b6d116445e1818ebb3819101221",
   "metadata": {},
   "source": "循环拿到模型回复后，第一件事不是执行，而是先读懂这段文本。我们要判断：模型想调用工具，还是直接宣布完成。做这件事的组件叫解析器。它把文本转成循环能直接读取的数据。\\n\\n模型的回复只可能有三种内容：\`Thought\` 加 \`Action\`、\`Final Answer\`、或普通文本。解析器负责识别这三种情况，返回统一的 (kind, action, payload) 结构。\\n\\n三种内容各举一个例子。\\n\\n**1. 推理加动作**。模型一边想，一边指定要调用的工具：\\n\\n\`\`\`text\\nThought: 需要先检索课程资料。\\nAction: search(\\"CS329A agents\\")\\n\`\`\`\\n\\nAction 后面是工具名和参数。解析器要把这行提取成结构化的 (工具名, 参数)。循环才知道该调用哪个函数。\\n\\n**2. 最终答案**，表示任务完成：\\n\\n\`\`\`text\\nFinal Answer: CS329A 是斯坦福的 AI Agent 课程\\n\`\`\`\\n\\n**3. 普通文本**。既没有 Action，也没有 Final Answer。比如模型在复述任务。解析器对这类文本返回\\"无动作\\"。循环就不调用工具。\\n\\n解析的顺序有讲究。先找 Final Answer，再找 Action。模型可能在同一次回复里，既给出 Action，又给出 Final Answer。此时循环应该先把动作执行完，再按最终答案终止。解析器返回的 (kind, action, payload) 三个字段，分别回答三个问题。是否终止，执行哪个工具，最终答案是什么。"
  },
  {
   "cell_type": "code",
   "execution_count": null,
   "id": "17dbfbd3eb854799b303f8e4468d903b",
   "metadata": {},
   "outputs": [],
   "source": [
    "import re\\n",
  ...
  },
  {
   "cell_type": "code",
   "execution_count": null,
   "id": "904cf3306cae48b688899cae0bff90b1",
   "metadata": {},
   "outputs": [],
   "source": [
    "class AgentLoop:\\n",
  ...
  },
  {
   "cell_type": "code",
   "execution_count": null,
   "id": "d5dff4de3b2c4ef79aae79fb3ebfc33e",
   "metadata": {},
   "outputs": [],
   "source": [
    "def calc(expr):\\n",
  ...
  {
   "cell_type": "markdown",
   "id": "f0bc426b3922449788f71f4f8d55a2ca",
   "metadata": {},
   "source": "有一个常见误解需要澄清。Agent 循环不等于多轮对话。\\n\\nChatGPT 的多轮，只是记住轮次。模型每次仍在生成文本。没有动作，也没有反馈。\\n\\nAgent 循环的每一轮，一定经过环境。哪怕只是读一个文件。判断标准是中间有没有可执行的、改变世界的动作。上一节里，\`calc\` 就是这样的动作。它真的执行了算术并返回结果。\\n\\n把两种轨迹并排看。\\n\\n- 多轮对话：用户一句、模型一句。列表里没有 Observation。\\n- Agent 循环：模型输出 Action，程序执行工具，把 Observation 喂回去。列表里至少出现一条来自工具的记录。\\n\\n读一个文件也算动作。因为它确实从环境读回了内容。这段内容进入了模型下一轮的上下文。反过来，模型输出一段不含 Action 的文字，就只是多轮对话里的一轮。\\n\\n我们把同一个循环的大脑换成 llm_client 的 脚本化示例 客户端。看循环如何处理模型输出的脚本化轨迹。"
  },
  {
   "cell_type": "code",
   "execution_count": null,
   "id": "2af1b0b39160443e8474420dcef82885",
   "metadata": {},
   "outputs": [],
   "source": [
    "# 同一段循环代码，大脑换成 llm_client 的 脚本化示例，行为随之变化\\n",
    ...
    "    print(f\\"[{tag}] {msg['content']}\\")\\n",
    "    print()\\n",
    "print(\\"关键观察：脚本化示例 把 Action 与 Final Answer 放在同一次回复里，循环先执行动作再终止\\")\\n"
   ]
  },
  {
   "cell_type": "code",
   "execution_count": null,
   "id": "0e55c912dbdc48e6b05a23caf1595d0a",
   "metadata": {},
   "outputs": [],
   "source": [
  ...
  {
   "cell_type": "markdown",
   "id": "5a82479bf2194503ba942b7dde4ed824",
   "metadata": {},
   "source": "## 4. Agent 生态地图\\n\\n我们刚刚从零实现了一条属于自己的循环。现在退一步看。围绕 Agent 循环，业界已经形成了好几种成熟的做法，叫范式。了解它们，能让我们知道自己的循环在业界地图上位于哪。也能看到别人是怎么搭循环的。这一节把主流范式列出来。\\n\\n需要先说明一点。这些范式不是互斥的分类，而是\\"决策怎么来\\"的几种方案。真实系统通常是多种范式的组合。我们把六种范式、各自的代表工作，以及它们在本课中的位置列出来。"
  },
  {
   "cell_type": "code",
   "execution_count": null,
   "id": "161cd5f8071e4162933a9bc2127a167c",
   "metadata": {},
   "outputs": [],
   "source": [
  ...
  {
   "cell_type": "markdown",
   "id": "b029675c87034103a937f109f29274f7",
   "metadata": {},
   "source": "六种范式里，ReAct 与 Planning 最贴近循环本身。先看懂这两个。其余四种用一句话说明。\\n\\nReAct 是推理与行动交替。模型每轮先输出一段 Thought，说明为什么这样做。再输出一个 Action，指定要调用的工具。环境返回 Observation 后，进入下一轮。前一条 Agent 循环就是这个形状。Thought、Action、Observation 交替出现。只是我们的解析器没有把 Thought 单独取出来。\\n\\nPlanning 是动手前先规划。把目标分解成子目标，或对动作序列做搜索。分解的例子是\\"写一份课程笔记\\"。先列成\\"查大纲、检索资料、写正文、校对\\"四步，再逐步执行。搜索是同时尝试多条动作路线。保留能达成目标的那条，LATS 属于这类工作。一句话概括两者的区别。ReAct 边看边做，Planning 先计划后行动。\\n\\n其余四种各自举一个例子。\\n\\n- **Tool use**：让模型输出结构化工具调用。Toolformer 是早期代表。今天的 MCP 是这类调用的标准化接口。\\n- **Multi-agent**：让多个 Agent 对话协作。一位老师和一个学生轮流出题答题，就是典型用法。\\n- **Memory-based**：用显式长期记忆跨会话工作。MemGPT 用分层记忆管理长对话。超出上下文的内容先转存到外部存储，需要时再取回。\\n- **Frameworks**：把循环、工具、记忆封装成库。LangGraph 和 Claude SDK 属于这一类。全程可以参照。\\n\\n真实系统很少只用一种范式。以 ReAct 做主干，用 Planning 分解长任务，用 Memory 记住历史，用 Tool use 接外部工具。这是常见的组合方式。"
  },
  {
   "cell_type": "code",
   "execution_count": null,
   "id": "58f29eb3b11441dcb1214c29fbee995a",
   "metadata": {},
   "outputs": [],
   "source": [
  ...
  {
   "cell_type": "markdown",
   "id": "a1f2c0748ca7474e916857c13d152d1a",
   "metadata": {},
   "source": "## 5. 课程路线\\n\\n到这一讲结束，我们已经看到了循环的骨架。感知、决策、行动、反馈。\\n\\n真实的 Agent 还缺很多能力。这门课就按\\"缺什么补什么\\"的顺序，把 17 个 notebook 排下来。这一节把整张课程地图展开。方便你随时回来查某一讲的位置。\\n\\n课程沿四部分展开，共 17 个 notebook。\\n\\n- Part 1 补能力。test-time compute（把额外算力花在推理阶段）、验证、工具、规划。从\\"LLM 只能生成\\"出发，逐层加。\\n- Part 2 补怎么变强。训练期缩放、开放进化、搜索、后训练演进。\\n- Part 3 补工程化。SWE、记忆、评测。\\n- Part 4 补边界。推理、数学、自治、机器人。\\n\\n我们把每部分的讲次列出来。"
  },
  {
   "cell_type": "code",
   "execution_count": null,
   "id": "ef20cd8f18904ac4b469df44ce112a0f",
   "metadata": {},
   "outputs": [],
   "source": [
  ...
  },
  {
   "cell_type": "code",
   "execution_count": null,
   "id": "6f4a8ef19b9948b6b03e72a6e2a96d5d",
   "metadata": {},
   "outputs": [],
   "source": [
    "import numpy as np\\n",
  ...
  {
   "cell_type": "markdown",
   "id": "ba79faf023224ec7acc1bdfad14a0eb8",
   "metadata": {},
   "source": [
    "## 作业\\n",
    "\\n",
  ...
  {
   "cell_type": "markdown",
   "id": "582dc09122354de1bad876a1c21c8998",
   "metadata": {},
   "source": [
    "**作业 1：补全最小循环的终止与回填**\\n",
    "\\n",
  ...
  },
  {
   "cell_type": "code",
   "execution_count": null,
   "id": "970c2dfc0cc84859855c26ca2168fba4",
   "metadata": {},
   "outputs": [],
   "source": [
  ...
  {
   "cell_type": "markdown",
   "id": "333d935007dc46b885cfaadd0017413d",
   "metadata": {},
   "source": [
    "**作业 2：动作解析器**\\n",
    "\\n",
  ...
  },
  {
   "cell_type": "code",
   "execution_count": null,
   "id": "274066d95e574286af96c011c39745ee",
   "metadata": {},
   "outputs": [],
   "source": [
    "import re\\n",
  ...
  {
   "cell_type": "markdown",
   "id": "ab459427cb3a4a53b8cb21f03a937069",
   "metadata": {},
   "source": [
    "**作业 3：单次调用与循环的量化差异**\\n",
    "\\n",
  ...
  },
  {
   "cell_type": "code",
   "execution_count": null,
   "id": "9aa12d45153a454c81ed68955d782b79",
   "metadata": {},
   "outputs": [],
   "source": [
    "def count_tool_calls(messages):\\n",
  ...
  {
   "cell_type": "markdown",
   "id": "63ada992e8e44bd194bf2e11a274a329",
   "metadata": {},
   "source": [
    "## 参考资料\\n",
    "\\n",
