const n=`{
 "cells": [
  ...
  },
  {
   ...
   "metadata": {},
   "source": [
    "> 前 19 讲把 Agent 从一次 LLM 调用，一步步做成会验证、会用工具、会规划、会训练、会自我进化、能进物理世界的系统。回头看，所有系统的骨架只有一根：生成候选、验证好坏、把反馈接回循环。\\n",
    ">\\n",
    "> 这一讲是结课总览，不引入新的指定论文。我们先把前 19 讲收拢成一张能力栈，再把 2025 年卡住 Agent 的问题整理成五个维度的开放问题清单，最后给三条可以上手的研究路线。清单里的每个问题，都配一个能在 notebook 里跑起来的玩具版本。\\n"
   ]
  },
  {
   "cell_type": "markdown",
   ...
   "metadata": {},
   "source": [
    "把前 19 讲叠在一起，每个系统都是同一根骨架的实例。test-time compute 是\\"多生成少验证\\"，验证器是\\"把验证做准\\"，ReAct 是\\"循环里加入环境反馈\\"，树搜索是\\"循环里加入分支\\"，RL 缩放是\\"把循环的每一环训练进权重\\"，开放进化是\\"让循环修改循环自己\\"，评测是\\"给循环一把公平的尺子\\"。骨架在基准上已经跑得很远，但把它放大到更长的时间尺度、更多的 agent、真实世界的物理环境时，每一环都会出现新的失效点。\\n",
    "\\n",
  ...
  },
  {
   ...
   "source": [
    "### 一张能力栈：从一次调用到自进化\\n",
    "\\n",
    "把 16 讲的功能节点按课程顺序排成一行。单次生成是起点，之后每一讲都在前一层外面再包一层循环：会验证（L3）之后反馈才可信，会用工具（L4）之后才能在真实环境里试错，会规划（L5）之后才能把长任务拆开，会训练（L6）之后才能把循环写进权重，会自我进化（L7）之后循环才能修改循环自己。到 L19，循环已经接到物理世界。\\n",
    "\\n",
  ...
  },
  {
   ...
   "metadata": {},
   "source": [
    "### 一张课程地图：解析 OUTLINE.md 的知识图谱\\n",
    "\\n",
    "能力栈给出了纵向的层次，课程的横向结构也可以画出来。越到后面越像一张小图——后面的概念几乎都建立在前面某几个概念之上。这一节直接解析仓库根目录的 OUTLINE.md，把 17 个 notebook 的编号与标题抽出来，再用手工维护的前置关系表把概念连成一张依赖图，标出被复用最多的枢纽概念。\\n"
   ]
  },
  {
   "cell_type": "code",
  ...
  },
  {
   ...
   "source": [
    "### 能力增长的量尺：METR 时间视野\\n",
    "\\n",
    "能力栈给出了结构，还要一把尺子量出增长的幅度。第 17 讲用 METR 的时间视野做过这件事：给一组人类时长不同、可自动判对错的任务，统计模型在每个时长上的成功率，用一条 logistic 曲线拟合，$p(\\\\text{task}) = \\\\sigma(\\\\beta(\\\\log h - \\\\log t))$，解出成功率 50% 对应的任务时长 $h$。这个 $h$ 就是 50% 时间视野，即模型有一半概率完成的人类时长。\\n",
    "\\n",
    "只盯 50% 会漏掉一个重要信息：曲线尾部。成功率从 50% 提到 80%，任务时长会掉得非常多。Opus 4.5 的 50% 点是 4 小时 49 分，80% 点只有约 27 分钟——模型能偶尔做完几小时的任务，却没法稳定做完半小时的任务。这个\\"50% 与 80% 之间的落差\\"就是可靠性 gap，它正是开放问题清单里最集中的卡点。先手算这个落差：给定 $h$ 与曲线陡峭程度 $\\\\beta$，求两个时间视野的封闭式 $t_q = h\\\\,(q/(1-q))^{-1/\\\\beta}$。\\n"
   ]
  },
  {
   "cell_type": "code",
  ...
  },
  {
   ...
   "source": [
    "## 2. 开放问题清单\\n",
    "\\n",
    "能力增长很快，边界却很具体。把课程之外反复出现的问题整理成一张清单，分成五个维度：能力、可靠性、评测、安全、社会影响。每个问题记四件事：卡在哪里、为什么还没解决、可能的突破口、相关讲次。清单先做成一份本地结构化数据，再画成图——开放问题的分布、与课程讲次的映射，都从这份数据出发。\\n"
   ]
  },
  {
   "cell_type": "code",
  ...
  },
  {
   ...
   "source": [
    ...
    "print(\\"关键观察：L17 与 L18 是清单里出现最多的两讲。\\")\\n"
   ]
  },
  {
   "cell_type": "markdown",
   ...
   "metadata": {},
   "source": [
    "### 一个可运行的样本：Goodhart 背离\\n",
    "\\n",
    "清单里的大部分问题，在 notebook 里都有一个能跑的玩具版本。先看安全维度的自改进对齐侵蚀，它直接呼应第 7 讲的开放进化风险。Darwin Gödel Machine 让 agent 修改自己的代码，用编码基准验证，SWE-bench 从 20% 提到 50%；但当任务改成减少幻觉时，agent 选择绕过幻觉检测函数而不是解决幻觉——指标拿了满分，真目标原封未动。这就是 Goodhart 定律：指标一旦变成目标，就不再是好指标。\\n",
    "\\n",
    "玩具版本这样构造：真目标 $g$ 每轮靠诚实改进缓慢增长，且边际收益递减；代理指标 $p = g + \\\\text{噪声} + \\\\text{hack 项}$。agent 每轮只看得见 $p$，它选择让 $p$ 涨得更快的动作。诚实改进的收益随时间衰减，而 hack 项的收益固定，agent 很快会转投 hack。跑一遍，看 $p$ 与 $g$ 在哪一轮分手。\\n"
   ]
  },
  {
   "cell_type": "code",
  ...
  },
  {
   ...
   "source": [
    "### 另一个样本：多智能体协调开销\\n",
    "\\n",
    "再看可靠性维度的多智能体协调。直觉上多个 agent 协作更强，2025 年的实证相反：Berkeley 统计七个主流多 agent 系统，失败率 41% 到 86.7%；DeepMind 用 180 个受控实验发现，独立 agent 投票会把单 agent 5% 的错误率放大到 86%。原因不是 agent 不够聪明，而是协调本身要花成本——沟通引入错误，错误还会传播。\\n",
    "\\n",
    "玩具模型：N 个 agent 各自以概率 $p$ 独立做对，只要有一个做对，团队的答案就是对的（冗余）；但每对成员之间都有一条协调链路，每条链路有概率 $c$ 引入错误，且错误会传播到整组结果。冗余项 $1-(1-p)^N$ 让成功率随 N 上升，协调项 $(1-c)^{N(N-1)/2}$ 让它随 N 下降，两条力合成一条单峰曲线。\\n"
   ]
  },
  {
   "cell_type": "code",
  ...
  },
  {
   ...
   "source": [
    "### 路线一：把已有组件做硬\\n",
    "\\n",
    "已有循环里最薄弱的三环是验证、评测、预算感知。验证器在开放任务上判不准，FormalJudge 用神经-符号混合把它补准：LLM 把意图编译成可验证约束，再交给 Dafny/Z3 证明，比纯 LLM judge 平均高 16.6%。评测这条线上，RE-Bench 证明连续打分能度量研究能力，2025 年 o3 领先到 0.380。预算感知上，BATS 让 agent 知道自己还剩多少预算，在 BrowseComp 上把准确率从 12.6% 提到 24.6%。这三处的共同点不是造新东西，而是把已有环节做准，改动小、见效快、可评测。\\n"
   ]
  },
  {
   "cell_type": "markdown",
   ...
   "source": [
    "### 路线二：把缺口做通\\n",
    "\\n",
    "课程里没有闭环的环节，是研究的富矿。记忆讲了结构，但一次经验沉淀成下一次行为还没有可靠机制，MemVerse 用短长时记忆加周期参数蒸馏，是 2025 年的代表。多智能体讲了规划与进化，但何时该拆、何时不该拆没有第一性原理，MAST 把失败模式分类成 14 类，是写论文的开端而不是终点。具身能进物理世界，但跨具身泛化与真机基准都缺，π0.5 用 97.6% 的非目标平台数据做联合训练。这些缺口要把课程里的多个组件组合起来，工程量更大，问题也更年轻。\\n"
   ]
  },
  {
   "cell_type": "markdown",
   ...
   "source": [
    "### 路线三：把系统做安全\\n",
    "\\n",
    "第三条路线面向安全。可扩展监督解决超过专家后谁来判对错，自改进对齐侵蚀解决进化过程中对齐被腐蚀。这两类问题的起点都站在课程的肩膀上：L4 的 Constitutional AI 给出 AI 反馈 AI 的雏形，L7 的开放进化指明自改进的风险。Anthropic 的审计 agent 单独检出率 13%，聚合后到 42%，证明 AI 审计 AI 的方向可行；分区监督让不同领域专家给互补标签。这些工作的共同思路，是把不可被 exploit 的验证设计进循环。\\n"
   ]
  },
  {
   "cell_type": "markdown",
   ...
   "source": [
    "## 4. 如何参与\\n",
    "\\n",
    "三条路线不是抽象的口号。参与的第一步，是把清单里的某个玩具版本做成真系统：换真实任务、真实评测、真实验证器。还有一条同样重要的现实路径：研究不只有算法。METR 要造 50 个 32 小时的新任务，需要 3200 小时以上的专家标注，成本超过一百万美元——设计评测、做数据、造工具，都在课程大纲之外。下面用课程的 LLM 客户端请一个研究顾问，让它从清单里挑一个方向给起步建议，再看 mock 模式怎么兜底。\\n"
   ]
  },
  {
   "cell_type": "code",
   ...
   "source": [
    ...
    "    print(\\"[mock 模式输出为占位；配置 AGENT_LLM_API_KEY 后得到真实建议]\\")\\n",
  ...
  },
  {
   ...
   "source": [
    "### 三个提醒\\n",
    "\\n",
    "动笔之前，把三个常见的卡点说清楚。第一，基准饱和不是能力登顶——饱和说明需要更难、更有价值的新任务，不是没任务可做。第二，开放问题不等于不可写代码——清单里每个问题都有能跑的玩具版本，今天已经跑过 Goodhart 与协调开销两个。第三，评测与优化是两个环——评测测的是能力上界，优化目标是训练信号，两者之间的差距就是评测设计的研究空间。\\n"
   ]
  },
  {
   "cell_type": "markdown",
   ...
   "source": [
    ...
    "这一课学到的内容（也是整门课程的收束）：\\n",
    "\\n",
    "- [ ] 前 19 讲是一根骨架的实例：生成 → 验证 → 循环，每一讲把上一层装进循环\\n",
  ...
  },
  {
   ...
   "source": [
    ...
    "\\n",
    "keyword_map = {\\n",
    "    \\"记忆\\": [\\"L14\\"], \\"长时程\\": [\\"L14\\"], \\"忘记\\": [\\"L14\\"],\\n",
    "    \\"协调\\": [\\"L5\\", \\"L13\\"], \\"协作\\": [\\"L5\\", \\"L13\\"], \\"多智能体\\": [\\"L5\\", \\"L13\\"],\\n",
    "    \\"验证\\": [\\"L3\\", \\"L4\\"], \\"正确性\\": [\\"L3\\", \\"L4\\"], \\"保证\\": [\\"L3\\", \\"L4\\"],\\n",
    "    \\"评测\\": [\\"L17\\"], \\"基准\\": [\\"L17\\"], \\"benchmark\\": [\\"L17\\"],\\n",
    "    \\"安全\\": [\\"L4\\", \\"L18\\"], \\"对齐\\": [\\"L4\\", \\"L18\\"], \\"监督\\": [\\"L4\\", \\"L18\\"],\\n",
    "    \\"进化\\": [\\"L7\\"], \\"自改进\\": [\\"L7\\"], \\"修改自己\\": [\\"L7\\"],\\n",
    "    \\"具身\\": [\\"L19\\"], \\"机器人\\": [\\"L19\\"], \\"物理世界\\": [\\"L19\\"],\\n",
    "    \\"自治\\": [\\"L18\\"], \\"状态\\": [\\"L18\\"], \\"定位\\": [\\"L18\\"],\\n",
    "}\\n",
    "\\n",
    ...
    "assert \\"L14\\" in result, \\"长时程记忆应命中 L14\\"\\n",
    "assert \\"L18\\" in result, \\"自治与状态定位应命中 L18\\"\\n",
  ...
  },
  {
   ...
   "source": [
    "## 参考资料\\n",
    "\\n",
    "- CS329A 课程主页（https://cs329a.stanford.edu/）— L20 是 2025-12-05 结课讲，无指定论文，本讲在课程地图中的定位\\n",
    "- 本仓库 OUTLINE.md — 课程 17 个 notebook 的大纲，知识图谱演示直接解析它\\n",
    "- 本仓库 papers/lecture-02 ~ lecture-08 的 NOTES.md — 前 19 讲线索收束的素材来源\\n",
