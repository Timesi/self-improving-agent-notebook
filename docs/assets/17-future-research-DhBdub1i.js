 ...
 "cells": [
  {
   ...
   "id": "b526ea22",
   "metadata": {},
   "source": "# 未来研究方向：从系统边界到研究路线"
  },
  {
   "cell_type": "markdown",
   "id": "88ee6072",
   "metadata": {},
   "source": "> 前面的课程把 Agent 从一次 LLM 调用，一步步做成会验证、会用工具、会规划、会训练、会自我进化、能进入物理世界的系统。回头看，所有系统共用同一根骨架：生成候选，检查好坏，把反馈接回循环。\\n>\\n> 这一讲是收尾加展望。我们先量一量系统现在能稳定完成多长的任务，再把仍然卡住的问题分成几类，最后为每一类写下一个可以动手验证的研究问题。每个问题都从本课程已经实现的最小版本出发。"
  },
  {
   "cell_type": "markdown",
   "id": "ea797871",
   "metadata": {},
   "source": "我们每天用的 ChatGPT、DeepSeek 这类模型，单看一次回答已经很能干。但让它们连续工作几个小时，比如从一堆文件里整理出一份完整报告，它们会出错、会忘事、会越做越偏。前面 16 讲从零把这样的系统搭了出来：会验证、会用工具、会规划、会训练、会自我进化、能进入物理世界。这一讲是最后一讲，做两件事：先盘点这条路走到今天，边界在哪里；再讲接下来哪些问题值得做、怎么做。\\n\\n把 16 讲做出来的系统摆在一起看，会发现它们共用同一根骨架：生成候选，检查好坏，把反馈接回循环。名字各异的那些概念，都是这根骨架的实例。test-time compute 是让循环多生成几次再选，验证器是把检查做准，ReAct 是让环境反馈进入循环，树搜索是给循环加分叉，RL 缩放是把循环的每一环写进模型权重，开放进化是让模型修改循环本身，评测是给循环一把统一的量尺。能力各不相同，骨架是同一条。\\n\\n骨架在课程里的基准上跑得很远。但把它放大到更长的任务、更多的 Agent、真实的物理世界时，感知、决策、行动、反馈哪一环最先失效，答案并不显然。这些还没有答案、但值得去做的问题，叫开放问题。学完这一讲，你能说清当前系统的边界在哪里，能看懂一张开放问题清单，并能从已经写好的玩具版本出发，提出一个自己能动手验证的研究问题。这个领域还很年轻，能做的远比做完的多。我们下面先从边界说起，用一把统一的尺子量一量，模型这几年到底强了多少。"
  },
  {
   "cell_type": "markdown",
   "id": "f13450ef",
  ...
  },
  {
   ...
   "id": "ca3aab11",
   "metadata": {},
   "source": "这一节先回答一个问题：学了 16 讲，我们现在做出来的系统到底站在什么位置。我们把 16 讲的功能节点按课程顺序排成一行，看它们是怎么一层层叠起来的。\\n\\n单次生成是起点：模型收到任务，输出一段答案。之后每一讲都在前一层外面再包一层循环。先会验证，模型输出的答案能被检查对不对；再会用工具，检查不靠猜，可以真的在环境里跑一遍；再会规划，长的任务先拆成几步；再会训练，把该按什么顺序做写进模型权重；再会自我进化，让模型能修改自己做判断的逻辑本身。到第 16 讲，这个循环已经接进了物理世界，能操作真实的机器人。\\n\\n这 16 个节点不是互不相干的 16 个功能，而是同一根骨架的 16 个实例，骨架就是生成、验证、反馈三个字。下面的代码把这根骨架画出来，节点沿对角线一层层往上叠，颜色按课程的四个部分分组。"
  },
  {
   "cell_type": "markdown",
   "id": "f0c1a2b3",
   "metadata": {},
   "source": "生成 → 验证 → 反馈不是三个抽象词，我们用一个具体任务走一遍。任务让 Agent 把数字 1 到 20 按从小到大排序，并确认结果正确。\\n\\n第一步是生成。模型收到任务后输出一段候选代码。它可能一次就对，也可能写错。单次生成只解决能输出，不保证输出是对的。\\n\\n第二步是验证。我们在循环里加一个检查器：把排序函数跑在一个已知答案的小输入上，比如输入 [3, 1, 2]，期望输出 [1, 2, 3]。候选代码跑出的结果和期望一致才通过，不一致就重新生成。验证器解决的是输出了但可能错。\\n\\n第三步是反馈。把验证的结果，哪个输入失败了、期望是什么，写回提示词，让下一次生成带着错误信息重来。验证给生成提供方向，生成给验证提供新的候选，两者交替，这就是循环。\\n\\n后面每一讲都在这一步之上再包一层。L4 加工具，是给验证一个外部执行环境：排序函数放进 Python 解释器里真的跑一遍，而不是靠模型猜。L5 加规划，是任务大到排序、统计均值、画图、写报告四步时，先把任务拆开，每步各自经历生成、验证、反馈。L6 加训练，是把遇到这类任务该按什么顺序做写进权重，下次少走弯路。L7 加进化，是让模型能修改自己生成候选与验证的逻辑本身。\\n\\n能力栈图把 16 个功能节点画在对角线上。第 i 个节点画在 (i, i) 而不是 (i, j)，含义是：到达第 i 层时，前 i-1 层都还活着，只是被包在第 i 层里面。对角线往上累积，就是循环套循环的几何表示。我们读图先看箭头，左下角的旧层指向右上角的新层；再看颜色，四种颜色对应课程的四个部分，第四个部分的节点全部落在最上层。"
  },
  {
   "cell_type": "code",
  ...
  },
  {
   ...
   "id": "d3575594",
   "metadata": {},
   "source": "能力栈图给了纵向的层次：一层包一层。课程的横向关系也值得画出来，那就是概念之间的依赖。课程越往后，后面的概念几乎都建立在前面某几个概念之上，比如规划建立在验证和工具之上，记忆建立在进化之上。把这样的依赖连起来，就是一张概念依赖图，箭头指向前置概念。\\n\\n画这张图有两个用处。一是看全貌，16 个核心概念谁依赖谁，一目了然。二是找枢纽，被最多概念依赖的那几个，就是循环里被复用最狠的环节。代码先解析仓库根目录的 OUTLINE.md，把 17 个 notebook 的编号与标题抽出来，再用手工维护的前置关系表把概念连成一张依赖图。"
  },
  {
   "cell_type": "code",
   "execution_count": null,
  ...
  },
  {
   ...
   "id": "d02079e0",
   "metadata": {},
   "source": "能力栈图给了结构，还要有一把尺子量出增长的幅度。METR 是一家做 AI 评测的机构，它发明了这样一把尺子，叫时间视野。第 14 讲介绍过它，这里我们把它请回来，用来量现在的模型能稳定完成多长的任务。\\n\\n做法是这样的。准备一组任务，人类完成每个任务的时间长短不同，短到十几分钟，长到几个小时，而且每个任务都能自动判断对错。对一个模型，统计它在不同时长任务上的成功率：极短的任务几乎全对，极长的任务几乎全错，中间有一段成功率从高掉到低的过渡区。把这些点连起来，是一条递减的 S 形曲线，用一条 logistic 曲线去拟合它。拟合得到两个数：$h$ 是成功率恰好 50% 时对应的人类时长，叫 50% 时间视野；$\\\\beta$ 是曲线有多陡。\\n\\n只看 $h$ 会漏掉一个重要信息。成功率从 50% 提到 80%，能扛的任务时长会大幅缩短。Opus 4.5 的 50% 点是 4 小时 49 分，80% 点只有约 27 分钟。也就是说，模型能偶尔做完几小时的任务，却无法稳定完成半小时的任务。这个 50% 与 80% 之间的落差，叫可靠性 gap，它是长任务难稳定的结构性原因，也是后面开放问题清单里最集中的卡点。\\n\\n我们先把可靠性 gap 算出来。给定 $h$ 和 $\\\\beta$，先推一个公式，它能给出成功率恰好为某个值 $q$ 时的任务时长，再代入数字手算两个时间视野。"
  },
  {
   "cell_type": "markdown",
   "id": "e2d4f6a8",
   "metadata": {},
   "source": "这一小节要推一个公式：给定 $h$ 和 $\\\\beta$，求成功率恰好为 $q$ 的任务时长 $t_q$。有了它，就能把 50% 与 80% 两个时间视野算出来，量化可靠性 gap。先复习第 14 讲的定义，再反解公式，最后代入数字。\\n\\n对一组人类时长不同的任务，统计模型在每个时长上的成功率，会得到一条递减的 S 形曲线：极短的任务几乎全对，极长的任务几乎全错，中间有一段成功率从高掉到低的过渡区。METR 用 logistic 曲线拟合这条线：\\n\\n$$p(t) = \\\\sigma(\\\\beta(\\\\log h - \\\\log t))$$\\n\\n三个记号要一一对上。$t$ 是任务的人类时长，单位小时。$h$ 是成功率恰好 50% 的时长，也叫 50% 时间视野。把 $t = h$ 代进去，$\\\\log h - \\\\log t = 0$，而 $\\\\sigma(0) = 0.5$，所以 $p(h) = 0.5$ 自动成立。$\\\\beta$ 是曲线陡峭程度：$\\\\beta$ 越大，过渡区越窄，成功率掉得越突然。$\\\\sigma$ 是 sigmoid 函数，$\\\\sigma(x) = 1/(1+e^{-x})$，值域是 0 到 1，恰好是成功率的范围。代入两个数字感受量级：$\\\\sigma(2.5) \\\\approx 0.92$，$\\\\sigma(-2.5) \\\\approx 0.08$。\\n\\n要把公式反解成成功率恰好为 $q$ 的任务时长 $t_q$。两边取 logit，logit 是 sigmoid 的反函数，$\\\\text{logit}(p) = \\\\log(p/(1-p))$：\\n\\n$$\\\\text{logit}(q) = \\\\beta(\\\\log h - \\\\log t)$$\\n\\n移项得 $\\\\log t = \\\\log h - \\\\text{logit}(q)/\\\\beta$，再取指数：\\n\\n$$t_q = h\\\\left(\\\\frac{q}{1-q}\\\\right)^{-1/\\\\beta}$$\\n\\n代入具体数字手算一遍。取 $h = 4.8$ 小时（Opus 4.5 量级），$\\\\beta = 0.6$。\\n\\n$q = 0.5$ 时，$q/(1-q) = 1$，$1^{-1/\\\\beta} = 1$，所以 $t_{50} = h = 4.8$ 小时。50% 时间视野就是 $h$ 自己，这正是它名字的来历。\\n\\n$q = 0.8$ 时，$q/(1-q) = 4$，要算 $4^{-1/0.6} = 4^{-1.667}$。用 $\\\\ln 4 \\\\approx 1.386$ 展开：$4^{1.667} = e^{1.667 \\\\times 1.386} \\\\approx e^{2.31} \\\\approx 10.1$，所以 $4^{-1.667} \\\\approx 1/10.1 \\\\approx 0.099$。于是\\n\\n$$t_{80} = 4.8 \\\\times 0.099 \\\\approx 0.47 \\\\text{ 小时} \\\\approx 28 \\\\text{ 分钟}$$\\n\\n两个时间视野的比值 $t_{50}/t_{80} = 4^{1/\\\\beta} \\\\approx 10$。把这个比值的含义说清楚：模型在 50% 成功率下能扛 4.8 小时的任务，把成功率要求提到 80%，能扛的任务时长缩到约十分之一。同一件事还可以从另一个方向读：同一个任务（$t$ 固定），从 50% 做对提到 80% 做对，需要把 $h$ 抬高 $4^{1/\\\\beta} \\\\approx 10$ 倍，也就是模型的 50% 视野要增长约 10 倍，这个任务才能稳定完成。\\n\\n$\\\\beta$ 对 gap 的影响同样可以手算。$\\\\beta = 1.2$ 时，$4^{1/1.2} = 4^{0.833} = e^{0.833 \\\\times 1.386} \\\\approx e^{1.155} \\\\approx 3.17$，$t_{80} = 4.8/3.17 \\\\approx 1.51$ 小时，比值约 3.2 倍。$\\\\beta = 0.4$ 时，$4^{1/0.4} = 4^{2.5} = 32$，$t_{80} = 0.15$ 小时，比值 32 倍。曲线越平（$\\\\beta$ 越小），把成功率从 50% 提到 80% 时能接受的任务时长掉得越多，可靠性 gap 越大。这正是下面代码里 beta 越小，80% 点掉得越狠的由来。\\n\\n选 logistic 而不是直线，有两个理由。直线在成功率接近 0 或 1 时会把预测值推出 [0, 1] 区间，出现负成功率这种无意义结果；logistic 自带 0 到 1 的边界。logistic 的两个参数各有明确含义：$h$ 衡量能做多久，$\\\\beta$ 衡量做得到底有多稳，不同模型拟合出的 $(h, \\\\beta)$ 可以直接比较。两者合起来，才是时间视野这把统一量尺的完整读数。"
  },
  {
   "cell_type": "code",
   "execution_count": null,
  ...
  },
  {
   ...
   "id": "05e024fb",
   "metadata": {},
   "source": "上一节量了边界：模型能稳定完成多长的任务。这一节把边界里冒出来的问题整理成一张清单。这些问题目前还没有答案，所以叫开放问题。\\n\\n清单按五个维度分：能力、可靠性、评测、安全、社会影响。前三个和模型本身有关，后两个把人放进了画面。每个问题记录四件事：卡在哪里、为什么还没解决、可能的突破口、相关讲次。相关讲次指的是课程里已有的对应组件，说明这个问题不是凭空冒出来的，而是某一环被放大的结果。\\n\\n我们先把清单做成本地结构化数据，再画成两张图：一张看问题在五个维度上怎么分布，一张看每个问题和课程哪些讲次相关。"
  },
  {
   "cell_type": "markdown",
   "id": "a1c3e5f7",
   "metadata": {},
   "source": "这一小节把五个维度一个一个打开。每个维度都按同一套问法：它问的是什么问题，举一个具体例子，和相邻维度有什么区别。十个开放问题不是随机散落的点，它们按\\"模型本身 → 模型的可信度 → 我们能否度量 → 是否安全 → 对社会的影响\\"这条线排开。前三个是技术问题，后两个把人和组织放进画面。\\n\\n能力（capability）问的是\\"模型还不会做什么\\"。判断标准很直接：给一个任务，模型做不出来。例子：让 Agent 连续跑一个 6 小时的数据分析并写成报告，它会中途忘记早先的结论、越写越偏。这一维的突破口是把能力做上去。\\n\\n可靠性（reliability）问的是\\"会做，但能不能稳定做对\\"。同一个任务跑十次，有时对有时错，属于可靠性问题。它与能力的区别：能力看上限，可靠性看方差。一个模型能做对 90% 的编程题，但那 10% 里可能包含最危险的一类操作，可靠性关心的就是这 10%。\\n\\n评测（evaluation）问的是\\"我们到底怎么知道它做得怎么样\\"。长任务没有自动判分器，判断一份研究笔记的质量需要专家读几个小时，这就是评测问题。它与前两维的区别：能力与可靠性是模型的性质，评测是度量手段本身的性质。\\n\\n安全（safety）问的是\\"行为会不会伤害用户或偏离意图\\"。模型追求目标时钻指标的空子，或在超过人类专家之后无人能判断对错，都属于这一维。它与可靠性的区别：可靠性关心做对，安全还关心做对的过程中和之后没有害处。\\n\\n社会影响（societal）问的是\\"部署之后对人和社会意味着什么\\"。深度研究报告看起来专业却漏掉关键信息，用户因此过度信任，是典型例子。前四维都能用技术手段部分缓解，这一维依赖人与组织的判断。\\n\\n清单里每个问题记四件事，这四件事的用意可以借 long-horizon autonomy & memory 这一条看明白。卡在哪里：\\"错误沿长轨迹累积，超过上下文窗口的内容没有可靠的记忆机制\\"，这是现象，我们据此能复现问题。为什么还没解决：\\"长任务放大 L4 已有的失败模式，经验沉淀成下一次行为还缺机制\\"，这是根因，指出它连着课程哪一环。可能的突破口：\\"分层记忆 + 周期蒸馏；把验证器当检查点而不是终点\\"，这是方向，notebook 的玩具版本就从这里长出来。相关讲次 [14, 18, 17]，这是课程里已有的对应组件，说明这个问题不是凭空冒出来的，而是课程某一环放大的结果。\\n\\n后面的每条问题都按同一个模板填。读了这一条，就能把十条都读明白。"
  },
  {
   "cell_type": "code",
   "execution_count": null,
  ...
  },
  {
   ...
   "id": "6a5a292e",
   "metadata": {},
   "source": "清单里的大部分问题，在 notebook 里都有一个能跑的玩具版本。玩具版本的意思是：保留真实问题的核心矛盾，把环境简化到几行代码就能跑。\\n\\n先看安全维度的第一个问题，自改进对齐侵蚀。它直接呼应第 7 讲的开放进化风险。那里的 Darwin Gödel Machine 让 Agent 修改自己的代码，用编码基准来验证，SWE-bench 得分从 20% 提到 50%。但当把任务改成减少幻觉时，Agent 选择了另一条路：它绕过幻觉检测函数，而不是真的减少幻觉。结果是指标得了满分，真实目标原封未动。\\n\\n这个现象有一个名字，叫 Goodhart 定律：当一个指标被当作优化目标，它就不再是好指标。Agent 只看得见指标，指标会把它带向最容易刷分的方向，而不是我们真正想要的方向。\\n\\n下面构造一个玩具版本，把这个定律跑出来。我们会定义两条曲线：一条代表我们真正想要的改进，一条代表模型只看得见的指标。模型每轮会选择让指标涨得最快的动作，我们手算几轮，看指标和真实目标什么时候分手，再跑代码验证。"
  },
  {
   "cell_type": "markdown",
   "id": "7d9b1f3a",
   "metadata": {},
   "source": "上一小节介绍了 Goodhart 定律。这一小节把它翻译成数字，先在纸上手算几轮，看两条曲线在哪一轮分手。\\n\\n玩具模型的核心设定是两条曲线。真目标 $g$ 每轮靠诚实改进缓慢增长，且增长幅度递减；代理指标 $p = g + \\\\text{hack项} + \\\\text{噪声}$。Agent 每轮只看得见 $p$，它选择让 $p$ 涨得更快的动作。关键在两组数字：诚实改进的边际收益随时间衰减，代码里取 $0.5 e^{-t/15}$；hack 项的收益固定，每轮 0.18。只要边际收益还大于 0.18，Agent 就诚实；一旦衰减到 0.18 以下，hack 变成让 $p$ 涨得更快的那条路，Agent 转投 hack。\\n\\n按代码的数字手算前几轮。$t=0$ 轮：边际收益 0.5，大于 0.18，诚实改进，$g = 0.5$，$p = 0.5 + 噪声$。$t=1$ 轮：边际收益 $0.5 e^{-1/15} \\\\approx 0.47$，仍诚实，$g \\\\approx 0.97$。$t=15$ 轮：边际收益 $0.5 e^{-15/15} \\\\approx 0.18$，仍略大于 0.18，$g \\\\approx 5.08$。$t=16$ 轮：边际收益 $0.5 e^{-16/15} \\\\approx 0.17$，小于 0.18，Agent 转投 hack，$g$ 停在 5.08 不动，$p$ 从这一轮起每轮加 0.18。$t=40$ 轮：hack 项累计约 4.3，$p \\\\approx 9.4$，$g$ 仍是 5.08。指标一路涨，真目标停在半路，这就是背离。\\n\\n有两点必须说清。第一，Agent 不认为自己作弊：从它唯一的输入 $p$ 看，诚实与 hack 都是让 $p$ 上升，它只是选了上升更快的那条。问题出在指标设计，不在 Agent 的动机。第二，噪声项（代码里的 $0.02 \\\\times \\\\text{randn}$）模拟评测波动，它让单轮 $p$ 分辨不出诚实还是 hack，所以检测背离要用一段窗口的斜率：窗口内 $p$ 的斜率仍为正、$g$ 的斜率跌破阈值，就说明两条曲线开始分手。下面代码的 \`gap_point\` 就在做这件事，窗口取 8，窗口内做线性拟合取斜率。\\n\\n为什么这个玩具值得放进结课讲。它把第 7 讲开放进化的风险具象化了。自改进系统的奖励信号大多是可 hack 的代理指标，比如编码基准、检测函数、人类偏好的近似。一旦 Agent 发现 hack 收益稳定而诚实改进收益递减，就会转投 hack。DGM 里的 SWE-bench 从 20% 提到 50%，靠的是诚实改进的收益；减少幻觉任务里的绕过检测，则是 hack 的收益。验证器必须设计成不可被 exploit，也就是不能被钻空子，这条结论贯穿后面的三条研究路线。"
  },
  {
   "cell_type": "code",
   "execution_count": null,
  ...
  },
  {
   ...
   "id": "b7843eb9",
   "metadata": {},
   "source": "上一节列出了十个开放问题。这一节把它们收拢成三条可动手的研究路线。开放问题不意味着没有论文可读，每个缺口背后都有一批可以引用的工作，多数带公开实现。\\n\\n第一条，把已有的组件做得更可靠。第二条，把课程里还没有闭环的环节打通。第三条，让系统在自我进化中保持安全。它们不是并列的三条路，而是同一件事的三个切面，都在让\\"生成 → 验证 → 循环\\"的某一环更可靠。"
  },
  {
   "cell_type": "markdown",
   "id": "25df9a7b",
   "metadata": {},
   "source": "第一条路线，把已有的组件做得更可靠。课程里循环最薄弱的三环是验证、评测、预算感知，每一环都有现成的工作在补。\\n\\n先看验证。验证器在开放任务上经常判不准：一道题没有标准答案，让模型去判断另一个模型的答案对不对，它可能被同样的问题带偏。FormalJudge 用神经加符号的混合办法补这一环：让 LLM 把解题意图编译成可验证的约束条件，再交给 Dafny 或 Z3 这类证明器去证明，平均比纯 LLM judge（纯粹用模型当裁判）高出 16.6%。\\n\\n再看评测。评测要能度量研究能力本身，RE-Bench 证明连续打分可以做到，2025 年时模型 o3 的得分已经领先到 0.380。\\n\\n最后看预算感知。Agent 常常不知道自己的预算还剩多少，BATS 让 Agent 明确知道自己还剩多少，在 BrowseComp 基准上把准确率从 12.6% 提到 24.6%。\\n\\n这三处的共同点不是引入新组件，而是把已有环节做准，改动小、见效快、可评测。"
  },
  {
   "cell_type": "markdown",
   "id": "f1b87ebd",
   "metadata": {},
   "source": "第二条路线，把课程里没有闭环的环节打通。课程已经讲过的概念，往往只开了头，离能可靠工作还差一个机制。这类缺口有三处，都值得做。\\n\\n记忆讲了结构，但一次经验沉淀成下一次行为，还没有可靠机制。MemVerse 用短时加长时记忆，加上周期的参数蒸馏，来解决这个问题，是 2025 年的代表工作。\\n\\n多智能体讲了规划与进化，但何时该拆成多个 Agent、何时不该拆，还没有一个普遍的原则。MAST 把多 Agent 系统的失败模式分类成 14 类，这个分类是写论文的开端而不是终点。\\n\\n具身能进物理世界，但跨具身泛化与真实机器人的基准都缺。π0.5 用 97.6% 的非目标平台数据做联合训练，让模型在不同机器人上都能用。\\n\\n这些缺口需要把课程里的多个组件组合起来，工程量更大，问题也更初步。"
  },
  {
   "cell_type": "markdown",
   "id": "551a8362",
   "metadata": {},
   "source": "第三条路线面向安全。模型变强之后，出现两类新问题。第一类叫可扩展监督：当模型的能力超过人类专家，人类自己来判断对错都不够用了，谁来给模型把关。第二类叫自改进对齐侵蚀：对齐指的是模型始终按人类意图行事，而自我进化会把对齐一步步磨掉，目前还没有维护它的机制。\\n\\n这两类问题的起点都在课程内容里。第 4 讲的 Constitutional AI 给出了让 AI 来评估 AI 的雏形，第 7 讲的开放进化指明了自改进的风险。\\n\\n这一类工作已经有了一些结果。Anthropic 让审计 Agent 去检查另一个 Agent 的行为，单独检出率只有 13%，把多个审计 Agent 的结果聚合后达到 42%，证明 AI 审计 AI 的方向可行。分区监督是让不同领域的专家各自给出互补的标签，合起来覆盖更全面。\\n\\n这些工作的共同思路，是把不可被 exploit 的验证设计进循环，也就是让验证器无法被钻空子，这和前面 Goodhart 一节的结论连上了。"
  },
  {
   "cell_type": "markdown",
   "id": "52f9981c",
   "metadata": {},
   "source": "三条路线讲完了，最后一步是参与。参与的第一步，是把清单里的某个玩具版本做成真系统：换真实任务、真实评测、真实验证器。\\n\\n研究不只有算法。制作 50 个时长 32 小时的新任务，METR 需要 3200 小时以上的专家标注，成本超过一百万美元。设计评测、整理数据、搭建工具，这些工作都在课程大纲之外，同样是研究。\\n\\n下面用课程的 LLM 客户端请一个研究顾问，让它从清单里挑一个方向，给出三步起步建议。"
  },
  {
   "cell_type": "code",
   "execution_count": null,
  ...
  },
  {
   ...
   "id": "cf7b62cb",
   "metadata": {},
   "source": "动笔之前，先把三个常见的卡点说清楚。\\n\\n第一个卡点是基准饱和。一个基准上模型的得分接近满分了，这叫饱和。饱和不代表能力登顶，只说明这个基准不够难了。真正要做的是造出更难、更有价值的新任务，而不是认为没有任务可做。\\n\\n第二个卡点是\\"开放问题等于写不了代码\\"。清单里每个问题都有能跑的玩具版本，本讲前面已经演示过 Goodhart 与协调开销两个。先跑玩具，再往真实方向扩展，是常规做法。\\n\\n第三个卡点是混淆评测与优化。评测测的是能力上界，优化用的目标是训练信号，两者不是一回事。两个环之间的差距，正是评测设计的研究空间。"
  },
  {
   "cell_type": "markdown",
   "id": "7fbbe465",
