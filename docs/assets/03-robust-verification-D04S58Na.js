const n=`{
 "cells": [
  ...
  },
  {
   ...
   "id": "8561a2b3",
   "metadata": {},
   "source": "> 上一讲我们让模型对同一道题反复采样，再用 self-consistency 把多条答案放在一起做多数投票。投票只统计答案出现的次数，不检查每条答案的推理过程。当错误的答案扎堆出现时，多数投票会选出错误的答案。\\n>\\n> 这一讲在采样之后加一个检查环节：用一个独立的验证器给每条候选答案打分，再挑分数最高的那个。我们会看到验证器怎么从数据里学出打分，只检查最终答案和检查每一步推理这两种做法各有什么优劣，以及没有人工标注时验证器还能不能训练。"
  },
  {
   "cell_type": "markdown",
   "id": "24bcdb99",
   "metadata": {},
   "source": "同一个模型回答同一道难题，多问几次，答案常常不一样，有的对有的错。上一讲我们利用这种随机性，反复采样，再把多条答案放在一起投票。但投票只数答案出现的次数，不检查推理过程，错误的答案常常扎堆，投票就会选错。要从一堆候选里挑出正确的那一个，需要一条独立的通路来评判每条答案的好坏，这就是第 1 讲生态地图里挂在 L3 的验证环节。学完这一讲，我们能在自己的循环外面加一个验证器，实现\\"采样多条、打分、挑最优\\"的选答流程。\\n\\n这种\\"生成一堆、再挑一个\\"的需求在真实系统里很常见。很多产品在后台让模型生成好几个候选答案，再挑一个最稳的返回给用户；训练模型时，也需要给模型的输出打一个\\"好还是坏\\"的分数。评判一条已经生成的答案，比从零生成它容易：生成必须从头推演整条推理，评判只需要对照写出来的步骤，找出错在哪一步。这条独立的评判通路，我们叫它验证器（verifier），它给每条候选答案打一个 0 到 1 之间的正确概率，分数最高的候选就作为最终答案。\\n\\n这一讲要做的，是把验证器亲手实现出来，并看清它带来多大提升。我们会在一个能自动判对错的玩具任务上实现验证器，用来做 best-of-N 选解，再比较\\"只看最终答案对错\\"和\\"检查每一步推理\\"两种做法。第一件事，是把\\"正确答案已经被生成出来、却因为选错而丢掉\\"这个损失量化出来。"
  },
  {
   "cell_type": "markdown",
   "id": "10a47425",
   "metadata": {},
   "source": "## 1. 生成与验证的差距\\n\\n上一讲我们反复采样，把多条答案放在一起投票。这一节把注意力从\\"生成\\"转到\\"选择\\"：采样出很多候选之后，选择环节到底损失了多少。看清这个损失，才能判断验证环节值不值得加。\\n\\n先看模型为什么需要验证。自回归模型一次只输出一个 token，没有回头改错的能力。某个中间步骤一旦算错，后面的步骤全部建立在错误之上，整条解被污染。错误答案扎堆时，多数投票也会失效。\\n\\nWeaver 论文把这个问题量化成两个量。\`Pass@K\`：候选池里存在至少一条正确解的比例，衡量\\"正确答案有没有被生成出来\\"。\`SuccessRate\`：选择器实际挑对的概率，衡量\\"选得对不对\\"。两者的差就是生成-验证差距（generation-verification gap）。\\n\\n要把这两个量测出来，需要一个能自动判对错的实验环境。我们把\\"答案唯一、可程序判对错\\"这一性质编码成一个玩具任务：每道题由两个子表达式和一个组合算子构成，最终答案是两个子结果组合得到的值。解文本的格式固定，便于按行解析和重算。"
  },
  {
   "cell_type": "markdown",
   "id": "a040232b",
   "metadata": {},
   "source": "## 2. 验证器如何从答案中学会打分\\n\\n这一节回答一个问题：验证器打分的本事从哪来。答案是从数据里学出来的。我们沿 Cobbe 等人提出的流程走一遍，再用玩具任务看看自动标签里混进了什么噪声。\\n\\n流程来自 Cobbe 等人的论文。他们在发布 GSM8K 数学应用题数据集的同时，给出了验证器的训练流程，分三段。先在训练题上微调生成器，让它能解出训练题；再对每道题从生成器采样 100 条候选解，按\\"最终答案是否等于标准答案\\"自动打上正负标签；最后在这个自动标签集上训练一个独立的验证器。整个过程不需要人看任何一条解。\\n\\n验证器与生成器是两个独立的网络。验证器在每条解的末尾输出一个正确概率，也就是给整条解一个分数；也可以在每个 token 之后都预测一次整条解的正确概率，测试时取最后一个 token 的分数。后者比前者更细，我们先把验证器想成\\"在解的末尾给一个分\\"，后面会看到更细的版本。\\n\\n自动打标有一个明显的噪声源：中间步算错、最终答案却碰巧正确的解（歪打正着），会被标成正例；中间步全对、最终答案写错的解，会被标成负例。标签和推理质量对不上，就是噪声。我们用玩具任务把这份噪声统计出来。"
  },
  {
   "cell_type": "markdown",
   "id": "853c5e7c",
   "metadata": {},
   "source": "## 3. 结果验证器与过程验证器\\n\\n这一节回答一个问题：监督信号应该给整条解，还是给每一步。我们比较两种做法，并说明\\"只看最终答案\\"会漏掉什么。\\n\\n上一节的结果验证器只看最终答案对不对：每条解一个标签。这个做法有一个隐患。一条解被标成负例，只说明\\"某处错了\\"，没有说明错在哪一步。第一步算错和第二步算错，标签完全一样。难题的候选解大多含错，这种负标签的信息量很低。我们把这个问题叫信用分配问题：整条解一个分数，没法把过错分给具体的每一步。\\n\\n一个自然的想法，是让监督信号细到每一步。对推理链的每一步给出一个正确概率，这样\\"哪一步拉低了整条解\\"就被直接写进标签。这种验证器叫过程验证器（PRM，process reward model）；上一节那种只给整条解一个标签的，叫结果验证器（ORM，outcome reward model）。\\n\\n下表是四种中间步与最终答案的组合，以及两种监督分别给出的标签：\\n\\n| 中间步 | 最终答案 | 结果监督标签 | 过程监督标签 |\\n|---|---|---|---|\\n| 全对 | 对 | 正例 | 每步都正 |\\n| 有错 | 对（歪打正着） | 正例 | 含一个负步 |\\n| 全对 | 错（收尾写错） | 负例 | 每步都正 |\\n| 有错 | 错 | 负例 | 含负步 |\\n\\n注意中间两行。结果监督把\\"中间错、答案对\\"标成正例，把\\"中间对、答案错\\"标成负例，标签和推理质量脱节；过程监督直接给出每一步的真值，错的位置一目了然。"
  },
  {
   "cell_type": "markdown",
   "id": "d0425862",
   "metadata": {},
   "source": "## 4. 把检查粒度推进到每一步\\n\\n这一节回答一个问题：把检查粒度从结果推进到每一步，在真实数据和我们的玩具任务上，各能带来多少提升。\\n\\nLightman 等人（2023）把验证从判结果推进到判过程。过程验证器对推理链的每一步输出该步正确的概率，整条解的分数用乘积归约，也就是所有步都正确的概率。标签里如果出现 neutral（无法判断对错），按正例处理。\\n\\n在 MATH 数据集的 best-of-1860 上，PRM 达到 78.2%，结果验证器 ORM 为 72.4%，多数投票为 69.6%，差距随候选数 N 增大而拉大。训练 PRM 需要的 PRM800K 是 80 万条人工步级标注，只标到第一个错误步为止，兼顾成本与信息量。\\n\\n我们在玩具任务上复现这条主线：训练两个验证器，一个用结果标签（每解一条），一个用过程标签（每步一条），比较它们学出来的分数在 best-of-N 里的表现。这里我们把第 2 节用的一致性特征还原为原始数值，让两个验证器面对同样的信息量。"
  },
  {
   "cell_type": "markdown",
   "id": "27ab7e8f",
   "metadata": {},
   "source": "## 5. 没有人逐步标注时怎么办\\n\\n这一节回答一个问题：每一步的标签都很贵，能不能不用人标。我们看 Math-Shepherd 怎么做自动标注，以及自动标注会带来什么新问题。\\n\\n过程监督需要人工标注每一步，PRM800K 有 80 万条标签，成本很高。Math-Shepherd（Wang 等人，2023）用自动标注替代人工。思路是：某一步的质量，由\\"从这一步继续还能不能推出正确答案\\"决定，而不是由这一步本身算得对不对决定。做法是让一个补全器从当前中间步续写 N 条后续路径，再看多少条最终到达正确答案，据此给这一步打标。\\n\\n自动标注有两种估计方式，各有取舍。具体怎么算、各自的毛病在哪，下一段用具体数字展开，再用脚本化的补全器复现。"
  },
  {
   "cell_type": "markdown",
   "id": "d909f1e8",
   "metadata": {},
   "source": "## 6. 验证器与采样结合\\n\\n这一节回答两个问题：验证器怎么和上一讲的采样配起来用，以及一个验证器不够可靠时怎么办。\\n\\n上一讲的重复采样保证\\"候选里至少有一条正确\\"，验证器负责\\"从候选中挑出正确的那条\\"，两者合起来叫 best-of-N：采样 N 条候选解，验证器给每条打分，选分数最高的那条。实验里有两个值得看的细节：选完最高分之后还能不能再补一步，以及和多数投票相比能好多少。\\n\\n单个弱验证器不够可靠时，可以把多个验证器加权集成。Weaver（Saad-Falcon 等人，2025）的思路是不需要人工标签，直接从验证器之间的同意程度反推出各自的精度，再按朴素贝叶斯聚合。细节留在后面的实验里，这里先记住结论：多个弱验证器集成，能逼近一个更强的验证器。"
  },
  {
   "cell_type": "markdown",
   "id": "252c199f",
   "metadata": {},
   "source": "验证信号可以按来源排成一条序列：程序真值检查（执行器/最终答案）、结果监督（整条标签）、过程监督（每步标签）、多验证器集成（无标签）。成本递增，鲁棒性也递增。下一讲我们看另一种更硬的验证信号，把推理放进工具里执行，以执行结果为准；第 6 讲的强化学习会把本讲的验证器当作奖励（reward）信号，奖励的质量直接决定训练的上限。"
  },
  {
   "cell_type": "markdown",
   "id": "8ef96a95",
  ...
  },
  {
   ...
   "id": "04fb449a",
   "metadata": {},
   "source": "Math-Shepherd 要解决的是步级标签的获取成本：步级标签好用，但人工标每一步太贵，于是把标注流程自动化。\\n标注的角度也换了：不问这一步本身算得对不对，而问从这一步继续还能不能推出正确答案。这个角度来自 MCTS（蒙特卡洛树搜索）：先用一个补全器（completer）从当前中间步续写 N 条后续路径，再看多少条最终到达黄金答案。\\n用具体数字演示一遍。假设某一步之后，补全器续写了 N = 8 条完整解，其中 3 条最终答案等于黄金答案、5 条不等于。硬估计（HE）只看存在性：有一条续写到达正确答案就标 1，所以这一步标 1。软估计（SE）看频率：3 / 8 = 0.375，这一步标 0.375。\\n两种估计用同一个式子表达。记第 j 条续写的最终答案为 $a_j$，黄金答案为 $a^*$：\\n\\n\`\`\`text\\nHE: y_i = 1[存在 j 使 a_j = a*]\\nSE: y_i = (1/N) * Σ_j 1[a_j = a*]\\n\`\`\`\\n\\nN 越大，SE 越接近这一步真实的正确率，HE 却更容易出假阳性。只要续写里有一条蒙对，这一步就被标成 1，即使它本身是错的。作者发现 SE 的分布更接近人工标注，HE 在 N 增大时因假阳性而退化。下面用一个脚本化的补全器复现这个规律。"
  },
  {
   "cell_type": "code",
   "execution_count": null,
  ...
  },
  {
   ...
   "id": "521f467a",
   "metadata": {},
   "source": "步级标签的价值不只在测试时选解，还能当作强化学习的奖励。\\n强化学习是一种靠反馈调整行为的训练方式，反馈信号叫奖励（reward）。传统结果监督在整条解结束时给一个奖励；步级标签允许在每一步结束时各给一个奖励，这一步算对给正分，算错给负分。训练时每一步都收到信号，比只在终点收到一个信号更容易学。\\nMath-Shepherd 用自动标注的步级标签做了 step-by-step PPO，效果优于用最终结果做奖励的 ORM-PPO。这两种训练算法不在这一讲展开，第 6 讲的 GRPO 延续的正是用步级奖励训练推理模型这个思路。\\n下面先用 SE 分数做 best-of-N 选解，再看一个步级奖励的小例子：整条解的奖励是各步奖励之和。"
  },
  {
   "cell_type": "code",
   "execution_count": null,
  ...
  },
  {
   ...
   "id": "5af8279d",
   "metadata": {},
   "source": "单个验证器不可靠时，把多个弱验证器集成起来。这里没有标签，于是需要从验证器自身的行为里估计每个验证器的精度。Weaver 的思路是：验证器之间的同意程度里藏着各自的精度。\\n先从两个验证器建立直觉。设一条解的正确与否为 Y（1 对 0 错），验证器各自独立地对每条解打 1/0，1 表示判定正确、0 表示判定错误。在 Y=1 的解上，验证器 k 打 1 的概率叫真阳性率 TPR_k；在 Y=0 的解上，它打 0 的概率叫真阴性率 TNR_k。这两个量就是验证器的精度。\\n精度不能直接观测，但另一些量可以：验证器 k 打 1 的比例，以及验证器 i、j 同时打 1 的比例。我们要把这两类能数出来的比例，和想求的 p、TPR、TNR 用等式连起来。记 $p = P(Y=1)$ 为解的正确率，由条件独立：\\n\\n\`\`\`text\\nP(S_k = 1)          = p·TPR_k + (1-p)·(1-TNR_k)\\nP(S_i = 1, S_j = 1) = p·TPR_i·TPR_j + (1-p)·(1-TNR_i)·(1-TNR_j)\\n\`\`\`\\n\\n左边是能直接统计的观测值，右边是含未知数的模型。方程数够不够，决定这些未知数能不能解出来。两个验证器有 5 个未知数（p 加每个验证器的 TPR、TNR），却只有 3 个方程，解不出；验证器增加到 4 个以上，方程数（K 个单变量矩加 K(K-1)/2 个成对矩）就超过未知数（1 + 2K），用最小二乘可以把每个 TPR、TNR 都解出来。这种用可观测的量反推不可观测参数的方法，叫矩估计，全程不需要人工标签。\\n有了精度，选解变成概率推断。给定一条解，各验证器打了向量 s，朴素贝叶斯假设各验证器在正确性条件下独立，算出后验概率：\\n\\n\`\`\`text\\nP(Y=1 | s) ∝ P(Y=1) · Π_k P(S_k = s_k | Y=1)\\n\`\`\`\\n\\n其中 $P(S_k=1|Y=1) = TPR_k$、$P(S_k=0|Y=1) = 1 - TPR_k$。分数最高的解，就是被一堆可靠验证器共同看好的解。下面代码模拟 5 个不同精度的弱验证器，只用无标签的同意率估计 TPR/TNR，再按后验选解。"
  },
  {
   "cell_type": "code",
   "execution_count": null,
  ...
  },
  {
   ...
   "source": [
    ...
    "        break\\n",
    "\\n",
    "print(\\"当前为 真实 API 演示：LLM 对加减法给出确定性结果；真实 API 下模型可能算错，由验证器拦截。\\")\\n",
