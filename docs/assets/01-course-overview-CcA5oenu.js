const n=`{
 "cells": [
  ...
  },
  {
   ...
   "metadata": {},
   "source": "这一讲从一个具体的任务开始。\\n\\n任务是这样的：把仓库里所有过期的 TODO 标记出来，并运行测试。\\n\\n这是一个\\"做事\\"的任务。\\n\\n我们把任务交给 ChatGPT。ChatGPT 会输出一段文字。它告诉我们应该怎么改。但它不会真的去改。\\n\\n原因是，模型的输入和输出都只是文字。它碰不到我们的文件。它跑不了测试。\\n\\n单次问答，做不了这件事。\\n\\n办法是把它放进一个循环里。\\n\\n我们把这个循环一步一步画出来。\\n\\n1. 把任务交给模型。\\n2. 模型根据当前的情况，决定下一步做什么。\\n3. 执行这一步。比如编辑一个文件，或者运行一条命令。\\n4. 把执行结果喂回给模型。\\n5. 回到第 2 步。模型再看结果，决定下一步。\\n\\n如此重复，直到任务完成。\\n\\n承担这个循环的结构，就是这一讲的主角。它叫 Agent。\\n\\n循环里最常用的三个词，我们先分别说清楚。\\n\\n1. 环境。环境是 Agent 身外的世界。文件系统、网页、数据库、代码执行器都算。\\n2. 状态。状态是任务进行到当前的全部信息。包括已经做过的动作和已经拿到的中间结果。\\n3. 动作。动作是模型输出的、能在环境里真实执行的一步操作。比如编辑一个文件，或运行一条命令。\\n\\n放到我们的任务上，这三个词是：\\n\\n- 环境：仓库和测试命令。\\n- 状态：已扫描的文件，和已标记的过期条目。\\n- 动作：编辑文件、运行测试。\\n\\n到这一讲结束，我们要亲手从零写出一条这样的循环。写完就明白，它和普通 LLM 应用的区别在哪里。"
  },
  {
   "cell_type": "markdown",
