const n=`{
 "cells": [
  ...
  },
  {
   ...
   "id": "md-03",
   "metadata": {},
   "source": "我们先复习一个简单的概率事实。\\n\\n假设某道题，模型单次作答，答对的概率是 0.3。\\n\\n如果让它独立尝试 k 次，至少有一次答对的概率是：\\n\\n$$1 - (1 - 0.3)^k$$\\n\\n我们代入几个 k 看看。\\n\\n- k = 1：概率是 0.30。\\n- k = 2：概率是 0.51。\\n- k = 5：概率是 0.83。\\n- k = 10：概率是 0.97。\\n\\n同一个模型，只是多答了几次。一道题的通过概率，就从 0.3 升到了接近 1。\\n\\n这个事实说明一件事。\\n\\n模型训练完之后，即使权重完全不动，能力也能提升。关键在回答问题时多花算力。\\n\\n这种训练结束后再花算力的方式，叫 test-time compute。\\n\\nOpenAI o1、DeepSeek-R1 这类模型，回答难题时会在内部多试几遍、自我检查。它们靠的正是这种方式。\\n\\n这一讲要回答两个问题。\\n\\n1. 多花算力，到底能换来多少准确率。\\n2. 给定一笔固定算力，怎么分配，收益最大。\\n\\n第一节，我们先建立一个合成基准。把\\"一道题能否被解出\\"变成可以测量的数量。"
  },
  {
   "cell_type": "markdown",
   "id": "md-04",
