const n=`{
 "cells": [
  ...
  },
  {
   ...
   "metadata": {},
   "source": [
    "> 前六讲回答的是同一个问题：人如何设计一个更好的 Agent。第 4 讲搭建了 ReAct 循环，第 5 讲设计了任务分解与树搜索，第 6 讲用强化学习训练 Agent 的参数。设计者始终是人，Agent 只是被设计出来的对象。\\n",
    ">\\n",
    "> 这一节把设计者本身交给搜索。我们让一个 Agent 去设计另一个 Agent：生成候选 Agent 的代码，在任务上评测打分，把好的结果存进 archive，再从这些结果中得到启发生成下一批。这样一套变异、选择、积累的循环就是开放进化。我们从 ADAS 的最小闭环出发，把它放大到 AI Scientist 的科研流程，再看 AlphaEvolve 把整份代码当作基因组来进化，最后讨论这套循环的失败模式。"
   ]
  },
  {
   "cell_type": "markdown",
   "id": "788574931dc75e94",
   "metadata": {},
   "source": "平时我们用电脑和手机，会碰到一个熟悉的节奏：软件隔一段时间就更新一次版本，新版本加新功能、修旧错误，之后再有新版本。自然界里也有类似的现象：动物和植物一代代繁衍，后代一点点变得更适应环境。这两件事的共同点是，系统不是被一次性设计好的，而是被不断改动、被挑选、一点点累积变好的。把同一个过程搬到 Agent 上，就是我们这一讲要做的：让一个 Agent 系统自己产生改动、自己判断改动好不好、把好的改动留到下一轮。这种循环叫开放进化。\\n\\n前几讲的训练和微调改的是模型的参数。第 6 讲用强化学习训练 Agent 的参数，训练结束，参数固定，Agent 的能力也固定下来。开放进化改的不是参数，而是更上层的设计：Agent 的代码、研究的想法、算法的结构。它不是把模型练到更好，而是让系统自己生成一批新方案，挑出好的，再基于好的方案继续生成，像物种演化那样一代代累积。这一讲会用到三个具体系统，ADAS、AI Scientist 和 AlphaEvolve，它们分别进化 Agent 的代码、研究的想法和整份算法源码。\\n\\n这套循环也有真实的风险。当系统自己评价自己的产出，而评价分数和真实目标脱节时，进化会找到取巧的路径，得高分却没有真的完成任务。评测的独立性是这套循环唯一的护栏，第 4 节会专门讨论。\\n\\n学完这一讲，我们能亲手实现一个会自我迭代的简化 Agent 系统。让一个程序去读已有 Agent 的代码，生成新代码，在任务上评测打分，把得分高的存进一个仓库，下一轮再基于仓库里的结果继续生成，这个负责设计 Agent 的程序叫元 Agent。在整套 Agent 框架里，第 1 讲的感知、决策、行动、反馈是一个循环，这一讲补的是循环最外层的能力：反馈不只用于改进当前这一次行动，还用于改进 Agent 本身。下面从 ADAS 开始，先看一个 Agent 如何设计另一个 Agent。"
  },
  {
   "cell_type": "markdown",
   "id": "70c72b462b84cb65",
   "metadata": {},
   "source": "这一节解决一个问题：一个 Agent 怎样去设计另一个 Agent。我们要搭一套最小的系统，让一个程序负责读已有的 Agent 代码、写出新代码、评测新代码的好坏、把好的代码留下来。负责设计 Agent 的这个程序有一个名字，叫元 Agent。整套系统叫 ADAS（Automated Design of Agentic Systems，Agent 系统的自动设计），它把设计 Agent 当成一个优化问题来解。\\n\\nAgent 的全部组件——提示词、工具调用、控制流——都写在同一个函数里，这个函数叫 forward。函数写好之后，整个 Agent 的行为就确定了，搜索 Agent 就是搜索这个函数的写法。函数所在的代码空间很大，单靠人一个个试不现实，所以 ADAS 用一套循环来找，循环由三个组件组成：变异算子、选择机制、种群。\\n\\n三个组件分别负责三件事。变异算子产生新代码，这一节由大模型扮演，它读已有的 Agent 代码，写出一份新的。选择机制判断新代码好不好，它是一段评测函数，在固定任务上运行新代码，得到一个准确率。种群存放变体和得分，这里是一个叫 archive 的字典，字典里每个名字对应一份代码和一个得分，供下一轮参考。下面先把评测函数和任务定下来。"
  },
  {
   "cell_type": "markdown",
   "id": "18766d1985d74a46",
   "metadata": {},
   "source": "设计 Agent 有两个层次。低层次是调参数：换提示词里的词、改推理步数、改采样次数。高层次是改结构：换一种决策方式，甚至换一套控制流。ADAS 搜索的是高层次，它的搜索空间是 Agent 的源代码，而不是一组参数。\\n\\n代码能作为搜索空间，因为 Python 的表达能力足够强。条件、循环、递归、调用其他函数，任意可计算的逻辑都能用代码写出来，理论上任何 Agent 行为都能被某段代码表达。这种\\"什么都能表达\\"的性质有一个名字，叫图灵完备。搜索空间足够大，大到人没想过的方案也可能藏在其中，代价是空间太大，必须靠变异、选择、积累的循环来搜索。\\n\\n用一个具体例子看代码如何承载策略。基线 Agent 的 forward 只有一行：\\n\\n\`\`\`text\\ndef forward(task):\\n    a, b = task\\n    return solve(a, b, recipe='direct')\\n\`\`\`\\n\\nrecipe 取什么值，Agent 就用什么策略。取 direct 是直接算，取 decompose 是先拆开再算，取 ensemble 是同时跑多个配方再投票。换一个词，Agent 的行为就换一种。真实系统里 forward 还可以写 if、for、多次调用 solve，控制流越丰富，能表达的策略越多。以代码为搜索空间，就是让变异算子在写代码这个空间里寻找更好的 forward。\\n\\n要把这个空间变成可以搜索的对象，先得有一份固定的任务，再有一把给方案打分的标尺，也就是评测函数。下面把这两样定下来。"
  },
  {
   "cell_type": "code",
  ...
  },
  {
   ...
   "id": "eb0446cc958c490b",
   "metadata": {},
   "source": "两条曲线讲的是两种搜索策略的差异。爬山从 (0, 1, 2) 出发，每步看四个邻居里谁评分最高，就移过去，最后停在 (7, 6, 2)，评分 0.585。随机搜索独立采样 30 个参数点，记录迄今最好的评分，最好的一次达到 (6, 10, 5)，评分 0.636。\\n\\n两个结果对比着看。爬山每步都向上，路线连续，终点 0.585 比起点 0.224 好得多；但它只看得到紧挨着的四个邻居，视野局限于当前位置附近。评分函数在参数空间里可能有不止一个高点，爬山的终点未必是最高处。随机搜索不沿邻居走，而是到处撒点，样本够多时更可能落进评分更高的区域，本例里 0.636 就高过了爬山终点。\\n\\n这个对比说明一件事：只靠局部修正，或者盲目乱试，都是粗糙的搜索。要让 Agent 的设计越过人工能想到的方案，需要把变异、选择、记忆组合成一个循环。下面进入 ADAS 的最小闭环。"
  },
  {
   "cell_type": "markdown",
   "id": "0f793e72e7aebbb5",
   "metadata": {},
   "source": "前面两个小节都在参数空间里演示变异和选择，但 ADAS 真正的搜索空间是代码。这一小节回答一个问题：把变异、选择、记忆组合成一个闭环，循环该怎么转。最小闭环是四步：元 Agent 读 archive 摘要，写出一份 forward 函数代码；我们把它编译成可调用对象；在算术任务上评测得到准确率；把结果连同代码存进 archive。archive 从两份基线 Agent 起步，逐轮生长。先搭基础设施。"
  },
  {
   "cell_type": "code",
   "execution_count": null,
  ...
  },
  {
   ...
   "id": "f916f93918176b00",
   "metadata": {},
   "source": "这一节回答一个问题：把第一节的循环从设计 Agent 放大到做科研，循环该怎么转。ADAS 把设计 Agent 放进了循环。AI Scientist 把同样的循环放大到完整的科研流程：提出研究 idea，写代码做实验，把结果写成论文，由自动评审器打分，再把通过的 idea 连同分数存进知识 archive。与 ADAS 相比有三个升级：变体从 Agent 代码变成研究 idea，适应度从任务准确率变成论文评审，种群从代码库变成知识库。\\n\\n自动评审器是闭环的关键，也是风险所在。论文里用按 NeurIPS 指南打分的 GPT-4o 评审 agent，在 500 篇 ICLR 2022 论文上达到接近人类水平的判断。当作者与评审都是同一个 AI 时，闭环里没有外部真值，也就是没有一个系统之外的客观标准来裁定结果对不对。我们用最小循环观察这个结构：大模型提出 idea，确定性环境跑出实验指标，评审函数决定取舍。"
  },
  {
   "cell_type": "code",
   "execution_count": null,
