const n=`{
 "cells": [
  ...
  },
  {
   ...
   "id": "md-02",
   "metadata": {},
   "source": "> 第 1 讲里，Agent 是\\"一次调用之外还能做更多计算\\"的结构。那只是一个定性的说法。这一讲把它变成一条可以测量的轴。模型训练完之后，不再更新权重。只在推理阶段多花算力，能力还能提升多少。\\n>\\n> 我们从最简单的\\"同一道题多答几次\\"开始。从零实现 pass@k 估计、多数投票和 best-of-n。再看给定一笔固定的算力预算时，怎么按题目难度分配收益最大。最后把多种方法组合成分层系统，用自动搜索找出最好的组合。全部实验用合成数据，离线可跑。"
  },
  {
   "cell_type": "markdown",
   "id": "md-03",
   "metadata": {},
   "source": "我们先复习一个简单的概率事实。\\n\\n假设某道题，模型单次作答答对的概率是 0.3。如果让它独立尝试 k 次，至少有一次答对的概率是：\\n\\n$$1 - (1 - 0.3)^k$$\\n\\n我们代入几个 k 看看。\\n\\n- k = 1 时，概率是 0.30。\\n- k = 2 时，概率是 0.51。\\n- k = 5 时，概率是 0.83。\\n- k = 10 时，概率是 0.97。\\n\\n同一个模型，只是多答了几次，一道题的通过概率就从 0.3 升到了接近 1。\\n\\n这个事实说明一件事：模型训练完之后，即使权重完全不动，能力也能提升。关键在回答问题时多花算力。这种训练结束后再花算力的方式，叫 test-time compute。OpenAI o1、DeepSeek-R1 这类模型，回答难题时会在内部多试几遍、自我检查，它们靠的正是这种方式。\\n\\n这一讲要回答两个问题。第一，多花算力到底能换来多少准确率。第二，给定一笔固定算力，怎么分配收益最大。第一节，我们先建立一个合成基准，把\\"一道题能否被解出\\"变成可以测量的数量。"
  },
  {
   "cell_type": "markdown",
   "id": "md-04",
  ...
  },
  {
   ...
   "id": "md-05",
   "metadata": {},
   "source": "这一节先给\\"花算力\\"一个统一的框架。再搭一个贯穿全讲的实验环境。\\n\\n模型在推理阶段可以做的选择，Snell 等人的工作把它归成两个旋钮。\\n\\n1. 第一个旋钮决定\\"生成什么样的候选答案\\"。让模型多试几次，还是让它在草稿上自我修订，都改这里。论文里叫提议分布（proposer）。\\n2. 第二个旋钮决定\\"拿到一批候选后怎么挑\\"。挑得分最高的，还是多数投票，都改这里。叫输出后处理（verifier）。\\n\\n后面几节的每个实验，都是这两个旋钮的某种组合。先记住这个框架。具体用法，后面逐个展开。\\n\\n实验需要一个可以反复使用的题目集。真实题目没法控制难度。所以这一讲用合成数据。我们模拟一批数学应用题。每道题给一个\\"单次正确率\\"，记为 p@1。p@1 表示\\"这道题一次作答就答对的概率\\"。p@1 高，题容易。p@1 低，题难。\\n\\n题目有难有易。大多数题 p@1 很低，只有少数题 p@1 高。这种\\"多数难、少数易\\"的分布，叫重尾分布。之后的每个实验，都以这批题目和它们的 p@1 为输入。先手动算一遍\\"多答几次\\"的收益，再生成这批题目。"
  },
  {
   "cell_type": "code",
  ...
  },
  {
   ...
   "id": "md-09",
   "metadata": {},
   "source": "第 1 节留了一个问题：多花算力到底提升多少。论文 Large Language Monkeys 给出了一个直接的回答：重复采样能大幅提高覆盖率。\\n\\n先看\\"重复采样\\"是什么意思。同一道题，模型生成 k 个候选答案。再从这 k 个里挑一个。这就是 pass@k。\\"采样 k 次后的通过率\\"。k 越大，花在推理上的算力越多。\\n\\n挑的时候需要一个选择标准。这个标准引出一对容易混淆的量。\\n\\n- 覆盖率（coverage）问\\"模型到底能不能解出这道题\\"。看的是 k 个候选里有没有正确的。\\n- 精度（precision）问\\"选择器挑得准不准\\"。看的是挑出来的那个是不是正确的。\\n\\n一个衡量\\"生成\\"端，一个衡量\\"挑选\\"端。后两节会反复用到这两个量。\\n\\n论文给出的数字很具体。SWE-bench Lite 上，DeepSeek-Coder-V2-Instruct 单次只解出 15.9% 的真实 GitHub issue。采 250 次后，提升到 56%。Gemma-2B 在 CodeContests 上 pass@1 只有 0.02%。采 1 万次后，pass@10k 提升到 7.1%，提高了 300 倍。采样次数能稳定换取覆盖率。而且收益的形态有规律。先处理一个测量上的细节，再看规律。"
  },
  {
   "cell_type": "markdown",
   "id": "md-cov-prec",
  ...
  },
  {
   ...
   "id": "md-10",
   "metadata": {},
   "source": "现在要处理\\"采样 k 次\\"里的一个测量细节。假设我们每道题真的生成 N 个样本。其中 C 个正确。直接数\\"有多少题在 N 个样本里出现过正确\\"，会系统性高估 pass@k。因为我们把 N 个样本全用上了，回答的其实是 pass@N。Chen 等人给出一个无偏估计：\\n\\n$$\\\\mathrm{pass@k} = \\\\frac{1}{P}\\\\sum_{i=1}^{P}\\\\left[1 - \\\\frac{\\\\binom{N-C_i}{k}}{\\\\binom{N}{k}}\\\\right]$$\\n\\n先看单个括号里的意思。从 N 个样本里选 k 个。一个正确的都没选到的比例是多少。用 1 减掉，就是\\"至少选到一个正确的\\"概率。分子就是从 N 个样本里选 k 个、一个正确的都没选到的组合数。\\n\\n再手算一个例子。某道题采样 N = 10 个样本，其中 C = 3 个正确。\\n\\n- k = 1 时：$\\\\binom{7}{1}/\\\\binom{10}{1} = 0.7$，pass@1 = 0.3，等于 C/N，符合直觉。\\n- k = 5 时：$\\\\binom{7}{5}/\\\\binom{10}{5} = 21/252 \\\\approx 0.083$，pass@5 ≈ 0.917。\\n- k = 10 时：$\\\\binom{7}{10} = 0$，pass@10 = 1。\\n\\n这三个值留给下面的代码验证。"
  },
  {
   "cell_type": "markdown",
   "id": "md-passk-why",
  ...
  },
  {
   ...
   "id": "md-13",
   "metadata": {},
   "source": "多花算力换覆盖率，收益不是线性的。把合成基准的覆盖率画成 k 的函数，会得到一条典型的推理期缩放曲线：开头涨得快，后面趋于饱和。论文里 Llama-3-8B-Instruct 在 MATH 上的覆盖率先从 100 样本的 82.9% 提升到 10000 样本的 98.44%。拟合出 $\\\\text{coverage} = \\\\exp(a\\\\,k^b)$，其中 a = -1.33、b = -0.43。我们在合成基准上复现这条曲线，并用最小二乘拟合同一个形式。"
  },
  {
   "cell_type": "markdown",
   "id": "md-powerlaw",
   "metadata": {},
   "source": "### 幂律是什么，以及为什么用 log-log 拟合\\n\\n这一节回答两个问题：幂律长什么样，以及怎么从数据里把它的两个参数拟合出来。\\n\\ncoverage 曲线 $c(k) = \\\\exp(a\\\\,k^b)$ 是典型的幂律。幂律指某个量随另一个量的幂次变化。$k^b$ 就是 k 的 b 次方。它与指数衰减 $(1-p)^k$ 的区分方式是画图。指数衰减在 log-linear 图上呈直线。k 每增加固定量，值缩小固定比例。幂律在 log-log 图上呈直线。k 每放大固定倍数，值缩小固定倍数。\\n\\n用论文参数 a = -1.33、b = -0.43 手算两个点。k = 100 时，$100^{-0.43} = 10^{-0.86} \\\\approx 0.138$。$c(100) = \\\\exp(-1.33 \\\\times 0.138) = e^{-0.18} \\\\approx 0.83$，对应论文的 82.9%。k = 10000 时，$10000^{-0.43} = 10^{-1.72} \\\\approx 0.019$。$c(10000) = e^{-0.025} \\\\approx 0.97$，与论文的 98.44% 同量级（拟合值本身是对观测的近似）。曲线快速爬升后趋于饱和。\\n\\n聚合覆盖率恰好呈幂律，这个现象的来源值得单独说明。Schaeffer 等人在 《How Do Large Language Monkeys Get Their Power (Laws)?》里给出了解释。单道题的失败率是指数衰减。但当各题的 pass@1 呈重左尾分布时，聚合到整个基准的失败率就从指数衰减变成幂律。下面一节手算这个转变。\\n\\n要对数据拟合 $c = \\\\exp(a k^b)$，先把方程变形。两边取对数得 $\\\\log c = a k^b$。a 是负数，$\\\\log c$ 也是负数，不方便再取对数。于是先取负号：$-\\\\log c = -a k^b$，再取对数：\\n\\n$$\\\\log(-\\\\log c) = \\\\log(-a) + b\\\\log k$$\\n\\n右边是 $\\\\log k$ 的一次函数。斜率就是 b，截距是 $\\\\log(-a)$。因此在 $\\\\log(-\\\\log c)$ 对 $\\\\log k$ 的图上，数据落在一条直线上。用一次线性回归（\`np.polyfit\`，degree 1）拟合斜率与截距，就能还原出 b 和 a。代码里 \`fit_power_law\` 做的就是这件事。"
  },
  {
   "cell_type": "code",
   "execution_count": null,
  ...
  },
  {
   ...
   "id": "md-22",
   "metadata": {},
   "source": "前面几节，所有题目共用同一种花法。但同一笔算力预算，花在不同难度的题上，效果不一样。容易的题，给个草稿再修一修就能做对。难的题，修多少次都可能在原地打转。不如多试几次，让多个独立候选互相竞争。把预算按题目难度分配，就是 compute-optimal 缩放。论文报告同等精度下，它比均匀 best-of-n 少花约 4 倍算力。\\n\\n难度的划分需要一个可操作的标准。论文用 base LLM 对每道题采 2048 个样本，估出 pass@1。按 5 分位数切成 5 个难度档。我们沿用这个做法。把合成基准按 $p_i$ 升序排好，均分成 5 档。档 1 最难、档 5 最易。我们用一套简化的修订模型，模拟\\"并行条数 × 顺序步数\\"的预算拆分。观察各档的最优拆分方式。"
  },
  {
   "cell_type": "markdown",
   "id": "md-budgetsplit",
   "metadata": {},
   "source": "### 预算拆分的两个方向\\n\\n一笔预算可以朝两个方向拆：并行条数和顺序步数。\\n\\n并行条数，是同时跑几条独立的链。顺序步数，是每条链内部做多少次修订。\\n\\n代码用参数 t 控制拆分。\\n\\n- 每条链的顺序步数：$N_{seq} = \\\\text{budget}^t$\\n- 并行条数：$N_{par} = \\\\text{budget} // N_{seq}$\\n\\n两者乘积不超过预算。\\n\\n用 budget = 16 手算三种情况。\\n\\n- t = 0：$N_{seq} = 16^0 = 1$，$N_{par} = 16$。即 16 条一步完成的独立尝试，等价于纯重复采样。\\n- t = 1：$N_{seq} = 16$，$N_{par} = 1$。即一条链连修 16 步，等价于纯顺序修订。\\n- t = 0.5：$N_{seq} = 16^{0.5} = 4$，$N_{par} = 4$。即 4 条链各修 4 步，总预算恰好 4 × 4 = 16。\\n\\nt 从 0 滑到 1，就是把算力从广撒网挪向深挖掘。\\n\\n难题偏并行、易题偏顺序。原因在修订模型 \`revision_gain\` 里。\\n\\n- p@1 达到 0.03 的题：每一步修订有 3p 的概率修复成功（上限 0.9）。\\n- p@1 低于 0.03 的题：修订增益为 0。一连串修订只会绕着原来的错误打转，不产生新信息。\\n\\n对难题（p@1 很低），增益为 0。链的成功率恒等于 p。加长顺序步数毫无作用。唯一的提升途径是增加并行条数。让多个独立尝试互相竞争。所以最优 t 偏向 0。\\n\\n对易题，每次修订都显著提高成功率。把预算集中到一条链上连续修订，比分散成多条并行链更划算。所以最优 t 偏向 1。\\n\\n这解释了为什么档 1（最难）的最优 t 接近 0。档 5（最易）的最优 t 接近 1。"
  },
  {
   "cell_type": "code",
   "execution_count": null,
  ...
  },
  {
   ...
   "id": "md-25",
   "metadata": {},
   "source": "前面算的都是\\"同一笔推理预算怎么花\\"。还有一个更根本的问题。这笔算力如果拿去训练一个更大的模型，会不会更值。Snell 的答案是，取决于推理负载比例 $R = D_{infer}/D_{train}$。\\n\\n训练算力约 $6ND_{train}$，推理算力约 $2ND_{infer}$。把模型参数放大 M 倍，总算力变成 M 倍的训练加推理。用小模型加 test-time compute 匹配这笔总预算时，可用的采样次数变为\\n\\n$$S = M + 3\\\\,\\\\frac{D_{train}}{D_{infer}}(M-1)$$\\n\\n取 M = 14。R 取 0.16（自举式训练，推理负载低）、0.79（典型负载）、22（大规模部署，推理负载高）。S 分别约 258、63、16。我们画出小模型的采样曲线。把\\"14 倍大模型贪心解码\\"的精度当作一条水平线。看三个 R 下的可用样本数，能把小模型推到线上方还是下方。"
  },
  {
   "cell_type": "markdown",
   "id": "md-flops",
