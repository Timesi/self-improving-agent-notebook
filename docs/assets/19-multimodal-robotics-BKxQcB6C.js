 ...
 "cells": [
  ...
  },
  {
   ...
   "source": [
    ...
    ">\\n",
    "> 这一讲把 Agent 放进物理世界——面对一台真实机械臂。核心模型类别是视觉-语言-动作模型（VLA），思路是把连续动作离散化成 token，放进视觉-语言模型的词汇表，用 next-token 预测同时输出指令理解、语言推理与机器人控制动作。"
   ]
  },
  {
   "cell_type": "markdown",
   ...
   "metadata": {},
   "source": [
    "一个能写代码、能搜资料、能规划多步任务的 Agent，接到一台机械臂上，却很可能连\\"把杯子放到盘子里\\"都完成不了。文本世界的动作是工具调用，结果以字符串返回；物理世界的动作是位移、转角与夹爪开合，结果以传感器读数返回。两者之间隔着连续控制、闭环反馈与不完美的真实环境。\\n",
    "\\n",
    "另一个约束来自数据。web 上可用的图文数据以十亿条计，而机器人轨迹数据（Open X-Embodiment）只有约一百万条，差了三个数量级。任何从头训练的策略都无法获得 web 模型已有的语义常识。这一讲的四条线由此展开：先定义具身智能与动作空间，再实现 VLA 的核心组件，然后走一遍机器人数据的采集与批量处理，最后看物理世界的反馈如何让模型持续改进。"
   ]
  },
  {
   "cell_type": "markdown",
  ...
  },
  {
   ...
   "metadata": {},
   "source": [
    "具身智能（embodied intelligence）把 Agent 的循环完整放进物理世界：感知环境（摄像头图像）、理解指令（自然语言）、输出动作（电机指令）、再观察结果（新的图像帧）。循环结构不变，输入输出都换成了物理信号。\\n",
    "\\n",
    "动作空间要定义得具体。一台移动操作臂，一次动作由 7 个连续量描述：末端执行器的三维位移增量（Δpos_x, Δpos_y, Δpos_z）、三维旋转增量（Δrot_x, Δrot_y, Δrot_z）与夹爪开度（gripper，0 到 1）。每个连续维度按均匀区间离散成 256 个 bin，用一个 0 到 255 的整数表示，另加一个终止位。\\n",
    "\\n",
    "先手算一个值。夹爪开度 a = 0.4 落在 [0, 1] 上，bin 宽为 1/256 ≈ 0.0039，序号为\\n",
    "idx = min(255, ⌊a / (1/256)⌋) = ⌊0.4 × 256⌋ ≈ 102。\\n",
    "反解取 bin 中心：â = (102 + 0.5) / 256 ≈ 0.4004，误差在半个 bin 宽以内。规模上的反差先看一眼。"
   ]
  },
  {
   "cell_type": "code",
  ...
  },
  {
   ...
   "metadata": {},
   "source": [
    "RT-2 把\\"动作是另一种语言\\"落到实现上：离散化后的 7 个 bin 序号加上终止位，拼成一个空格分隔的字符串，例如 \`\\"1 128 91 241 5 101 127 200\\"\`。这个字符串与自然语言一起进入训练集，模型以 VQA 格式回答 \\"Q: what action should the robot take to [task]? A:\\"，训练目标仍是 next-token prediction，动作 token 与文字 token 在同一序列里逐位预测。\\n",
    "\\n",
    "动作 token 的分配有一个取舍。RT-2 的 PaLI-X 主干给 0 到 1000 的整数各分配了独立 token，bin 序号直接绑定对应整数 token；PaLM-E 主干没有这个便利，则覆盖 256 个最不常用的 token。覆盖现有 token 本身是一种 symbol tuning——模型不增加参数，只是把旧 token 的含义改写为动作 bin。"
   ]
  },
  {
   "cell_type": "code",
  ...
  },
  {
   ...
   "metadata": {},
   "source": [
    "解码时还有一个约束。普通 VQA 任务可以输出任意自然语言 token，机器人任务却不能——argmax 可能落在某个文字 token 上，得到非法动作。RT-2 的做法是机器人任务解码时屏蔽词汇表，只允许采样动作 token，称为输出约束（output constraint）。它不改变训练，只在推理时生效：把非动作位置的 logits 置为 −inf，softmax 后这些位置的概率就是 0。"
   ]
  },
  {
   "cell_type": "code",
  ...
  },
  {
   ...
   "metadata": {},
   "source": [
    "模型部分用 torch 从零搭一个极小的 VLA。三块组件对应真实 VLA 的三段：视觉编码器（真实模型是 SigLIP 加 DINOv2 融合视觉塔，这里用两层全连接简化，把图像特征映射成向量）、指令编码器（真实模型是 Llama 语言主干，这里用 token embedding 加平均池化）、动作解码器（真实模型输出各动作槽位的 256 类分布，这里输出 7×256 的 logits）。图像向量与指令向量拼接后进动作解码器，交叉熵只在动作 token 上计算，与 OpenVLA 的训练目标一致。"
   ]
  },
  {
   "cell_type": "code",
  ...
  },
  {
   ...
   "metadata": {},
   "source": [
    "还有一个关键训练细节：co-fine-tuning。RT-2 不是只在机器人数据上微调，而是把机器人轨迹与原始 web 视觉-语言数据一起微调，并提高机器人数据在每批里的采样权重。原因是灾难性遗忘——只训机器人数据，模型会丢掉 web 学到的抽象概念，泛化能力随之下降。\\n",
    "\\n",
    "下面用共享主干的两任务 toy 实验复现这个效应。共享部分是文本编码器，语言任务把它读出类别标签，机器人任务把它读出动作。为了让遗忘真实发生，机器人头只用一层线性映射、中间不留非线性——这样动作任务无法靠头部自行解决，必须改写共享的文本特征，机器人梯度才真正碰到语言能力所在的位置。"
   ]
  },
  {
   "cell_type": "code",
  ...
  },
  {
   ...
   "metadata": {},
   "source": [
    "机器人数据靠遥操作采集：操作员戴上 VR 头显或直接拖动机械臂完成一次任务，系统同时记录摄像头图像、夹爪状态与动作指令。这样的一条记录叫一条轨迹（episode），结构是一段语言指令加一串图像帧加一串 7 维动作。Open X-Embodiment 把 70 多个实验室的这类数据汇总成统一格式，筛选后约 100 万条轨迹。下面合成一条 toy 轨迹，走一遍从原始记录到训练样本的转换。"
   ]
  },
  {
   "cell_type": "code",
  ...
  },
  {
   ...
   "metadata": {},
   "source": [
    "训练时要一次喂一批样本。动作 token 固定是 8 个整数，天然等长；图像是定尺寸数组，也直接张量化。需要变长处理的是指令文本：不同指令的 token 数量不同，要 pad 到批内最长并记录 attention mask，让模型忽略填充位。"
   ]
  },
  {
   "cell_type": "code",
  ...
  },
  {
   ...
   "metadata": {},
   "source": [
    "训练不是一次性完成的。一个典型的落地循环是：遥操作采集一批轨迹 → 训练模型 → 在真实机械臂上闭环评测（模型实时读图像输出动作）→ 把失败轨迹修正后回流数据集 → 再训练。仿真与真实之间还有 sim-to-real 差距：仿真的物理参数、光照、纹理更理想，仿真里的成功不保证真实环境成功，闭环评测因此必须回到真机。\\n",
    "\\n",
    "闭环评测也暴露数据质量问题。OpenVLA 清洗 Bridge 数据时发现大量全零动作（夹具未动却被记录成零位移），这类样本直接污染动作 token 的分布。数据清洗本身是\\"从物理世界学习\\"的一部分：记录与标签都不完美，需要反馈来甄别。"
   ]
  },
  {
   "cell_type": "code",
  ...
  },
  {
   ...
   "metadata": {},
   "source": [
    "最后看语言推理如何桥接到低层控制。RT-2 的 CoT 变体把每条数据增广成 \\"Instruction: I'm hungry. Plan: pick the snack. Action: 1 128 124 ...\\" 的格式，让模型先生成自然语言计划、再输出动作 token。语言计划承接 web 预训练带来的推理能力，动作 token 则约束在动作词表内。下面用 llm_client 走一遍这个链式格式，并把动作字符串反解回连续动作。"
   ]
  },
  {
   "cell_type": "code",
   ...
   "outputs": [],
   "source": [
    "# 初始化 LLM 客户端（无 key 时自动进入 mock 模式）\\n",
    "import os\\n",
    "import sys\\n",
    "\\n",
  ...
  },
  {
   ...
   "source": [
    ...
    "import re\\n",
    "\\n",
    "# 规则式兜底规划器：mock 模式下提供可解析的 Plan/Action 占位\\n",
    "FALLBACK = {\\n",
    ...
    "print(\\"模型回复：\\\\n\\" + reply)\\n",
    "\\n",
    "if client.is_mock or parse_action_part(reply) is None:\\n",
    "    plan, action_str = rule_plan(\\"hungry\\")\\n",
    "    source = \\"mock 模式使用规则兜底（占位）\\"\\n",
    "else:\\n",
  ...
  },
  {
   ...
   "source": [
    "## 小结\\n",
    "\\n",
    "这一讲完成了多模态机器人智能体的核心实现：\\n",
    "\\n",
    "- [ ] 具身智能与 VLA：Agent 循环放进物理世界，动作由 7 维连续量（位移、旋转、夹爪）描述\\n",
    "- [ ] 数据鸿沟：web 图文数据与机器人轨迹数据差约 3 个数量级，是复用 VLM 的核心动机\\n",
    "- [ ] 动作 token 化：连续动作按 256 bin 均匀离散化，编解码往返误差在 bin 宽以内\\n",
    "- [ ] 分位数 binning：OpenVLA 用 1-99 分位数定 bin 区间，抗离群动作优于 min-max\\n",
    "- [ ] 词汇表屏蔽：机器人任务解码时只允许采样动作 token，非动作位置 logits 置 -inf\\n",
    "- [ ] mini VLA：视觉编码器（MLP）+ 指令编码 + 动作解码器，交叉熵只在动作 token 上计算\\n",
    "- [ ] co-fine-tuning：机器人数据与 web 数据一起微调，防止语言能力灾难性遗忘\\n",
    "- [ ] 数据管线：一条轨迹 → (图像, 指令, 动作 token 字符串) 样本，指令按批内最长 pad 并加 attention mask\\n",
    "- [ ] 反馈闭环：遥操作采集 → 训练 → 真机评测 → 失败样本回流；Plan+Action 链式推理桥接语言与动作"
   ]
  },
  {
   "cell_type": "markdown",
