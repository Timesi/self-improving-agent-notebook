const n=`{
 "cells": [
  ...
  },
  {
   ...
   "metadata": {},
   "source": [
    "> 前几讲各处理了一类方法：06 讲给出强化学习的算法引擎（STaR、GRPO、DAPO），07 讲展示 Agent 自我改进的形态，08 讲把搜索与深度研究做成一个具体的 Agent。这些方法分布在不同的章节里，各自回答一个局部问题。\\n",
    ">\\n",
    "> 本讲把它们收拢成一条时间线，回答一个总的问题：预训练只给了模型续写文本的能力，它怎样变成听话的助手，又怎样变成会用工具完成任务的 Agent。我们跟随一个模型的一生，观察每一阶段输入什么新信号，以及训练信号如何从人类手里一步步交到环境手里。\\n"
   ]
  },
  {
   "cell_type": "markdown",
   "id": "09-02",
   "metadata": {},
   "source": "我们每天都在用 ChatGPT 或 DeepSeek，同一个模型既会聊天，也能写代码、订机票、用浏览器查资料。它并不是天生就会这些的。模型在预训练阶段只学会了一件事：把一句话接下去。给它\\"天气不错，我们\\"，它能补出\\"出去散步\\"。可它不知道什么是指令。我们问它\\"12 加 8 等于几\\"，它只会顺着这句话继续写，而不是回答 20。\\n\\n要让模型从\\"会接话\\"变成\\"会干活\\"，人类在预训练之后又做了大量训练：先拿人类写好的问答示例教它照着做，再让标注员给它的不同回答打分排序，后来干脆用可以验证的答案、以及环境执行的结果来训练它。发生在预训练之后、目的是让模型更好用的这些训练，统称后训练，英文叫 post-training。这一讲讲的就是这条路：一个只会续写文本的模型，怎样变成听话的助手，又怎样变成会用工具的 Agent。\\n\\n学完这一讲，我们能说清从续写器到 Agent 之间要经历哪几种训练信号、每种信号解决什么问题，能亲手比较它们给出的更新方向有什么不同，也能看懂 SFT、RLHF、RLVR 这些名词各自指什么。在整套 Agent 框架（感知、决策、行动、反馈的循环）里，前几讲写的是 Agent 怎样利用反馈做决策，这一讲补的是反馈本身：这些训练信号怎样把模型本身训练得更好。\\n\\n第一节先看起点：一个只有预训练能力的模型，面对指令时会输出什么。"
  },
  {
   "cell_type": "markdown",
   "id": "09-03",
