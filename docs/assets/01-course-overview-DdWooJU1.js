 ...
 "cells": [
  {
   "cell_type": "markdown",
   "metadata": {},
   "source": "# 课程总览：什么是 AI Agent\\n"
  },
  {
   "cell_type": "markdown",
   "metadata": {},
   "source": "> 我们已经知道，大语言模型能生成流畅的文本，能写代码、做翻译、回答知识性问题。但单次调用只输出一段文本，不接触外部世界，也无法把一个需要多步操作的任务从头做到尾。\\n>\\n> 这一讲是整门课的起点，回答三个问题：AI Agent 是什么，为什么它是大语言模型的下一个形态，这门课将沿着什么路线把 Agent 亲手做出来。我们会先看一个单次调用做不到、Agent 循环能做到的对照，再亲手从零搭出第一个最小循环。\\n"
  },
  {
   "cell_type": "markdown",
   "metadata": {},
   "source": "Agent 没有一个统一的定义，但这一讲可以确立一个贯穿全课的说法：Agent 是一个循环，LLM 根据当前状态决定下一步动作，动作在环境中执行，观察结果再喂回 LLM，直到任务完成。这句话里的每个词都对应一个具体组件，后面五讲会逐一展开。\\n\\n先看一个具体任务：把仓库里所有过期的 TODO 标记出来，并运行测试。把它交给一个只有对话能力的大语言模型，它会给出建议文本，但不会真的去改文件。要让模型完成这类任务，需要把单次问答改造成循环执行——模型负责思考和选动作，我们负责维护状态、调用工具、把环境结果带回去。\\n\\n这一讲先建立这张地图，从单次调用与 Agent 的差异出发，给出定义，亲手实现一条最小循环，再看主流范式与课程路线。\\n"
  },
  {
   "cell_type": "markdown",
   "metadata": {},
   "source": "## 1. 从 LLM 应用到 Agent\\n\\n单个 forward pass 的 LLM 有四条硬边界。\\n\\n一是只能\\"说\\"不能\\"做\\"：它输出 token，不接触世界，不知道网页、数据库、文件或代码执行器里发生了什么。二是上下文窗口有限：长任务和大量中间结果塞不进一次调用。三是错了不会自己纠正：单次生成没有执行反馈，无法知道答案跑起来是错的。四是知识静态：训练截止日期之后的新事实无法获取。\\n\\n先建立统一的客户端，再跑一次单次调用，观察它的输出。\\n"
  },
  {
   "cell_type": "code",
   "metadata": {},
   "source": "# 把仓库根目录加入模块搜索路径，才能 import 到 llm_client\\nimport sys\\nimport os\\n_root = os.path.abspath(os.getcwd())\\nwhile not os.path.exists(os.path.join(_root, \\"llm_client.py\\")):\\n    _root = os.path.dirname(_root)\\n    if _root == os.path.dirname(_root):\\n        break\\nif _root not in sys.path:\\n    sys.path.insert(0, _root)\\n\\nfrom llm_client import get_llm\\n\\n# force_mock=True 保证离线可复现；配置真实 key 后去掉该参数即可\\nclient = get_llm(force_mock=True)\\nprint(\\"客户端类型:\\", type(client).__name__)\\nprint(\\"mock 模式:\\", client.is_mock)\\n",
  ...
  },
  {
   "cell_type": "code",
   "metadata": {},
   "source": "# 一个 LLM 应用的核心动作：把任务拼进 prompt，调用一次，拿到文本\\ntask = \\"把仓库里所有过期的 TODO 标记出来，并运行测试\\"\\nreply = client.chat([{\\"role\\": \\"user\\", \\"content\\": task}])\\nprint(\\"模型回复（单次调用）:\\\\n\\", reply)\\nprint(\\"\\\\n上下文长度:\\", len(reply), \\"字符\\")\\nprint(\\"关键观察：回复只是一段文本，没有真的修改仓库，也没有运行测试\\")\\n",
   "execution_count": null,
   "outputs": []
  },
  {
  ...
  },
  {
   "cell_type": "markdown",
   "metadata": {},
   "source": "循环之外还有几个常被讲成独立模块、其实是循环配件的部分。\\n\\n记忆负责把上下文窗口之外的信息接回来：短期是消息历史，长期是外部存储（向量库或文件）。规划把目标分解成子目标，或对动作序列做搜索。验证在循环之外加一个检查环节，让输出经过验证才被接受。它们不是循环的主体，但都挂接在循环的某个环节上，后面讲到对应讲次时再展开。\\n\\n循环的主体只有四步加一个终止条件，下面从零实现它。\\n"
  },
  {
   "cell_type": "markdown",
   "metadata": {},
   "source": "## 3. 一条 Agent 循环\\n\\n把上一节的组成写成可运行的代码，是这一讲的第一个动手目标。循环体做四件事：调用 LLM 得到回复，解析回复里的动作，执行动作并把观察结果回填进消息历史，重复直到模型给出 \`Final Answer\` 或步数用尽。消息历史就是循环的状态。\\n\\n任务取一个多步算术：计算 \`(3+5)×(7-2)\`。先在纸上分步算一遍，给代码一个对照基准。\\n"
  },
  {
   "cell_type": "code",
   "metadata": {},
   "source": "# 手算验证：不借助工具，先把 (3+5)×(7-2) 分步算一遍\\nstep1 = 3 + 5\\nstep2 = 7 - 2\\nstep3 = step1 * step2\\nprint(f\\"第一步 (3+5) = {step1}\\")\\nprint(f\\"第二步 (7-2) = {step2}\\")\\nprint(f\\"第三步 8×5   = {step3}\\")\\nprint(\\"手算与之后 Agent 循环的结果应当一致\\")\\n",
   "execution_count": null,
   "outputs": []
  },
  {
   "cell_type": "markdown",
   "metadata": {},
   "source": "模型输出的文本需要先被解析成结构化动作。模型可能输出三种内容：\`Thought\` 加 \`Action\`、\`Final Answer\`、或普通文本。解析器负责识别这三种情况，返回统一的 (kind, action, payload) 结构。\\n"
  },
  {
   "cell_type": "code",
   "metadata": {},
  ...
  },
  {
   "cell_type": "markdown",
   "metadata": {},
   "source": "## 5. 课程路线\\n\\n课程沿四部分展开，共 17 个 notebook。Part 1 补能力：test-time compute、验证、工具、规划，从\\"LLM 只能生成\\"出发逐层加。Part 2 补怎么变强：训练期缩放、开放进化、搜索、后训练演进。Part 3 补工程化：SWE、记忆、评测。Part 4 补边界：推理、数学、自治、机器人。下面把每部分的讲次摊开。\\n"
  },
  {
   "cell_type": "code",
   "metadata": {},
   "source": "# 课程地图：四部分、17 个 notebook\\nroadmap = [\\n    (\\"Part 1  Foundation\\",\\n     [\\"L1 课程总览\\", \\"L2 Test-time compute\\", \\"L3 鲁棒验证\\",\\n      \\"L4 工具与代码反馈\\", \\"L5 多步规划\\"]),\\n    (\\"Part 2  Training\\",\\n     [\\"L6 训练期缩放\\", \\"L7 开放进化\\", \\"L8 搜索与深度研究\\", \\"L9 后训练演进\\"]),\\n    (\\"Part 3  Engineering\\",\\n     [\\"L13 SWE Agent\\", \\"L14 记忆系统\\", \\"L17 评测\\"]),\\n    (\\"Part 4  Frontiers\\",\\n     [\\"L15 LLM 推理\\", \\"L16 数学 Agent\\", \\"L18 自治 Agent\\",\\n      \\"L19 多模态机器人\\", \\"L20 未来方向\\"]),\\n]\\nfor part, lectures in roadmap:\\n    print(part, \\"—\\", \\"、\\".join(lectures))\\ntotal = sum(len(lectures) for _, lectures in roadmap)\\nprint(f\\"\\\\n总计 {total} 个 notebook\\")\\n",
   "execution_count": null,
   "outputs": []
  },
  {
  ...
  },
  {
   "cell_type": "markdown",
   "metadata": {},
   "source": "每一讲都沿同一条教学契约推进：直觉理解 → 手算验证 → 代码实现 → 实验观察。这一讲已经示范了前面几步：先建立循环的直觉，再在纸上分步推演，最后从零把它写出来。后续每一讲都会重复这个节奏。\\n\\n到这一讲结束，我们已经写出了第一个最小循环：一个 \`AgentLoop\` 类、一个工具注册表、一个能解析动作的解析器，加起来不到五十行。这足以说明一件事：Agent 与应用的差距在结构而不在模型。循环把环境、工具、记忆和多步算力接入模型，能力增量由此而来。\\n\\n下一讲从 test-time compute 开始，看训练完成后，把额外的算力花在推理阶段，如何补偿单次生成的质量。\\n"
  },
  {
   "cell_type": "markdown",
   "metadata": {},
  ...
  },
  {
   "cell_type": "markdown",
   "metadata": {},
   "source": "**作业 2：动作解析器**\\n\\n从零写一个更简的 \`parse_action\`，返回 \`(kind, name, args)\` 三元组：支持 \`Action\`、\`Final Answer\`、无动作三种情况，并正确处理多余空白与多行。参考答案已经填好，请先自己在草稿上补全，再运行对照。\\n\\n小提示：先找 Final Answer 再做 Action，因为 Final 所在行不该被当成 Action。\\n"
  },
  {
   "cell_type": "code",
   "metadata": {},
