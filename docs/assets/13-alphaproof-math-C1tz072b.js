const n=`{
 "cells": [
  ...
  },
  {
   "cell_type": "markdown",
   "metadata": {},
   "source": "你可能已经注意到，AI 做数学题的能力越来越强。几年前它还会在简单的方程上出错，到 2024 年，DeepMind 的 AlphaProof 与 AlphaGeometry 在国际数学奥林匹克（IMO）上达到银牌水平，2025 年 Gemini Deep Think 更是在比赛时限内拿到金牌。数学竞赛题本来是出给人类的，现在机器也能在这个赛场上拿奖。这个变化靠的不只是更大的模型，还有更聪明的解题流程。\\n\\n数学题之所以能当推理的试金石，是因为它对错有唯一答案，而且可以严格验证。一道题证出来了就是证出来了，不能说\\"看起来差不多\\"就算对；每一步都必须有规则依据，错一步就是错。对话任务里，一段错误但流畅的回答常被当作大致正确；数学任务里，这种含糊会被一个绝对公正、永不犯错的裁判当场判零分。\\n\\n要让机器在这位裁判面前站得住，这一讲要补两样能力，正好是第 1 讲里 Agent 循环的配件：验证和搜索（规划的一种）。验证给循环加一道检查，输出必须逐行通过才被接受；搜索属于规划那一类，一次尝试多条动作路线，保留能到终点的那条。上一讲我们看过模型自己多花算力去思考，这一讲再添两道保障：先验证再放行，这条路不通就换一条。学完这一讲，我们能亲手实现三样东西：一个只按规则推导几何结论的迷你引擎，一个逐行检查证明合法性的验证器，以及一个生成候选、验证、推进的搜索循环。这一讲先从判分规则看起，再动手搭建这三样东西。",
   "id": "cell-002"
  },
  {
   "cell_type": "markdown",
   "metadata": {},
   "source": "## 1. 数学：推理的试金石\\n\\n这一节用一条经典的错误证明，把\\"机器逐行把关\\"和\\"人眼大致判断\\"的差别落到具体例子上。下面这段证明声称 2 等于 1，是经典的除以零骗局：设 a = b，两边同乘 a 得 a² = ab，两边减 b² 得 a² − b² = ab − b²，即 (a+b)(a−b) = b(a−b)。两边同除以 (a−b) 得 a+b = b，代回 a = b 得 2b = b，于是 2 = 1。\\n\\n如果这段写在考卷上，人一眼能看出问题：因为 a = b，所以 a − b = 0，除以 0 是非法的。难处在于一个逐行处理符号的机器，若不检查除以 0 这条规则，也会一路算到底。自然语言证明的每一步都没有被机器验证，错误藏在某一行里，任何看起来合理的中间结果都不能保证结论正确。这正是数学任务与对话任务的分野：前者需要一个逐行把关的裁判，后者没有。",
   "id": "cell-003"
  },
  {
   "cell_type": "markdown",
   "metadata": {},
   "source": [
    "把 a 和 b 代入具体数字，看得更清楚。设 a=1，b=1。设 a=b 是 1=1，成立；两边同乘 a 得 a²=ab，即 1=1，成立；两边减 b² 得 a²−b²=ab−b²，即 0=0，成立；因式分解后 (a+b)(a−b)=b(a−b)，代入得 2×0=1×0，也就是 0=0，仍然成立。到这里每一步都对。关键在最后一步：两边同除以 (a−b)，而 a−b=1−1=0，等式两边都在除以 0，这一步没有定义。前面所有等式都合法，唯独这个除法不合法，2=1 的错误全在这一行。\\n",
    "\\n",
    "人一眼能看穿，是因为人脑自带\\"0 不能做除数\\"的背景知识，会下意识检查除数是不是零。一个逐行处理符号的机器没有这份背景知识：它把\\"等式两边同除一个数\\"当成一条纯形式的变换规则，不看除数取什么值，于是把 0 也当除数一路算到底。要让机器停下来，唯一办法是把\\"除数非零才允许同除\\"写成一条显式规则，让每一行都对照规则判定合法性。这就是逐行把关的裁判的含义：合法性由规则决定，而不是靠执行者用常识补全。\\n",
    "\\n",
    "这个例子还解释了判分为什么是全有或全无：验证器逐行检查，只要有一行非法，整条证明就不成立。哪怕证到倒数第二行，中间夹着一行非法变换，也和完全没做一样拿 0 分。奖励只能在这条连续链条的末端兑现，这是稀疏奖励最直接的来源。\\n"
   ],
   "id": "cell-003a"
  },
  {
  ...
  },
  {
   "cell_type": "markdown",
   "metadata": {},
   "source": "## 2. 让神经网络提出几何步骤\\n\\n这一节回答一个问题：几何证明题，机器怎么一步步推？答案是分工——一套只认规则的引擎负责一步步推导，卡住时由一个神经网络补上新对象。我们先看引擎这边怎么工作，再看两边怎么接起来。\\n\\n先看最基本的操作：从已知事实推出新事实。AlphaGeometry 把这类推理交给一个叫 DDAR 的引擎。它只按事先写好的规则处理符号，规则怎么定它就怎么推，不\\"理解\\"几何，这类程序叫符号引擎。DDAR 由两部分组成。Deductive Database（DD）是一组演绎规则，比如\\"等腰三角形两底角相等\\"、\\"全等三角形对应边相等\\"；Algebraic Reasoning（AR）负责解角度、比例与距离的方程。下面重点看 DD 的规则。\\n\\n每条 DD 规则都写成\\"如果某些条件成立，就推出一个结论\\"。比如条件是 isos(A,B,C)，表示三角形 ABC 以 A 为顶点等腰，结论是角 ABC 等于角 ACB。这类由一组条件合取推出一个结论的规则，每条都确定、可验证：只要条件满足，结论就一定成立。引擎反复套用这些规则，直到不再产生新事实为止。\\n\\nDDAR 有一个短板：需要辅助构造的题会彻底卡住。辅助构造指题面之外新加的点或线，比如取 BC 的中点 M、过 A 作 BC 的垂线。AlphaGeometry 用一个 150M 参数的 decoder-only transformer，在引擎卡住时提议下一步加哪个辅助点，然后交给 DDAR 继续推。\\n\\n我们把这条分工装进一个最小版本。下面的迷你几何符号引擎只处理一个 toy 定理：已知三角形 ABC 等腰（AB = AC，顶点 A），且 M 是底边 BC 的中点，求证角 AMB 等于角 AMC。规则库如下：\\n\\n- R1 \`isos(A,B,C)\` → \`ang_eq(ABC, ACB)\`：等腰三角形两底角相等\\n- R2 \`isos(A,B,C)\` → \`seg_eq(AB, AC)\`：等腰定义，两腰相等\\n- R3 \`mid(M,B,C)\` → \`seg_eq(BM, CM)\`：中点定义，BM = CM\\n- R4 \`mid(M,B,C)\` ∧ \`isos(A,B,C)\` → \`ang_eq(ABC, ABM)\`：M 在 BC 上，两角是同一个角\\n- R5 \`mid(M,B,C)\` ∧ \`isos(A,B,C)\` → \`ang_eq(ACB, ACM)\`：同理，C 端共线同角\\n- R6 \`ang_eq(x,y)\` ∧ \`ang_eq(y,z)\` → \`ang_eq(x,z)\`：角相等传递\\n- R7 \`ang_eq(x,y)\` → \`ang_eq(y,x)\`：角相等对称\\n- R8 边角边：\`seg_eq(AB,AC)\` ∧ \`seg_eq(BM,CM)\` ∧ \`ang_eq(ABM,ACM)\` → \`congr(ABM,ACM)\`\\n- R9 \`congr(ABM,ACM)\` → \`ang_eq(AMB,AMC)\`：全等三角形对应角相等",
   "id": "cell-006"
  },
  {
   "cell_type": "markdown",
   "metadata": {},
   "source": "先用一个具体三角形把规则库读明白。取 A=(0,3)、B=(−2,0)、C=(2,0)。两腰 AB 与 AC 的长都是 √((0−(−2))²+(3−0)²)=√13，所以三角形 ABC 是以 A 为顶点的等腰三角形，记作 isos(A,B,C)。两个底角分别是 ∠ABC 与 ∠ACB，用余弦定理手算：向量 BA=(2,3)、BC=(4,0)，cos∠ABC=(2×4+3×0)/(√13×4)=2/√13；向量 CA=(−2,3)、CB=(−4,0)，cos∠ACB=(8+0)/(√13×4)=2/√13。两个角的余弦相同，都在 0 到 π 之间，所以 ∠ABC=∠ACB。这正是规则 R1 表述的几何事实：等腰三角形的两底角相等。\\n\\n规则库里的每一条都写成\\"前提合取 → 一个结论\\"的形式，这样的规则叫 Horn 子句。左边是若干条必须同时成立的事实，右边是一条新事实。例如 R1 写作 \`isos(A,B,C) → ang_eq(ABC, ACB)\`，读作\\"如果三角形 ABC 以 A 为顶点等腰，那么角 ABC 等于角 ACB\\"。角用三个点命名，中间点是角的顶点：ang_eq(ABC, ACB) 表示 ∠ABC=∠ACB，在程序里对应六元组 \`(\\"ang_eq\\",\\"A\\",\\"B\\",\\"C\\",\\"A\\",\\"C\\",\\"B\\")\`。其余谓词同理：seg_eq(AB, AC) 表示线段 AB 等于 AC，mid(M,B,C) 表示 M 是线段 BC 的中点，congr(ABM, ACM) 表示三角形 ABM 与 ACM 全等。\\n\\n把九条规则逐条解释清楚。R1 由等腰推底角相等，R2 由等腰推两腰相等，R3 由中点推 BM=CM。R4 与 R5 说的是 M 在 BC 上，射线 BM 与射线 BC 重合，所以 ∠ABC 与 ∠ABM 是同一个角，∠ACB 与 ∠ACM 也是同一个角。R6 与 R7 是角相等的传递与对称，R8 是判定全等的边角边定理，前提是两邻边与夹角分别相等，R9 由全等推对应角相等。目标命题是 ∠AMB=∠AMC，记作 ang_eq(AMB, AMC)。\\n\\n每条规则都确定、可验证：只要前提在知识库里，结论就一定能加入，没有例外。这是符号引擎可以放心相信它的原因——演绎的每一步都由这样一条朴素的规则背书。",
   "id": "cell-006a"
  },
  {
   "cell_type": "code",
  ...
  },
  {
   ...
   "metadata": {},
   "source": [
    "符号引擎不会只推手算时挑出的那两条。它把反复应用规则这件事自动化，这个过程叫前向演绎。知识库从一个前提事实集合开始，反复扫描全部规则：只要一条规则的前提全部匹配库里的已有事实，就把结论加进知识库；一轮扫描下来没有新增任何事实，就停止。这时知识库叫闭包，也就是规则库能推出的全部事实。\\n",
    "\\n",
    "把版本 A 完整走一遍，前提只有 isos(A,B,C) 一条。第 1 轮扫描：R1 加入 ang_eq(ABC,ACB)，R2 加入 seg_eq(AB,AC)，此时库里只有一个角相等事实，R6 的传递配不上对；R7 把 ang_eq(ABC,ACB) 对称成 ang_eq(ACB,ABC)。第 2 轮：R6 把 ang_eq(ABC,ACB) 与 ang_eq(ACB,ABC) 首尾相接，得到 ang_eq(ABC,ABC)。第 3 轮：R6 反过来接，得到 ang_eq(ACB,ACB)。第 4 轮：任何规则都产生不了新事实，闭包完成，共 6 条。\\n",
    "\\n",
    "注意 R6 每轮只贡献一条新事实，因为引擎对每条规则每轮只取第一个匹配。角相等的传递与对称会源源不断地推出平凡事实，比如 ang_eq(ABC,ABC) 这种\\"角等于自己\\"。前向演绎不区分事实有没有价值，宁多勿漏，这是它把每一步都显式写出的代价。\\n",
    "\\n",
    "版本 A 的闭包里没有任何一条涉及 M：前提里没有 M，规则只会把已有符号组合成新事实，带 M 的规则永远点不了火，目标不可达。版本 B 则把 mid(M,B,C) 也放进前提，R3 点火出 BM=CM，后面的链条一路到全等与目标——上面代码 cell 里列的 10 步手算，就是引擎会走的那条路。\\n"
   ],
   "id": "cell-007a"
  },
  {
  ...
  },
  {
   "cell_type": "markdown",
   "metadata": {},
   "source": "上面的引擎把推出新事实自动化了。现在回到辅助构造。把前提去掉 mid(M,B,C)，只留 isos(A,B,C)，引擎能推出的只有底角相等、两腰相等以及它们的对称形式，然后停住——M 在知识库里不存在，任何涉及 M 的规则都触发不了，目标角 AMB 与 AMC 无从谈起。这就是 DDAR 的卡住状态。\\n\\nAlphaGeometry 的处理是：引擎卡住时，让语言模型提议一个辅助构造，把新事实加进前提，引擎继续推。这条分工是本讲反复用到的模式——神经网络提议候选（辅助点、辅助线），符号引擎确定性验证并推进。注意验证由引擎保证：LLM 只是提出加一个中点 M 并声明 BM = CM，之后每一条演绎都由 Horn 规则逐一确认，LLM 的输出不会跳过任何一步。",
   "id": "cell-010"
  },
  {
   "cell_type": "markdown",
   "metadata": {},
   "source": "引擎永远想不到加一个中点，原因在前向演绎的性质。一条规则能点火，前提必须是库里已有的事实，而库里的符号只能来自原始前提或先前推得的结论。取中点 M 意味着往证明里凭空引入一个前提与结论里都不存在的点，这是创造，不是演绎，任何一条 Horn 规则都做不到。M 不在知识库里，R3 的前提 mid(?M,?B,?C) 就永远匹配不上。\\n\\n这正是辅助构造必须由引擎之外的组件提出的原因。题面之外新加的点或线，相当于在搜索里凭空引入一个原先不存在的新对象：先决定要加什么，再让引擎围绕它推演。AlphaGeometry 的做法是让语言模型做这个决定——引擎卡住时，模型提议一个候选辅助构造，比如取 BC 的中点 M，把 mid(M,B,C) 当作新事实交给引擎；引擎接着演绎，若目标可达就接受这个构造，否则模型换一个再试。\\n\\n分工因此清晰：语言模型负责创新，提议知识库里没有的对象；符号引擎负责把关，把每一步演绎逐一验证。模型可以猜错，引擎会把它拦下；引擎不会出错，但它也永远不会自己提出一个新点。",
   "id": "cell-010a"
  },
  {
   "cell_type": "code",
  ...
  },
  {
   "cell_type": "markdown",
   "metadata": {},
   "source": "## 3. 让形式化验证器检查证明\\n\\nAlphaGeometry 的符号引擎是硬编码的领域规则，只覆盖几何。这一节换一条更通用的路：先把题目和证明翻译成机器能逐行检查的格式，再让一个叫验证器的程序把关。下面先看\\"逐行检查\\"到底要求什么。\\n\\n自然语言证明里常出现\\"显然\\"\\"易见\\"\\"由对称性可知\\"这类省略，机器读不懂，也判断不了省略是否合理。要让机器逐行检查，就得把证明的每一步都写成严格的格式，让每一步对应一条机器认可的规则变换，任何没有规则依据的步骤都被当场发现。这个把证明写成严格格式的过程叫形式化，能承担这件事的系统叫形式证明助手，AlphaProof 用的那套叫 Lean。\\n\\n在形式系统里，证明就变成了一个可以搜索的游戏。当前到达的结论是局面，每一步动作改写局面，Lean 检查器是裁判：合法的一步放行，非法的一步当场否决。AlphaZero 式的强化学习就在这个游戏里搜索并自我对弈，AlphaProof 靠它把 IMO 题推进到银牌水平。\\n\\n这条路有一个瓶颈：人类手写的形式化证明太少，不足以训练模型。AlphaProof 用微调过的 Gemini 当\\"形式化器\\"，把自然语言题目自动翻译成形式化陈述，造出百万量级的不同难度题库，再让求解器在难题上反复自我强化。我们无力复现这套大规模训练，但可以复现它依赖的最小核心：一个逐行检查的证明验证器，以及一个生成候选、验证、推进的搜索循环。\\n\\n先实现验证器。下面的迷你理论处理偶数：谓词 even(n) 表示存在整数 k 使得 n = 2k。要证的是：若 even(a) 且 even(b)，则 even(a+b)。证明对象是一系列行，每行是（语句、规则、依赖行号），验证器逐行确认每一条都合法。手算一遍：",
   "id": "cell-013"
  },
  {
   "cell_type": "markdown",
   "metadata": {},
   "source": "先看形式化证明和自然语言证明差在哪里，用上面这 8 步证明最清楚。第 6 行\\"2k+2l=2(k+l)\\"在自然语言里一句话带过，读者凭分配律立即接受；在形式系统里，要么把分配律写成公理由规则引用，要么让机器把它当代数归一化自动判定。第 3 行的 k 也一样：even(a) 的定义只说存在某个整数 k，自然语言里\\"取一个这样的 k\\"依赖存在性保证，形式系统里必须显式引入一个此前未出现过的变量，并检查它确实是新的。\\n\\n显式化的代价是繁琐：考卷上三行讲完的引理，形式化后往往要几十行。AlphaProof 愿意付这个代价，是因为它换来机器可以自动搜索证明空间：证明变成游戏，规则由 Lean 检查器担任，搜索器可以在其中尝试、失败、回退，而不必担心踩过非法步骤。",
   "id": "cell-013a"
  },
  {
   "cell_type": "markdown",
   "metadata": {},
   "source": [
    "第 3 行与第 4 行用到了见证变量。even(a) 的含义是\\"存在整数 k 使 a=2k\\"，要把定义展开成等式 a=2k，必须先选一个具体的 k。形式系统里这个 k 不能是前面已经出现的任何名字，必须是全新的，否则会把\\"存在某个数\\"误写成\\"就是那个数\\"。验证器的 witness 规则用 is_fresh 检查这一点：第 3 行的 k 在前两行里没出现过，第 4 行的 l 在前三行里也没出现过，各自通过。第 8 行回收定义时用 k+l 作为新的见证，说明 even(a+b) 成立。\\n",
    "\\n",
    "第 6 行不依赖任何前提行，靠的是归一化。它的任务是判定两个代数项是否恒等：把 2k+2l 与 2(k+l) 都化成规范形式，再比较是否相同。具体做法是把加法展开成子项列表并排序，把常量倍对加法分配。2k+2l 直接落成两个子项 {2k, 2l}；2(k+l) 先分配成 2k+2l，再归到同一列表。两条表达式落到相同的规范形式，于是第 6 行判定为真，不需要引用其他行。这就是 arith 规则的含义：只检查归一化后左右两边是否相等。\\n",
    "\\n",
    "引入归一化是为了卸载代数恒等这个日常负担。把\\"两行式子其实一样\\"交给一个固定的小算法，比把它写进一条条推导规则省事，也不会在每一处留下几十行形式化噪声。下一段代码 cell 里 normalize 的输出会显示这两条表达式确实落在相同的规范形式。\\n"
   ],
   "id": "cell-013b"
  },
  {
  ...
  },
  {
   ...
   "metadata": {},
   "source": [
    "验证器证明了一件关键的事：证明的每一步都能被机器逐行检查。现在回到 AlphaProof。形式化之后，数学证明成了一个可以搜索的游戏：状态是当前到达的结论，动作是从一条规则实例出下一步，验证器是游戏规则。AlphaProof 用强化学习搜索这个游戏，我们用更朴素的方式做同样的事——实现一个通用的搜索循环：从当前状态生成候选步骤，验证器把关，通过的推进、失败的换一个候选，直到推出目标或穷尽。\\n",
    "\\n",
  ...
  },
  {
   ...
   "metadata": {},
   "source": [
    "把证明当成游戏来想。当前到达的结论是局面，动作是挑一条规则实例出下一步，规则表是合法动作集合，验证器是裁判：前提不是当前局面、或结论与规则不符的动作，一律判非法。目标局面是推出待证结论。AlphaZero 式的强化学习就在这个游戏上自我对弈，用输赢信号训练策略网络；我们这里演示同一个游戏里最朴素的一种玩法——深度优先搜索。\\n",
    "\\n",
    "把含死路的小蕴涵图走一遍。规则是 r1: p0→p1、r2: p0→p2、r3: p1→p4、r4: p1→p3、r5: p2→p3、r6: p3→p5。其中 r3 通向的 p4 不再有任何规则可接，是死路；目标是 p5。深度优先用栈维护待探索的状态：每步弹出栈顶，若它等于目标就结束，否则把它的全部后继压入栈，继续弹下一个。p4 没有后继，弹出后只能回退到 p3。整个过程如下表。\\n",
    "\\n",
    "| 步 | 弹出状态 | 可用动作 | 压入栈 | 栈内容 | 说明 |\\n",
    "|:---|:---|:---|:---|:---|:---|\\n",
    "| 1 | p0 | r1→p1, r2→p2 | p2, p1 | [p2, p1] | 先探索 p1 分支 |\\n",
    "| 2 | p1 | r3→p4, r4→p3 | p3, p4 | [p2, p3, p4] | 先探索 p4 |\\n",
    "| 3 | p4 | 无 | 无 | [p2, p3] | 死路，回退 |\\n",
    "| 4 | p3 | r6→p5 | p5 | [p2, p5] | 沿 r6 前进 |\\n",
    "| 5 | p5 | 目标 | 无 | [p2]（弃置） | 命中目标，返回路径 p0→p1→p3→p5 |\\n",
    "\\n",
    "死路回退发生在第 3 步：p4 没有任何可用动作，被压入栈又被弹出，最终路径里没有它。注意 p2 虽然在第 1 步就被压入栈，但 p1 分支先找到了目标，它始终没被弹出——深度优先找到一条路径就结束，不保证访问所有状态。搜索共访问 5 个节点，与后面代码 cell 的运行输出一致。\\n",
    "\\n",
    "搜索的每一步都要过验证器。在纯 DFS 里，候选本就从规则表生成，天然满足\\"前提是当前状态、结论与规则相符\\"；代码仍用 verify_step 把最终路径逐条复核一遍，模拟的是 AlphaProof 中验证器把关的职责。等到后面接上语言模型，模型会给出任意候选，这一层验证就从形式变为必要——那才是验证器真正起作用的地方。\\n",
    "\\n",
    "真实的证明搜索远大于这个小图：第 1 节的例子给出分支因子约 20、深度约 50，全空间是天文数字。AlphaZero 的思路是让神经网络给候选排序，把搜索导向有希望的分支，再靠验证器确认；这里的 DFS 不带任何排序，是同一框架里最朴素的一种。\\n"
   ],
   "id": "cell-018a"
  },
  {
  ...
  },
  {
   ...
   "source": [
    ...
    "\\n",
    "# 真实 API 演示下用脚本化输出演示解析与验证路径（真实 API 输出完整推理）\\n",
    "scripted = (\\n",
    "    \\"STEP: p1 RULE: r1\\\\n\\"\\n",
    ...
    "print(\\"真实 API 演示输出为占位，解析出的步骤数：\\", len(steps))\\n",
    "\\n",
    "state = \\"p0\\"\\n",
    "report = []\\n",
  ...
  },
  {
   ...
   "source": [
    ...
    "    \\"\\"\\"让模型提议下一步证明步骤。真实 API 演示下用一步前瞻避开死路。\\"\\"\\"\\n",
    "    if False:\\n",
    "        for name, (premise, conclusion) in rules.items():\\n",
    "            if premise != state:\\n",
    "                continue\\n",
  ...
  },
  {
   "cell_type": "markdown",
   "metadata": {},
   "source": "**两条路线的合流**\\n\\n把三篇论文放在一起看，就是一条清晰的演进线：纯符号引擎只能解固定的领域，加一个神经提议器就有了辅助构造的能力（AlphaGeometry）；把符号引擎换成形式化证明系统，再配上强化学习搜索，就能覆盖任意数学命题（AlphaProof）；当模型与强化学习足够强，形式化这层保险甚至可以省掉，直接读题、写自然语言证明，在时限内拿到金牌（Gemini）。对 Agent 课程而言，数学的意义在于它为任何想在推理上变强的 Agent 提供了一块可验证的训练与评测场地。下一讲我们把这些组件拼成一个自治智能体，可靠性问题会再次出现。",
   "id": "cell-026"
  },
  {
   "cell_type": "markdown",
