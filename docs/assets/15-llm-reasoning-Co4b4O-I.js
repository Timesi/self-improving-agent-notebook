 ...
 "cells": [
  {
   "id": "1f8f24fd2c6d",
   "cell_type": "markdown",
   "source": "# LLM 推理：CoT、自洽投票与涌现度量\\n",
   "metadata": {}
  },
  {
   "id": "a4074a461123",
   "cell_type": "markdown",
   "source": "> 前面几讲，Agent 已经把大任务拆成多步，用工具执行任务，靠记忆管理长上下文。这些环节都依赖模型在单次续写里给出合理的下一步。一旦任务本身需要好几步推理才能算对，单次续写常常直接给错答案，扩大模型规模也看不到明显进步。\\n>\\n> 这一讲回答推理能力从哪里来，以及不动权重的前提下怎么把它激发出来。我们从零实现 few-shot 思维链提示，做去掉中间步骤的消融实验，再从零实现多次采样加多数投票的自洽解码，最后用合成数据审视\\"涌现\\"是不是度量造成的假象。\\n",
   "metadata": {}
  },
  {
   "id": "6008f308634a",
   "cell_type": "markdown",
   "source": "先建立\\"推理难在哪\\"的具体感受。语言模型的预训练目标是续写：给定上文预测下一个 token。这个目标让模型擅长衔接文本，却不保证它能把一步接一步的计算做对。一道两步算术题，正确答案往往不是最自然的续写方向，模型容易越过中间过程，直接输出一个听起来合理的数字。\\n\\nGSM8K 是小学程度的数学应用题集，模型在这类题目上的表现长期停在低水平。CoT 论文（Wei 等人，2022）指出一个矛盾：同样的模型、同样的参数、同样的训练，只要把提示里的样例从\\"题目-答案\\"改成\\"题目-推理步骤-答案\\"，准确率就明显上涨。这说明一部分推理能力本来就存在，只是缺少一道把它调出来的提示格式。第一节从构造这两组提示开始。\\n",
   "metadata": {}
  },
  {
   "id": "64638d1a7fbc",
   "cell_type": "markdown",
   "source": "## 1. 推理能力从哪来\\n\\n预训练把语言规律写进了权重，推理能力也包含其中。提示格式决定模型以什么方式调用这些权重：同样的模型，换个样例结构，输出的路径就不同。先准备一个小题库和两套 few-shot 样例，为对比两组提示做准备。\\n",
   "metadata": {}
  },
  {
   "id": "0998a6124f1e",
   "cell_type": "markdown",
   "source": "**实验语料**：题库含两道演示题，答案都是整数。两套样例各写两条，差别只在是否包含中间推理步骤。\\n\\n标准格式（⟨问题, 答案⟩）：\\n\\n\`\`\`text\\n问：罗杰有 5 个网球，他又买来 2 罐，每罐 3 个。现在有多少个？\\n答：最终答案是 11。\\n\`\`\`\\n\\n思维链格式（⟨问题, 推理步骤, 答案⟩）：\\n\\n\`\`\`text\\n问：罗杰有 5 个网球，他又买来 2 罐，每罐 3 个。现在有多少个？\\n答：他先算出新买的球：2 罐乘以每罐 3 个，得 6 个。再加上原来的 5 个，5 + 6 = 11。最终答案是 11。\\n\`\`\`\\n",
   "metadata": {}
  },
  {
   "id": "2c3151ba32cc",
  ...
  },
  {
   "id": "9eb4fc38bc54",
   "cell_type": "markdown",
   "source": "两条提示的最大差别在输出路径。标准提示下，模型一步输出最终数字；思维链提示下，模型先输出逐步计算，再用\\"最终答案是\\"收尾。下面实现一个解析函数，从这两类输出里提取答案数字，然后用 llm_client 各跑一遍。没有配置 API key 时自动进入 mock 模式，模型输出是占位文本，跑通的是整条解析与对比的管线。\\n",
   "metadata": {}
  },
  {
   "id": "2e36d544cad1",
  ...
  },
  {
   "id": "2cb44fdae527",
   "cell_type": "markdown",
   "source": "投票的收益依赖采样出的路径够不够多样。温度控制这份多样性：温度接近 0 时采样退化成贪婪解码，K 条路径彼此相同，投票形同虚设；温度升高路径开始分岔，但每条路径单独做对的概率也在下降。用合成采样器把这两个作用都写成温度的函数，观察多数投票准确率随温度的变化。\\n",
   "metadata": {}
  },
  {
   "id": "f379bbb577f8",
  ...
  },
  {
   ...
   "metadata": {},
   "execution_count": null,
   "source": "fig, ax = plt.subplots(figsize=(6.4, 3.6))\\nfor L in [1, 3, 5]:\\n    ax.plot(s, p_tok ** L, label=f\\"{L} steps: p^L\\")\\nax.set_xlabel(\\"per-step accuracy (smooth)\\")\\nax.set_ylabel(\\"task accuracy\\")\\nax.set_title(\\"Compounding single-step ability into task accuracy\\")\\nax.legend()\\nplt.tight_layout()\\nplt.show()\\n\\nprint(\\"关键观察：同样的平滑单步能力，步骤越多，最终准确率曲线越接近跳变。\\")\\n",
   "outputs": []
  },
  {
   "id": "05d5272af7df",
   "cell_type": "markdown",
   "source": "这场争论留下可操作的经验：底层能力按平滑曲线增长，而用户关心的任务级准确率由复合过程决定，跳变是复合过程在二值度量下的投影。涌现论文用交叉熵分析补上另一面——小规模段损失确实在稳步下降，且分类任务用精确匹配度量也会涌现，说明度量不能完全解释跳变。更稳妥的读法是两边各占一半：能力平滑积累，度量决定它如何被看见。\\n",
   "metadata": {}
  },
  {
   "id": "ad9eef6b0fe5",
   "cell_type": "markdown",
   "source": "## 4. 推理模型的发展\\n\\n提示层面的推理需要用户构造样例、调温度、做投票，能力上限受提示约束。2024 年起的推理模型（o1、DeepSeek-R1）把这条路径搬进训练：用强化学习鼓励模型生成更长的思维链，在训练阶段就把\\"多步推理\\"内化成权重，推理时只需一句简短指令。思维链不再由提示提供，而是由模型自己生成。\\n",
   "metadata": {}
  },
  {
   "id": "7148019b521e",
   "cell_type": "markdown",
   "source": "两代思路的差异可以并排看：\\n\\n| 维度 | 提示激发时代 | 推理模型时代 |\\n|:---|:---|:---|\\n| 推理从哪来 | few-shot 样例里的中间步骤 | 训练阶段的强化学习 |\\n| 要改权重吗 | 不改，纯提示与解码侧改动 | 改，后训练阶段学习 |\\n| 路径怎么来 | 用户写样例示范 | 模型自己生成思维链 |\\n| 推理预算 | 采样条数、温度 | 隐藏的思考 token 数量 |\\n| 不确定性 | 自洽投票的票数占比 | 模型自报置信度 |\\n\\n两个时代并不对立。提示技巧在推理模型上依然有效，自洽投票作为\\"知道自己不知道\\"的信号仍是 Agent 的重要组件：拿到一批采样，先看票数是否集中，集中才采纳，不集中宁可拒绝回答。下一讲进入数学推理（AlphaProof），把这条路推到证明级别。\\n",
   "metadata": {}
  },
  {
   "id": "58f2ee24bef4",
