const n=`{
 "cells": [
  ...
  },
  {
   "cell_type": "markdown",
   "metadata": {},
   "source": "> 上一讲我们沿着推理模型的思路，研究了怎样让 LLM 在生成答案时多花一步思考：思维链、自洽性、测试时计算。这些方法让推理变长，却没有正面回答一个更根本的问题——当任务要求绝对正确、半点含糊都不行时，模型的推理需要达到怎样的可靠程度。\\n>\\n> 这一讲把这个问题放到最苛刻的场景：国际数学奥林匹克（IMO）。我们沿着三条技术路线走一遍——AlphaGeometry 用神经-符号结合解几何题，AlphaProof 把证明变成强化学习可搜索的游戏，Gemini Deep Think 用纯自然语言在比赛时限内拿到官方认证的金牌。每一路我们都亲手实现其中的最小组件：一个迷你几何符号引擎、一个逐行检查的证明验证器，以及一个生成候选、验证、推进的搜索循环。",
   "id": "cell-001"
  },
  {
   "cell_type": "markdown",
   "metadata": {},
   "source": "数学竞赛题成为推理试金石的原因有三条。可验证：一道题的对错有客观判据，证明过程要么成立要么不成立，不能靠看起来合理蒙混过关。奖励稀疏：一道 IMO 题满分 7 分，但没有渐进分——证出来就是 7 分，差一步就是 0 分，中间没有任何信号告诉模型它接近了。需要搜索与工具：竞赛题的证明往往有几十步，单次生成几乎必然在某一步卡住，必须在巨大的证明空间里搜索，还要能提出新的辅助构造、借助形式化验证器把关。\\n\\n这三条合起来，让数学与开放对话形成鲜明对比。对话任务里，一段错误但流畅的回答常被当作大致正确；数学任务里，这种含糊会被一个绝对公正、永不犯错的裁判当场判零分。本讲先看这条判分规则带来的后果，再从几何到数论，逐一搭建求解它的组件。",
   "id": "cell-002"
  },
  {
   "cell_type": "markdown",
