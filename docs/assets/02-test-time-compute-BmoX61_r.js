const n=`{
 "cells": [
  ...
  },
  {
   ...
   "id": "md-02",
   "metadata": {},
   "source": "> 第 1 讲里，Agent 是\\"一次调用之外还能做更多计算\\"的结构，那只是一个定性的说法。这一讲把它变成一条可以测量的轴：模型训练完之后不再更新权重，只在推理阶段多花算力，能力还能提升多少。\\n>\\n> 我们从最简单的\\"同一道题多答几次\\"开始，从零实现 pass@k 估计、多数投票和 best-of-n，再看给定一笔固定的算力预算时，怎么按题目难度分配收益最大，最后把多种方法组合成分层系统，用自动搜索找出最好的组合。全部实验用合成数据，离线可跑。"
  },
  {
   "cell_type": "markdown",
   "id": "md-03",
   "metadata": {},
   "source": "你可能已经注意到，现在的模型会\\"想很久\\"。OpenAI o1、DeepSeek-R1 这类模型，训练完之后权重就固定了，但回答难题时，它们会在内部多试几遍、自我检查，再给出答案。它们比普通模型强，靠的不是重新训练，而是在回答问题时花了更多算力去思考。\\n\\n先看一个最小的例子，体会\\"不动权重也能变强\\"。假设某道题，模型单次作答答对的概率是 0.3。如果让它独立尝试 k 次，至少有一次答对的概率是 $1 - (1 - 0.3)^k$。k 取 1、2、5、10 时，这个概率分别是 0.30、0.51、0.83、0.97。同一个模型，只是多答了几次，一道题的通过概率就从 0.3 升到接近 1。\\n\\n这种训练结束后再花算力的方式，叫 test-time compute，重复采样、自我修订、候选验证、多数投票都属于它。这一讲要回答两个问题：多花算力到底能换来多少准确率；给定一笔固定算力，怎么分配收益最大。第一节先建立一个合成基准，把\\"一道题能否被解出\\"变成可以测量的数量。"
  },
  {
   "cell_type": "markdown",
   "id": "md-04",
  ...
  },
  {
   ...
   "id": "md-05",
   "metadata": {},
   "source": "这一节先给\\"花算力\\"一个统一的框架，再搭一个贯穿全讲的实验环境。\\n\\n模型在推理阶段可以做的选择，Snell 等人的工作把它归成两个旋钮。第一个旋钮决定\\"生成什么样的候选答案\\"：让模型多试几次，还是让它在草稿上自我修订，都改这里，论文里叫提议分布（proposer）。第二个旋钮决定\\"拿到一批候选后怎么挑\\"：挑得分最高的，还是多数投票，都改这里，叫输出后处理（verifier）。后面几节的每个实验，都是这两个旋钮的某种组合。先记住这个框架，具体用法后面逐个展开。\\n\\n实验需要一个可以反复使用的题目集。真实题目没法控制难度，所以这一讲用合成数据：模拟一批数学应用题，每道题给一个\\"单次正确率\\"，记为 p@1。p@1 表示\\"这道题一次作答就答对的概率\\"：p@1 高，题容易；p@1 低，题难。题目有难有易，大多数题 p@1 很低，只有少数题 p@1 高。这种\\"多数难、少数易\\"的分布叫重尾分布。之后的每个实验，都以这批题目和它们的 p@1 为输入。先手动算一遍\\"多答几次\\"的收益，再生成这批题目。"
  },
  {
   "cell_type": "code",
  ...
  },
  {
   ...
   "id": "md-09",
   "metadata": {},
   "source": "第 1 节留了一个问题：多花算力到底提升多少？论文 Large Language Monkeys 给出了一个直接的回答：重复采样能大幅提高覆盖率。\\n\\n先看\\"重复采样\\"是什么意思。同一道题，模型生成 k 个候选答案，再从这 k 个里挑一个，这就是 pass@k——\\"采样 k 次后的通过率\\"。k 越大，花在推理上的算力越多。挑的时候需要一个选择标准，这个标准引出一对容易混淆的量。覆盖率（coverage）问\\"模型到底能不能解出这道题\\"，看的是 k 个候选里有没有正确的；精度（precision）问\\"选择器挑得准不准\\"，看的是挑出来的那个是不是正确的。一个衡量\\"生成\\"端，一个衡量\\"挑选\\"端，后两节会反复用到这两个量。\\n\\n论文给出的数字很具体。SWE-bench Lite 上，DeepSeek-Coder-V2-Instruct 单次只解出 15.9% 的真实 GitHub issue，采 250 次后提升到 56%。Gemma-2B 在 CodeContests 上 pass@1 只有 0.02%，采 1 万次后 pass@10k 提升到 7.1%，提高了 300 倍。采样次数能稳定换取覆盖率，而且收益的形态有规律。先处理一个测量上的细节，再看规律。"
  },
  {
   "cell_type": "markdown",
   "id": "md-cov-prec",
  ...
  },
  {
   ...
   "id": "md-10",
   "metadata": {},
   "source": "现在要处理\\"采样 k 次\\"里的一个测量细节。假设我们每道题真的生成 N 个样本，其中 C 个正确。直接数\\"有多少题在 N 个样本里出现过正确\\"，会系统性高估 pass@k——因为我们把 N 个样本全用上了，回答的其实是 pass@N。Chen 等人给出一个无偏估计：\\n\\n$$\\\\mathrm{pass@k} = \\\\frac{1}{P}\\\\sum_{i=1}^{P}\\\\left[1 - \\\\frac{\\\\binom{N-C_i}{k}}{\\\\binom{N}{k}}\\\\right]$$\\n\\n先看单个括号里的意思：从 N 个样本里选 k 个，一个正确的都没选到的比例是多少，用 1 减掉，就是\\"至少选到一个正确的\\"概率。分子就是从 N 个样本里选 k 个、一个正确的都没选到的组合数。再手算一个例子。某道题采样 N = 10 个样本，其中 C = 3 个正确。k = 1 时，$\\\\binom{7}{1}/\\\\binom{10}{1} = 0.7$，pass@1 = 0.3，等于 C/N，符合直觉；k = 5 时，$\\\\binom{7}{5}/\\\\binom{10}{5} = 21/252 \\\\approx 0.083$，pass@5 ≈ 0.917；k = 10 时 $\\\\binom{7}{10} = 0$，pass@10 = 1。这三个值留给下面的代码验证。"
  },
  {
   "cell_type": "markdown",
   "id": "md-passk-why",
  ...
  },
  {
   ...
   "id": "md-13",
   "metadata": {},
   "source": "多花算力换覆盖率，收益的形态值得研究：开头涨得快，后面越来越慢，还是从头到尾匀速？把合成基准的覆盖率画成 k 的函数，会得到一条典型的推理期缩放曲线。论文里 Llama-3-8B-Instruct 在 MATH 上的覆盖率先从 100 样本的 82.9% 提升到 10000 样本的 98.44%，拟合出 $\\\\text{coverage} = \\\\exp(a\\\\,k^b)$，其中 a = -1.33、b = -0.43。我们在合成基准上复现这条曲线，并用最小二乘拟合同一个形式。"
  },
  {
   "cell_type": "markdown",
   "id": "md-powerlaw",
  ...
  },
  {
   ...
   "id": "md-15",
   "metadata": {},
   "source": "拟合出的幂律 $\\\\exp(a\\\\,k^b)$ 有一个出人意料的来源。单独看一道题，失败率是 $(1-p_i)^k$，随 k 指数衰减——k 每增加一个固定量，失败率就缩小一个固定比例。但把整个基准合起来看，聚合失败率只按幂律缓慢下降。同一个采样过程，单题指数衰减与聚合幂律并存，原因在 p@1 的分布：题目之间呈重尾时，大多数题 p@1 很低，这些\\"几乎解不出\\"的题把整体曲线拖成了幂律。Monkey Power Laws 一文的定理 3.1 证明，如果 p@1 的密度在 0 附近像 $C\\\\,p^{b-1}$ 那样发散，聚合失败率就按 $k^{-b}$ 缩放；反过来，观测到聚合幂律，也说明 p@1 分布有这种重左尾。我们同时画出单题与聚合两条失败率曲线，再移除重尾分布，看幂律是否消失。"
  },
  {
   "cell_type": "markdown",
   "id": "md-heavytail",
  ...
  },
  {
   ...
   "id": "md-18",
   "metadata": {},
   "source": "重复采样只能保证\\"候选里至少有一个正确\\"，最终答对多少，还取决于从候选里怎么挑——这一步看精度（precision）。最简单的选择器有两个。\\n\\n第一个是多数投票：对每个问题生成 k 个候选，取出现次数最多的答案，这是 self-consistency（Wang 等人）的做法。多数投票不需要验证器，但它要求正确解成为众数；自由形式答案几乎不重合时，正确解至少要出现两次。第二个是 best-of-n：给每个候选打分，取分数最高的。Oracle 验证器永远挑得到正确解，真实验证器会有噪声。\\n\\n论文里有一个反直觉的数字：MATH 上覆盖率提升到 95% 以上，多数投票却只从 40.50% 提升到 41.41%，reward model 加 best-of-n 也在约 100 个样本处封顶。覆盖率与真实成功率之间的 gap 随样本数扩大。我们用模拟重现这三条曲线，看差距从哪来：候选里有多类错误答案，错误类的扎堆会带偏多数投票。"
  },
  {
   "cell_type": "markdown",
   "id": "md-majority",
  ...
  },
  {
   ...
   "id": "md-22",
   "metadata": {},
   "source": "前面几节，所有题目共用同一种花法。但同一笔算力预算，花在不同难度的题上，效果不一样：容易的题，给个草稿再修一修就能做对；难的题，修多少次都可能在原地打转，不如多试几次，让多个独立候选互相竞争。把预算按题目难度分配，就是 compute-optimal 缩放，论文报告同等精度下它比均匀 best-of-n 少花约 4 倍算力。\\n\\n难度的划分需要一个可操作的标准。论文用 base LLM 对每道题采 2048 个样本估出 pass@1，按 5 分位数切成 5 个难度档。我们沿用这个做法：把合成基准按 $p_i$ 升序排好，均分成 5 档，档 1 最难、档 5 最易。我们用一套简化的修订模型模拟\\"并行条数 × 顺序步数\\"的预算拆分，观察各档的最优拆分方式。"
  },
  {
   "cell_type": "markdown",
   "id": "md-budgetsplit",
  ...
  },
  {
   ...
   "id": "md-25",
   "metadata": {},
   "source": "前面算的都是\\"同一笔推理预算怎么花\\"。还有一个更根本的问题：这笔算力如果拿去训练一个更大的模型，会不会更值？Snell 的答案是，取决于推理负载比例 $R = D_{infer}/D_{train}$。训练算力约 $6ND_{train}$，推理算力约 $2ND_{infer}$。把模型参数放大 M 倍，总算力变成 M 倍的训练加推理；用小模型加 test-time compute 匹配这笔总预算时，可用的采样次数变为\\n\\n$$S = M + 3\\\\,\\\\frac{D_{train}}{D_{infer}}(M-1)$$\\n\\n取 M = 14，R 取 0.16（自举式训练，推理负载低）、0.79（典型负载）、22（大规模部署，推理负载高）时，S 分别约 258、63、16。我们画出小模型的采样曲线，把\\"14 倍大模型贪心解码\\"的精度当作一条水平线，看三个 R 下的可用样本数能把小模型推到线上方还是下方。"
  },
  {
   "cell_type": "markdown",
   "id": "md-flops",
  ...
  },
  {
   ...
   "id": "md-28",
   "metadata": {},
   "source": "前面几节的方法各有适用场景：重复采样对覆盖率有效，顺序修订对易题有效，best-of-n 依赖验证器。没有一个方法能通吃所有任务，这是 Archon 论文的出发点：把各种技术组合成分层的 LLM 系统，再用自动搜索找出最优组合。系统的每个组件都是 text-to-text 操作，没有可训练的权重：Generator 生成候选，Fuser 把多个候选合并，Ranker 两两比较排序，Critic 先列优缺点再交给排序与融合，Verifier 先给推理再给判定，Unit-Test 生成器产出测试语句并打分。\\n\\n结构有若干硬规则：Generator 只能放在第一层，每层只放一类组件，Critic 必须在 Ranker 或 Fuser 之前，最后一层输出第一个字符串。去掉无效配置后，搜索空间有 9576 个配置，论文用贝叶斯优化在约两成数据上搜索，最佳架构平均超过当时 frontier 模型 15.1%。我们在一个 96 配置的简化空间里，从零实现网格搜索与随机搜索，观察最优架构是否随任务变化。"
  },
  {
   "cell_type": "markdown",
   "id": "md-archon",
