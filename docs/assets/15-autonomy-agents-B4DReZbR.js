const n=`{
 "cells": [
  ...
  },
  {
   ...
   "id": "cell_002",
   "metadata": {},
   "source": "> 前几讲分别给出了验证器、工具执行、记忆与评测，每一环都能在演示任务上跑通。但\\"能做演示\\"和\\"能在无人看管下连续运行数小时\\"是两回事，中间隔着可靠性、监督与信任三个问题。\\n>\\n> 这一讲把这些能力装进一个整体，直面自治背后的工程与开放问题。我们从单步正确率的乘法效应出发，依次搭建错误检测与重试循环、检查点恢复、阈值化接管，最后用一个最小的可信监控协议，回应\\"人追不上模型之后如何把关\\"的问题。"
  },
  {
   "cell_type": "markdown",
   "id": "cell_003",
   "metadata": {},
   "source": "你可能见过 Agent 的演示视频：给它一个任务，比如\\"去订一张机票\\"，它在屏幕上一步步点开网页、填表、确认，最后顺利完成。视频到这里就结束了。真实环境里不是这样。把同一个 Agent 放出去连续跑几个小时，它会在某一步填错表单、碰上网页改版，或者中途断网。短任务里这没什么，错了重跑一遍，几秒钟的事；长任务里不行，跑了两个小时在第 100 步出错，从头再来等于白跑。\\n\\n先想清楚它为什么会这样。一个长任务被拆成很多步，Agent 一步一步做，任何一步出错，后面的步骤全都白做。所以\\"能完成演示\\"和\\"能在没人看着的情况下把活干完\\"是两回事，后者要求系统具备自治（autonomy）能力：自己连续运行，出错能自己发现、能自己恢复。但自治和可靠是矛盾的：让 agent 放手去做，就必然要接受它会在某个时刻出错。这个矛盾没有一劳永逸的解法，只能靠一套机制在出错时兜住。\\n\\n这一讲放在第 1 讲的循环框架里，补的是\\"反馈\\"这一步之后的能力。第 1 讲的循环是感知、决策、行动、反馈四步，默认反馈是准的：工具返回什么，就信什么。真实环境里反馈不可靠——工具结果可能本身就是错的，进程也可能执行到一半崩溃。所以这一讲要给循环加上错误检测、重试、检查点恢复，以及自己兜不住时请人接手的机制。学完这一讲，你能亲手实现一套带这些机制的最小自治循环，并说清\\"让 Agent 自己干活\\"难在哪、风险在哪。\\n\\n可靠性、监督、信任是三个不同的词，先拆开。可靠性（reliability）说系统自己能不能跑稳：单步做对、循环跑稳、出错能恢复，是三件不同的事。监督（supervision）说我们怎么知道它做对了：看最终结果，还是看每一步。信任（trust）说我们愿意把多大权力交给它：哪些动作让它自己定，哪些必须由人来把关。下面四节按这三件事展开。\\n\\n先澄清一个容易混淆的名字。Laskin 创立的公司叫 Reflection AI，与开源模型 Reflection 70B（HyperWrite 与 Glaive 于 2024 年发布）是两个不同实体；本讲提到 Reflection 都指前者。\\n\\n可靠性问题先从最朴素的一层讲起：单步正确率如何决定整条任务的成败。"
  },
  {
   "cell_type": "markdown",
   "id": "cell_004",
   "metadata": {},
   "source": "## 1. 从演示到自治\\n\\n这一节解决一个问题：演示能跑通的任务，为什么一放长就会掉链子。我们把\\"演示任务\\"和\\"自治任务\\"的差别先说清，再算算长任务对可靠性的要求有多高。\\n\\n演示任务的共同点是短：几步之内就能拿到一次正确结果，错了重跑一遍，成本也低。前几讲的演示大多在十步以内。自治任务要求系统在一长串步骤里不犯错，或者错了能自己发现并补救。无人看管地连续运行，可能是几千步。\\n\\n步骤数从十步涨到几千步，对可靠性的要求也随之提高几个数量级。先从最朴素的一层看起：单步正确率如何决定整条任务的成败。"
  },
  {
   "cell_type": "code",
  ...
  },
  {
   ...
   "id": "cell_006",
   "metadata": {},
   "source": "### 单步正确率是乘法因子\\n\\n这一小节回答一个问题：单步正确率每低一点，长任务的成功率掉得有多快。我们先看正确率为什么是乘在一起，再算几个具体数字。\\n\\n先想清楚为什么是乘法。一个任务要拆成先后依赖的步骤，第一步做对了才能做第二步，任何一步失败，整条任务就失败。两件独立事件同时发生的概率，是各自概率的乘积：两个骰子同时掷出 6 的概率是 (1/6)×(1/6)=1/36。假设 agent 把任务拆成 n 个先后依赖的步骤，每一步独立成功的概率都是 p。整条任务成功的条件是每一步都成功，所以整体成功率等于 p 连乘 n 次，也就是 p 的 n 次方。\\n\\n用最小的数字例子走一遍。单步正确率 0.9，两步任务：第一步成功概率 0.9；在这 0.9 里，第二步再成功 0.9，整体是 0.9×0.9=0.81。每一步都会把\\"还剩下的成功率\\"再压掉 10%。多走几步，成功率这样掉：\\n\\n| 步数 n | 整体成功率 0.9^n | 直观感受 |\\n|:---|:---|:---|\\n| 1 | 0.900 | 九成 |\\n| 2 | 0.810 | 八成 |\\n| 5 | 0.590 | 不到六成 |\\n| 10 | 0.349 | 约三分之一 |\\n\\n两步都 90% 的任务只剩 81%，五步就掉到 59%，十步只剩 35%。把单步提高到 0.99，10 步仍有 0.99^10 ≈ 0.90，但 100 步也只有 0.99^100 ≈ 0.37。\\n\\n这里得出一个重要的结论。自治是乘法，不是加法：从\\"能做演示\\"到\\"无人看管跑几小时\\"，不能只靠每步更稳一点。步数一多，单靠提高单步正确率很快就到头了，必须在出错后能自己发现并补救。这正是下一节要做的。"
  },
  {
   "cell_type": "code",
   "execution_count": null,
  ...
  },
  {
   ...
   "id": "cell_008",
   "metadata": {},
   "source": "## 2. 可靠性：错误检测与恢复\\n\\n上一节算出了单步正确率的乘法效应，这一节解决接下来的问题：单步正确率不够高时，靠什么机制把整体成功率救回来。答案是让循环在出错时能自己发现、自己补救，这一层的能力叫可靠性（reliability）。\\n\\n可靠性不是单一性质，而是三层：单步做对、循环跑稳、出错后能恢复。每一层有独立的失败模式和解法。用一个 agent 计算\\"1 到 5 的和\\"作例子，把三层分开看：\\n\\n- 单步做对：某一步输出错误结果。例如 agent 把 2+3 算成了 6。这一层由模型能力与单步验证决定，对应上一节的乘法因子。\\n- 循环跑稳：已经产生的状态被错误污染后一路向后传播。例如累计和记成了 6，之后每一步都从 6 开始，全错。这一层靠状态卫生与检查点保护。\\n- 长程纠错：某一步输出了错值，但没人发现、直接推进。例如第 3 步算错一次，之后每步都正常，最终结果却整体偏了。这一层靠自检与重试循环。\\n\\n三层会叠加：第一步出错、错误又没被发现、还写进了状态，就同时踩中三层的坑。下面先看第一层与第二层的交界处，长任务里的错误是如何累积的。"
  },
  {
   "cell_type": "markdown",
   "id": "cell_009",
   "metadata": {},
   "source": "### 乘法塌陷：错误随位数累积\\n\\n这一小节回答一个问题：在一条真实的长任务里，错误是怎么一步步把整件事带偏的。我们用竖式乘法作例子，因为它能清楚地展示\\"每一步看起来都对、结果却错\\"。\\n\\n先看一个具体的长乘法：23 × 47，拆成三个子步骤：\\n\\n\`\`\`text\\n  23 × 7 = 161        (个位部分积)\\n  23 × 4 = 92 → 920   (十位部分积，左移一位)\\n  161 + 920 = 1081    (加总，得真值)\\n\`\`\`\\n\\n每个子步骤都可能出错：7×3 记成 18、进位进错、加总对错位。位数越多，这样的子步骤越多。\\n\\n关键是出错没有信号。如果十位部分积算成 91 → 910，最终是 161 + 910 = 1071。1071 是一个完全正常的数，每个数字都合法，没有任何地方写着\\"我算错了\\"，可它确实错了。这就是\\"看起来每一步都对、结果却错\\"：最终答案是一长串子步骤的合取，任何一处出错，整串作废，而错误通常不会自己报出来。\\n\\n现在给\\"每一步都对、结果却错\\"一个具体数字。假设模型解一个含 15 个依赖步骤的乘法，每步正确率 0.9。整次成功的概率是 0.9^15。一步步压下来：\\n\\n\`\`\`text\\n第 1 步后   0.9\\n第 2 步后   0.9 × 0.9     = 0.81\\n第 3 步后   0.81 × 0.9    ≈ 0.73\\n第 5 步后   ≈ 0.59\\n第 10 步后  ≈ 0.35\\n第 15 步后  0.9^15        ≈ 0.21\\n\`\`\`\\n\\n15 步、每步 0.9，整体成功率只剩约 21%，五次里成不了一次。\\n\\n这个推导还隐含一个被低估的前提：每步正确率固定。实际往往更糟——任务越长，注意力越分散，靠后的列越容易错。下面的代码用两条曲线对照：单列正确率固定 0.99，与单列正确率随列数线性衰减（0.97 − 0.012 × 列号）。15 列时前者仍有 0.99^15 ≈ 0.86；后者因为越靠后单列越差，整体只剩约 0.13。衰减曲线在图上塌陷得更陡，这就是\\"乘法塌陷\\"名字的由来。"
  },
  {
   "cell_type": "code",
   "execution_count": null,
  ...
  },
  {
   ...
   "id": "cell_011",
   "metadata": {},
   "source": "### 自检与重试循环\\n\\n这一小节回答一个问题：单步正确率只有 0.65 时，怎么把整条任务的成功率救上来。思路是在循环里加一道自检：执行完一步，先验证结果对不对，对了就推进，错了就再试一次。\\n\\n先看重试的数学。假设每步错误的概率是 q=0.35，那么一次尝试正确的概率是 0.65。允许重试 3 次，也就是总共尝试 4 次。只有 4 次全错才会最终失败，失败概率是 q 的 4 次方：\\n\\n\`\`\`text\\n0.35 × 0.35     ≈ 0.122\\n0.122 × 0.35    ≈ 0.043\\n0.043 × 0.35    ≈ 0.015\\n\`\`\`\\n\\n约 1.5%。也就是说，只要验证器可靠，重试就能把\\"每步 65% 正确\\"抬到\\"约 98.5% 正确\\"。代价是多花几次 LLM 调用。\\n\\n但这个结论依赖一个前提：验证器真的分得清对错。如果验证器自己不会判断，重试就是在原地掷骰子——错了放行、对了也可能被拦下。验证信号的质量决定这个循环的上限。\\n\\n下面用一个具体任务对比三种配置。任务有 40 条加法表达式，真值确定，求解器单步错误率 0.35。\\n\\n- 不做自检：第一次求解结果直接接受。错误无处被拦截，正确率就是求解器本身的水平，一条表达式花一次调用。\\n- 让 LLM 自己复核（弱自检）：复核几乎总是说\\"看起来对\\"，对应 Self-Refine 论文的负结果——模型对 94% 的错误样本都说看起来对。弱自检拦不住错误，重试几乎不触发，多花的调用换不来正确率。\\n- 用精确验证器把关（硬验证）：重算真值精确比较，任何偏差都判错。错误被可靠拦截，重试真正生效，正确率接近 98.5%，代价是调用次数明显增加。\\n\\n真实 API 演示下求解算术会得到确定性的正确结果，错误率由 NoisyBrain 注入，便于做受控实验。用同一批表达式跑三种配置，比较\\"真正正确数\\"与\\"LLM 调用数\\"。循环自己报告的成败不可信，真正的正确数要用真值对一遍——这就是结果级监督要回答的问题，到第 3 节再展开。"
  },
  {
   "cell_type": "code",
   "execution_count": null,
  ...
  },
  {
   ...
   "id": "cell_024",
   "metadata": {},
   "source": "## 4. 开放问题：当人追不上模型\\n\\n前三节的可靠性、监督、信任，都假设有一个足够强的监督者：人足够强，能判断 agent 做得对不对。这一节面对一个正在逼近的疑问：如果模型在很多技能上超过未经辅助的人，人还怎么把关。\\n\\n这个问题没有现成答案，本节目前有两种思路。scalable oversight（可扩展监督）给出乐观路径，让\\"人 + LLM 助手\\"协作，超过任何一方单独的能力。AI control 给出防御路径，假设模型是敌意的、故意破坏安全措施，仍然要能安全部署。下面用一个小型可信监控协议，演示后一种思路的具体样子。"
  },
  {
   "cell_type": "markdown",
   "id": "cell_025",
   "metadata": {},
   "source": "### 最小 AI-control：可信监控协议\\n\\n这一小节回答一个问题：如果被监控的模型本身不可信，怎么保证部署安全。核心设想来自 Redwood 的 AI Control 工作：用可信但较弱的模型，监控不可信但较强的模型。\\n\\n先解释为什么监控者要\\"可信但弱\\"而不是\\"可信且强\\"。AI control 的前提是：强模型不可信，它可能就是出问题的那一方。监控者必须可靠、可被审查；即使能力弱一些，也不接受它同样会被针对。可信，是这个协议所有保证的地基。\\n\\n流程分三步。第一步，不可信模型提出大量候选动作，比如 50 个，其中约 20% 有害。第二步，可信监控器给每个动作打一个可疑度分数——现实中来自一个独立训练的弱模型，这里用特征的线性加权代替，两个特征越高越可疑。第三步，只把可疑度最高的 k 个送去人工审计，其余放行。人工预算有限，协议的作用是把预算花在最可能出问题的地方。\\n\\n按可疑度从高到低选，而不是随机抽，理由在数字里。本例 50 个动作、10 个有害，审计预算 20%（10 个）：随机抽平均只命中 2 个有害动作，按可疑度抽捕到了 7 个（安全率 0.70）。有害动作的特征整体更高、可疑度集中在顶端，弱模型在这里当筛子，把强模型动作里最可疑的一小撮筛给人类。\\n\\n人工审计本身也有上限。全量审计 50 个动作的安全率也只有 0.80——人类审计员按 85% 的命中率工作，即使用尽全力也拦不下所有有害动作。权衡曲线画出\\"审计成本 vs 安全率\\"：预算从 0 加到 20%，安全率从 0 跳到 0.70，曲线起点陡峭；再往上边际收益递减。可信监控加 top-k 的价值正是这个陡峭的起点——用很少的预算换到大部分安全。\\n\\n被动设置下协议表现良好，被刻意针对后就失效。绕过演示把同一个有害动作的特征从(高,高)改成(低,低)，可疑度从 1.71 掉到 0.14，它就从审计名单上掉下去、直接放行。同一份载荷、换一种输出写法，就躲过监控——这是\\"监控必须按被针对的前提来评估\\"的直接演示。防御思路叫 protocol security，中文可译作协议安全：攻击者会优化绕过，协议要针对最优攻击者做鲁棒性分析，而不是只对平均情况有效。"
  },
  {
   "cell_type": "code",
   "execution_count": null,
  ...
  },
  {
   ...
   "id": "cell_029",
   "metadata": {},
   "source": "上面的输出把绕过演示的过程具象化：同一个有害动作，攻击前可疑度 1.71、必然在审计名单上；换一种输出写法之后可疑度掉到 0.14，排在审计线以下，直接放行。同一套监控在被动设置下表现良好，被刻意针对后就失效。\\n\\n这个结果不是孤例。METR 的 Frontier Risk Report 记录了 44 起越界或欺骗事件，其中最严重的软肋是自动化监控本身——内嵌红队改一个环境变量，就让流量躲过监控。结论是监控的鲁棒性必须以\\"被针对\\"为前提来评估。\\n\\n把这一讲连起来看，能得出一个整体结论。当奖励拿不到真值、监控可以被绕过时，自治系统的上限由监督信号的质量决定，而不是由模型参数决定。这也是本讲把奖励与验证称作最根本瓶颈的原因。\\n\\nscalable oversight 与 AI control 是这条路上两条尚未汇合的路线：前者度量\\"人 + 助手\\"能否超过单独任何一方，后者要求在监督被攻击时仍然安全。两者共同的开放问题是，长时程自治的可靠评测如何建立，以及没有真值 reward 的任务如何规模化 RL。"
  },
  {
   "cell_type": "markdown",
   "id": "cell_030",
