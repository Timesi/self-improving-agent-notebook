const n=`{
 "cells": [
  ...
  },
  {
   "cell_type": "markdown",
   "id": "e42a9bfd",
   "source": "**实验：均匀切分 vs 分位数切分。** 上面验证的是 RT-2 式的均匀切分，把区间按最小值和最大值等分。它有一个弱点：假设动作值散布在整个区间。实际数据里，大多数动作值往往挤在一个小范围，偶尔出现一个偏离很远的极值，这个极值叫离群值。一个离群值就能把区间撑大，每个 bin 变宽，分辨率被浪费。OpenVLA 改用分位数切分，让 bin 边界跟着数据的密集程度走。下面代码用含离群值的合成数据对比两种方式的 bin 宽度。",
   "metadata": {}
  },
  {
   "cell_type": "code",
  ...
  },
  {
   "cell_type": "markdown",
   "id": "5ed7119d",
   "source": "**实验：训练 mini VLA。** 上一节的代码定义了模型，这一节给它一个可以学的任务。我们合成一个 toy 数据集：每张 8×8 图像有一个亮度值，每条指令带一个编号，目标动作 bin 由“亮度等级 + 指令编号”共同决定。模型必须同时用上图像和文字的信息才能预测正确。训练 200 步后，看验证集上 7 个动作槽位全部预测正确的比例。",
   "metadata": {}
  },
  {
   "cell_type": "code",
  ...
  },
  {
   "cell_type": "markdown",
   "id": "c5e55ad0",
   "source": "**实验：观察训练曲线与采样。** 上一节的训练留下了 loss 和验证集准确率两条曲线，这一节把它们画出来看收敛情况，再从验证集里采样几条动作 token。采样的意义在于：推理时模型不是总取概率最大的 token，而是按分布随机选，这一步要确认采样结果仍然落在动作词表内，能直接解码成连续动作。",
   "metadata": {}
  },
  {
   "cell_type": "code",
  ...
  {
   "cell_type": "markdown",
   "id": "e82cce57651c45abb97d860a25e19b12",
   "metadata": {},
  ...
  {
   "cell_type": "markdown",
   "id": "1eb0025be47141ad92b06eccd6b297ba",
   "metadata": {},
   "source": [
    "## 3. 机器人数据的采集与批量处理"
   ]
  ...
  },
  {
   ...
   "id": "cae58b80",
   "metadata": {},
   "source": "最后一个问题：语言推理怎么接上低层控制。前面模型直接输出动作 token，但很多任务需要先想清楚再动手。RT-2 的一个变体把每条数据扩成 \\"Instruction: I'm hungry. Plan: pick the snack. Action: 1 128 124 ...\\" 这样的格式，让模型先生成一句自然语言计划，再输出动作 token。这个做法借鉴了思维链（chain-of-thought）的思路，写作 CoT。语言计划承接 web 预训练带来的推理能力，动作 token 则约束在动作词表内。下面用 llm_client 走一遍这个链式格式，并把动作字符串反解回连续动作。"
  },
  {
   "cell_type": "markdown",
   "id": "fff5def4ffc5464693217cdd15493726",
   "metadata": {},
