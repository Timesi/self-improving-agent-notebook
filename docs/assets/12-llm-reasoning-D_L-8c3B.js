const n=`{
 "cells": [
  ...
  },
  {
   ...
   "id": "a4074a461123",
   "metadata": {},
   "source": "> 前面几讲，Agent 已经把大任务拆成多步，用工具执行任务，靠记忆管理长上下文。这些环节都依赖模型在单次续写里给出合理的下一步。一旦任务本身需要多步推理才能算对，单次续写常常直接给错答案，扩大模型规模也看不到明显进步。\\n>\\n> 这一讲回答推理能力从哪里来，以及不动权重的前提下怎么把它激发出来。我们先实现\\"让模型把中间步骤写出来\\"的提示，再做去掉中间步骤的对比实验，然后实现\\"多试几次再投票\\"的解码方法，最后用我们生成的模拟数据检查一个说法：某些能力随模型变大\\"突然出现\\"，这个现象可能只是度量方式造成的假象。"
  },
  {
   "cell_type": "markdown",
   "id": "6008f308634a",
   "metadata": {},
   "source": "我们都有过这样的体验：在 ChatGPT 或 DeepSeek 里问一道复杂的算术题，模型不会立刻给答案，而是先写一句\\"让我想想\\"或\\"我们一步一步来\\"，然后拆成几小步慢慢算。在整套 Agent 框架（感知、决策、行动、反馈）里，模型每一次做决策之前，都要先把问题想清楚；这一讲补的正是\\"决策\\"里\\"怎么想\\"这块能力。学完这一讲，我们能自己写出\\"让模型分步思考\\"的提示，能理解为什么分步有效，还能动手实现一种\\"多试几次再投票\\"的解码方法。\\n\\n模型的输出是一小段一小段拼出来的，每一小段叫 token，通常是半个词或一个词；一整段话，就是模型一个 token 一个 token 接着写出来的。这种自动补全的能力来自预训练时见过的海量文字。于是有两种情况要分清。如果一道题在训练时见过，模型能把答案背出来；如果没见过，它只能猜一个听起来合理的数字。会背答案，是重复见过的东西；会推理，是没见过的问题也能一步步推出答案。区分它们的一个办法，是看模型能不能写出中间步骤：能一步步写出来，说明它在推；直接报一个数字，多半是在猜。\\n\\n推理能不能被看见，需要一个测试场。GSM8K 是小学程度的数学应用题集，模型在这类题目上的准确率长期偏低，是测试\\"会不会推\\"的常用数据集。CoT 论文（Wei 等人，2022）发现一个矛盾：同一个模型、同样的参数、同样的训练，只是把提示里的样例从\\"题目-答案\\"改成\\"题目-推理步骤-答案\\"，准确率就明显上涨。这说明一部分推理能力本来就存在，只是缺少一种合适的提示格式把它调出来。真实系统里也是这个用法：在提示词里加一句\\"让我们一步一步想\\"，模型就会先写推理过程，再给最终答案。这种\\"先写中间步骤、再给答案\\"的做法，就叫思维链（chain-of-thought，简称 CoT）。\\n\\n这一讲要做的第一件事，就是亲手验证这个矛盾。我们要构造两组提示，一组直接要答案，一组让模型先写步骤，然后对比两组输出。第一节从构造这两组提示开始。"
  },
  {
   "cell_type": "markdown",
   "id": "64638d1a7fbc",
   "metadata": {},
   "source": "这一节要做的是一件很具体的事：准备一个小题库，写出两组提示，一组直接要答案，一组让模型先写中间步骤，然后看模型各怎么回答。前面我们说过，分步思考能把模型已有的能力调出来，这一节就动手验证这句话。原理上一句话就能说清：预训练把语言规律写进了权重，推理能力也包含其中，提示格式决定模型以什么方式调用这些权重。真实系统里，我们发给 ChatGPT 的那段话就是提示，本节先看清它的结构。"
  },
  {
   "cell_type": "markdown",
   "id": "0998a6124f1e",
  ...
  },
  {
   ...
   "id": "9eb4fc38bc54",
   "metadata": {},
   "source": "两条提示的最大差别在输出路径。标准提示下，模型一步输出最终数字；思维链提示下，模型先输出逐步计算，再用\\"最终答案是\\"收尾。下面实现一个解析函数，从这两类输出里提取答案数字，然后用 llm_client 各跑一遍。没有配置 API key 时自动进入真实 API 演示，模型输出是占位文本，跑通的是整条解析与对比的管线。"
  },
  {
   "cell_type": "code",
   "execution_count": null,
  ...
  },
  {
   ...
   "id": "5b10e3d9fa8e",
   "metadata": {},
   "source": "上一节看到思维链有效，这一节要弄清这个增益来自哪一部分。一个直接的办法，是把中间步骤换一种形式，看效果还在不在。这种\\"去掉或替换一部分，观察变化\\"的实验方法叫消融（ablation）。CoT 论文做了一组消融：把中间步骤换成纯方程、换成省略号、或挪到答案之后，三种改法增益全部消失。剩下的唯一变量，是\\"用自然语言按顺序把步骤写出来\\"这件事本身。下面先用手算的玩具模型复现这个结论，再实现一种\\"多试几次再投票\\"的解码方法。"
  },
  {
   "cell_type": "markdown",
   "id": "2031c491af7d",
   "metadata": {},
   "source": "先建立数量感，也就是把\\"思维链有没有用\\"变成两个可以比的数字。设一道题需要 L 个顺序步骤。直接给答案的模型一次做对的概率是 p 直；分步模型每步做对概率是 p 步，最终正确需要 L 步全对，概率为 p 步的 L 次方。取 p 直 = 0.25、p 步 = 0.80、L = 3，先算两个数。\\n\\n三种消融各自去掉中间步骤的一部分。只给方程：样例里只有最终算式，模型仍要一次算完；只给点：步骤用省略号占位，相当于只提示\\"可以多算\\"，不提示怎么算；推理在答案之后：先给最终答案再补推理，模型生成答案时用不到步骤。三种变体都回到\\"一次作答\\"，所以它们的概率都用 p 直。"
  },
  {
   "cell_type": "code",
   "execution_count": null,
  ...
  },
  {
   ...
   "id": "7baf20526247",
   "metadata": {},
   "source": "一次思维链解码只走一条推理路径，某一步出错就一路带到终点。改进的思路是让模型多走几条路：从同一个提示出发，多次采样得到好几条回答，每条回答都算出一个最终答案，最后数一数哪个答案出现得最多，最多的胜出。多条路径在同一个答案上彼此一致，这个做法叫自洽解码（self-consistency）。它为什么有效，直觉是这样：一道多步推理题，通常有好几条不同的正确路径都通向同一个答案，正确路径会在答案处汇合；而错误路径各有各的错法，很少同时指向同一个错误数字。我们先用一个只有两个答案的设定手算，再在多个错误答案的设定下模拟。"
  },
  {
   "cell_type": "code",
   "execution_count": null,
  ...
  },
  {
   ...
   "id": "2cb44fdae527",
   "metadata": {},
   "source": "投票的收益依赖采样路径的多样性：路径必须各不相同，投票才有意义。一个控制多样性的旋钮叫温度（temperature）。温度越高，模型采样越随机，路径越容易分岔；温度接近 0 时，采样退化成每次都选概率最大的那个词，这种生成方式叫贪婪解码，K 条路径会彼此相同，投票形同虚设。但温度也不是越高越好：分布越平，每条路径单独做对的概率也在下降。下面用一个我们自己写的采样器，把这两个作用都写成温度的函数，观察多数投票准确率随温度的变化。"
  },
  {
   "cell_type": "markdown",
   "id": "7e4f50617283",
  ...
  },
  {
   ...
   "id": "5df36820e373",
   "metadata": {},
   "source": "多条路径的票数分布本身就是信息。直觉上，多条路径都指向同一个答案，说明模型对这个答案很有把握；路径各说各的，说明模型拿不准。把这种把握量化：先看票最多的答案占了多少票，算出它的占比，占比越接近 1，说明路径越一致。这个占比就叫一致性分数。我们合成一批难易混合的题目，验证一致性分数和\\"多数投票是否正确\\"是不是相关，再演示一个简单策略：一致性低于某个阈值的题，宁可放过不答。在 Agent 框架里，这个信号正是模型判断\\"什么时候不该自己拿主意\\"的依据。"
  },
  {
   "cell_type": "code",
   "execution_count": null,
  ...
  },
  {
   ...
   "id": "05d5272af7df",
   "metadata": {},
   "source": "这场争论留下可操作的经验：底层能力按平滑曲线增长，而用户关心的任务级准确率由复合过程决定；看起来的跳变，是复合过程被二值度量放大后的结果。涌现论文用另一类证据补上反方没说到的部分。训练时有一个量衡量模型预测和真实答案的差距，叫损失，交叉熵是常见的一种。用交叉熵看，小规模段的损失确实在稳步下降，说明能力在积累；而且分类任务用精确匹配度量，也会出现涌现。这两点说明，度量并不能完全解释跳变。更稳妥的读法是两边各占一半：能力平滑积累，度量决定它如何被看见。"
  },
  {
   "cell_type": "markdown",
   "id": "ad9eef6b0fe5",
   "metadata": {},
   "source": "前几节做的都是在提示层面激发推理：用户构造样例、调温度、做投票，能力上限受提示约束，换一道新题常常要重新设计提示。2024 年起的推理模型（OpenAI o1、DeepSeek-R1）走的是另一条路：把\\"多步推理\\"搬进训练阶段，让模型自己学会在内部思考。训练用的方法叫强化学习：给模型的回答打分，回答得好就鼓励，回答得差就惩罚，模型在反复练习中学会把思维链写得更长更准。这样训练完之后，多步推理已经内化成权重的一部分，推理时只需一句简短指令，思维链不再由提示提供，而是由模型自己生成。"
  },
  {
   "cell_type": "markdown",
   "id": "7148019b521e",
